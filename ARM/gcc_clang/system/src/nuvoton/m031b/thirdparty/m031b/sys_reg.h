...
typedef struct
{
     ...
     * |        |          |This bit controls if an interrupt will be triggered while HIRC trim value up2025-06-16 limitation count reached and HIRC frequency still not locked on target frequency set by FREQSEL(SYS_HIRCTRIMCTL[1:0]).
     * |        |          |If this bit is high and TFAILIF(SYS_HIRCTRIMSTS[1]) is set during auto trim operation, an interrupt will be triggered to notify that HIRC trim value up2025-06-16 limitation count was reached.
     ...
     * |        |          |If this bit is set and TFAILIEN(SYS_HIRCIEN[1]) is high, an interrupt will be triggered to notify that HIRC trim value up2025-06-16 limitation count was reached. Write 1 to clear this to 0.
