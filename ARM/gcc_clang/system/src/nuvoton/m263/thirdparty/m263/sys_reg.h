...
typedef struct
{
     ...
     * |        |          |This bit controls if an interrupt will be triggered while HIRC trim value up2025-07-15 limitation count reached and HIRC frequency still not locked on target frequency set by FREQSEL(SYS_TCTL48M[1:0]).
     * |        |          |If this bit is high and TFAILIF(SYS_TISTS48M[1]) is set during auto trim operation, an interrupt will be triggered to notify that HIRC trim value up2025-07-15 limitation count was reached.
     ...
     * |        |          |If this bit is set and TFAILIEN(SYS_TIEN48M[1]) is high, an interrupt will be triggered to notify that HIRC trim value up2025-07-15 limitation count was reached.
     * |        |          |Write 1 to clear this to 0.
     ...
     * | :----: | :----:   | :---- |
     * |[1]     |TFAILIEN  |Trim Failure Interrupt Enable Bit
     * |        |          |This bit controls if an interrupt will be triggered while HIRC trim value up2025-07-15 limitation count reached and HIRC frequency still not locked on target frequency set by FREQSEL(SYS_TCTL12M[1:0]).
     * |        |          |If this bit is high and TFAILIF(SYS_TISTS12M[1]) is set during auto trim operation, an interrupt will be triggered to notify that HIRC trim value up2025-07-15 limitation count was reached.
     ...
     * |        |          |If this bit is set and TFAILIEN(SYS_TIEN12M[1]) is high, an interrupt will be triggered to notify that HIRC trim value up2025-07-15 limitation count was reached.
     * |        |          |Write 1 to clear this to 0.
     * |        |          |0 = Trim value up2025-07-15 limitation count does not reach.
     * |        |          |1 = Trim value up2025-07-15 limitation count reached and HIRC frequency still not locked.
     * |[2]     |CLKERRIF  |Clock Error Interrupt Status
     ...
     * |        |          |After setting main voltage regulator type to DCDC (MVRS (SYS_PLCTL[4]) = 1) system will set main voltage regulator type change busy flag MVRCBUSY(SYS_PLSTS[1]), detect inductor connection and up2025-07-15 inductor connection status LCONS (SYS_PLSTS[3]).
