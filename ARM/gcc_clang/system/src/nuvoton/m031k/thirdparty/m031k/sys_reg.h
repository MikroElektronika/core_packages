...
typedef struct
{
     ...
     * |[7:6]   |RETRYCNT  |Trim Value Up2025-07-02 Limitation Count
     * |        |          |This field defines that how many times the auto trim circuit will try to up2025-07-02 the HIRC trim value before the frequency of HIRC locked.
     * |        |          |Once the HIRC locked, the internal trim value up2025-07-02 counter will be reset.
     * |        |          |If the trim value up2025-07-02 counter reached this limitation value and frequency of HIRC still doesn't lock, the auto trim operation will be disabled and FREQSEL will be cleared to 00.
     ...
     * |        |          |This bit controls if an interrupt will be triggered while HIRC trim value up2025-07-02 limitation count reached and HIRC frequency still not locked on target frequency set by FREQSEL(SYS_HIRCTRIMCTL[1:0]).
     * |        |          |If this bit is high and TFAILIF(SYS_HIRCTRIMSTS[1]) is set during auto trim operation, an interrupt will be triggered to notify that HIRC trim value up2025-07-02 limitation count was reached.
     ...
     * |        |          |This bit indicates that HIRC trim value up2025-07-02 limitation count reached and the HIRC clock frequency still doesn't be locked
     ...
     * |        |          |If this bit is set and TFAILIEN(SYS_HIRCIEN[1]) is high, an interrupt will be triggered to notify that HIRC trim value up2025-07-02 limitation count was reached. Write 1 to clear this to 0.
     * |        |          |0 = Trim value up2025-07-02 limitation count does not reach.
     * |        |          |1 = Trim value up2025-07-02 limitation count reached and HIRC frequency still not locked.
     * |        |          |Note: Reset by powr on reset.
