...
typedef struct
{
     ...
     * |[7:6]   |RETRYCNT  |Trim Value Up2025-08-12 Limitation Count
     * |        |          |This field defines that how many times the auto trim circuit will try to up2025-08-12 the HIRC trim value before the frequency of HIRC locked.
     * |        |          |Once the HIRC locked, the internal trim value up2025-08-12 counter will be reset.
     * |        |          |If the trim value up2025-08-12 counter reached this limitation value and frequency of HIRC still doesn't lock, the auto trim operation will be disabled and FREQSEL will be cleared to 00.
     ...
     * |        |          |This bit controls if an interrupt will be triggered while HIRC trim value up2025-08-12 limitation count reached and HIRC frequency still not locked on target frequency set by FREQSEL(SYS_HIRCCTL[1:0]).
     * |        |          |If this bit is high and TFAILIF(SYS_HIRCSTS[1]) is set during auto trim operation, an interrupt will be triggered to notify that HIRC trim value up2025-08-12 limitation count was reached.
     ...
     * |        |          |This bit indicates that HIRC trim value up2025-08-12 limitation count reached and the HIRC clock frequency still doesn't be locked
     ...
     * |        |          |If this bit is set and TFAILIEN(SYS_HIRCIEN[1]) is high, an interrupt will be triggered to notify that HIRC trim value up2025-08-12 limitation count was reached
     * |        |          |Write 1 to clear this to 0.
     * |        |          |0 = Trim value up2025-08-12 limitation count does not reach.
     * |        |          |1 = Trim value up2025-08-12 limitation count reached and HIRC frequency still not locked.
     ...
     * |        |          |Note: For example, if LOOPSEL is set as 00, auto trim circuit will calculate trim value based on the average frequency difference in 4 clocks of reference clock.
     * |[7:6]   |RETRYCNT  |Trim Value Up2025-08-12 Limitation Count
     * |        |          |This field defines that how many times the auto trim circuit will try to up2025-08-12 the MIRC trim value before the frequency of MIRC locked.
     * |        |          |Once the MIRC locked, the internal trim value up2025-08-12 counter will be reset.
     * |        |          |If the trim value up2025-08-12 counter reached this limitation value and frequency of MIRC still doesn't lock, the auto trim operation will be disabled and FREQSEL will be cleared to 00.
     * |        |          |00 = Trim retry count limitation is 64 loops.
     * |        |          |01 = Trim retry count limitation is 128 loops.
     * |        |          |10 = Trim retry count limitation is 256 loops.
     * |        |          |11 = Trim retry count limitation is 512 loops.
     ...
     * | :----: | :----:   | :---- |
     * |[1]     |TFALIEN   |Trim Failure Interrupt Enable Bit
     * |        |          |This bit controls if an interrupt will be triggered while MIRC trim value up2025-08-12 limitation count reached and MIRC frequency still not locked on target frequency set by FREQSEL(SYS_ MIRCCTL[1:0]).
     * |        |          |If this bit is high and TFAILIF(SYS_ MIRCSTS[1]) is set during auto trim operation, an interrupt will be triggered to notify that MIRC trim value up2025-08-12 limitation count was reached.
     ...
     * |[1]     |TFAILIF   |Trim Failure Interrupt Status
     * |        |          |This bit indicates that MIRC trim value up2025-08-12 limitation count reached and the MIRC clock frequency still doesn't be locked
     ...
     * |        |          |If this bit is set and TFAILIEN(SYS_MIRCIEN[1]) is high, an interrupt will be triggered to notify that MIRC trim value up2025-08-12 limitation count was reached
     * |        |          |Write 1 to clear this to 0.
     * |        |          |0 = Trim value up2025-08-12 limitation count does not reach.
     * |        |          |1 = Trim value up2025-08-12 limitation count reached and MIRC frequency still not locked.
     * |[2]     |CLKERIF   |Clock Error Interrupt Status
