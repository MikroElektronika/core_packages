...
typedef struct
{
     ...
     * |[7:6]   |RETRYCNT  |Trim Value Up2025-06-05 Limitation Count
     * |        |          |This field defines that how many times the auto trim circuit will try to up2025-06-05 the HIRC trim value before the frequency of HIRC locked.
     * |        |          |Once the HIRC is locked, the internal trim value up2025-06-05 counter will be reset.
     * |        |          |If the trim value up2025-06-05 counter reaches this limitation value and the frequency of HIRC is still not locked, the auto trim operation will be disabled and FREQSEL will be cleared to 00.
     ...
     * |        |          |This bit controls if an interrupt will be triggered while HIRC trim value up2025-06-05 limitation count reached and HIRC frequency still not locked on target frequency set by FREQSEL(SYS_IRCTCTL[1:0]).
     * |        |          |If this bit is high and TFAILIF(SYS_IRCTISTS[1]) is set during auto trim operation, an interrupt will be triggered to notify that HIRC trim value up2025-06-05 limitation count was reached.
     ...
     * |        |          |This bit indicates that HIRC trim value up2025-06-05 limitation count is reached and the HIRC clock frequency is still not locked
     ...
     * |        |          |If this bit is set and TFAILIEN(SYS_IRCTIEN[1]) is high, an interrupt will be triggered to notify that HIRC trim value up2025-06-05 limitation count was reached
     * |        |          |Write 1 to clear this to 0.
     * |        |          |0 = Trim value up2025-06-05 limitation count is not reached.
     * |        |          |1 = Trim value up2025-06-05 limitation count is reached and HIRC frequency is still not locked.
