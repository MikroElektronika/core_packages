...
void _sysctrl_init_sources(void)
{
    ...
    #endif
                                    SYSCTRL_OSC8M_PRESC(osc8m_prescaler)
                                    | (osc8m_runstdby << SYSCTRL_OSC8M_RUNSTDBY_Pos)
                                    | (osc8m_enable << SYSCTRL_OSC8M_ENABLE_Pos));
    #endif
    ...
    hri_sysctrl_write_OSC32K_reg(
        ...
        #endif
                (osc32k_wrtlock << SYSCTRL_OSC32K_WRTLOCK_Pos) | SYSCTRL_OSC32K_STARTUP(osc32k_startup)
                | (osc32k_runstdby << SYSCTRL_OSC32K_RUNSTDBY_Pos)
                | (osc32k_en32k << SYSCTRL_OSC32K_EN32K_Pos) | (osc32k_enable << SYSCTRL_OSC32K_ENABLE_Pos));
    #else
