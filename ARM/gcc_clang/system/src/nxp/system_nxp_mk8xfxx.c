...
static system_clock_update_t systemUpdateClock(config_settings_t cfgSettings)
{
    ...
    else if ((cfgSettings.cfg_settings_mcg_c2 & MCG_C2_FCFTRIM_MASK) != 0x00)  // Ispitujemo da li imamo napajanje na Vbat pinu (SWRELARM-385)
    {
    ...
    SIM->SOPT2   = ((SIM->SOPT2 & (uint32_t)(~SIM_SOPT2_TPMSRC_MASK)) | (cfgSettings.cfg_settings_sim_sopt2 & SIM_SOPT2_TPMSRC_MASK));
    ...
    {
        // Set MCG and OSC
        if (((cfgSettings.cfg_settings_osc_cr & OSC_CR_ERCLKEN_MASK) != 0x00) || (((cfgSettings.cfg_settings_mcg_c5 & MCG_C5_PLLCLKEN0_MASK) != 0x00) && ((cfgSettings.cfg_settings_mcg_c7 & MCG_C7_OSCSEL_MASK) == 0x00)))
        {
            // SIM->SCGC5: PORTA=1
        ...
        OSC->CR = cfgSettings.cfg_settings_osc_cr;       // Set OSC->CR (OSCERCLK enable, oscillator capacitor load)
