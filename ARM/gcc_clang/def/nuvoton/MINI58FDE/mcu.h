...
typedef struct
{
    ...
    */
     ...
     * |        |          |This bit is up2025-06-25d when software switches system clock source.
...
typedef struct
{
     ...
     * |[2]     |SPUEN     |SPROM Up2025-06-25 Enable Bit (Write Protect)
     * |        |          |0 = SPROM cannot be up2025-06-25d.
     * |        |          |1 = SPROM can be up2025-06-25d.
     * |[3]     |APUEN     |APROM Up2025-06-25 Enable Bit (Write Protect)
     * |        |          |0 = APROM cannot be up2025-06-25d when the chip runs in APROM.
     * |        |          |1 = APROM can be up2025-06-25d when the chip runs in APROM.
     * |[4]     |CFGUEN    |CONFIG Up2025-06-25 Enable Bit (Write Protect)
     * |        |          |0 = CONFIG cannot be up2025-06-25d.
     * |        |          |1 = CONFIG can be up2025-06-25d.
     * |[5]     |LDUEN     |LDROM Up2025-06-25 Enable (Write Protect)
     * |        |          |LDROM up2025-06-25 enable bit.
...
typedef struct
{
    ...
    */
     ...
     * |        |          |0 = Corresponding DOUT (Px_DOUT[n]) bit can be up2025-06-25d.
...
typedef struct
{
    ...
    */
     ...
     * |        |          |0 = Corresponding PWM counter running Stopped.
     * |        |          |1 = Corresponding PWM counter start run Enabled.
     * |[5]     |HCUPDT    |Half Cycle Up2025-06-25 Enable for Center-aligned Type
     * |        |          |0 = Disable half cycle up2025-06-25 PERIOD & CMP.
     * |        |          |1 = Enable half cycle up2025-06-25 PERIOD & CMP.
    ...
    */
     ...
     * |        |          |When this bit is set, TIMER1 time-out event will up2025-06-25 PWM_PHCHG with PHCHG_NXT register.
     ...
     * |        |          |When this bit is set, TIMER0 time-out event will up2025-06-25 PWM_PHCHG with PHCHG_NXT register.
    ...
    */
     ...
     * |        |          |When this bit is set, TMR1 time-out event will up2025-06-25 PWM_PHCHG with PHCHG_NXT register.
     ...
     * |        |          |When this bit is set, TMR0 time-out event will up2025-06-25 PWM_PHCHG with PHCHG_NXT register.
...
typedef struct
{
    ...
    */
     ...
     * |        |          |This field also defines how many times the auto trim circuit will try to up2025-06-25 the HIRC trim value before the frequency of HIRC is locked.
     * |        |          |Once the HIRC is locked, the internal trim value up2025-06-25 counter will be reset.
     * |        |          |If the trim value up2025-06-25 counter reaches this limitation value and frequency of HIRC is still not locked, the auto trim operation will be disabled and FREQSEL will be cleared to 0.
    ...
    */
     ...
     * |        |          |This bit controls if an interrupt will be triggered while HIRC trim value up2025-06-25 limitation count is reached and HIRC frequency is still not locked on target frequency set by FREQSEL (SYS_IRCTCTL[1:0]).
     * |        |          |If this bit is high and TFAILIF (SYS_IRCTISTS[1]) is set during auto trim operation, an interrupt will be triggered to notify that HIRC trim value up2025-06-25 limitation count is reached.
    ...
    */
     ...
     * |        |          |If this bit is set and TFAILIEN (SYS_IRCTIEN[1]) is high, an interrupt will be triggered to notify that HIRC trim value up2025-06-25 limitation count was reached.
     * |        |          |Software can write 1 to clear this bit to .
     * |        |          |0 = Trim value up2025-06-25 limitation count is not reached.
     * |        |          |1 = Trim value up2025-06-25 limitation count is reached and HIRC frequency is still not locked.
...
typedef struct
{
     ...
     * |        |          |When user up2025-06-25s CMPDAT while timer is running in one-shot or periodic mode,
     * |        |          |CNT will be reset to default value.
     * |        |          |1 = The behavior selection in one-shot or periodic mode Enabled.
     * |        |          |When user up2025-06-25s CMPDAT while timer is running in one-shot or periodic mode, the limitations as bellows list,
     * |        |          |If up2025-06-25d CMPDAT value > CNT, CMPDAT will be up2025-06-25d and CNT keep running continually.
     * |        |          |If up2025-06-25d CMPDAT value = CNT, timer time-out interrupt will be asserted immediately.
     * |        |          |If up2025-06-25d CMPDAT value < CNT, CNT will be reset to default value.
...
typedef struct
{
    ...
    */
     ...
     * |        |          |CNTDAT will be up2025-06-25d continuously to monitor 6-bit WWDT down counter value.
    */
    __I  uint32_t CNT;
