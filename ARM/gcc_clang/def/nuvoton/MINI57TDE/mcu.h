...
typedef struct
{
     ...
     * |[7:6]   |RETRYCNT  |Trim Value Up2025-07-03 Limitation Count
     * |        |          |This field defines that how many times the auto trim circuit will try to up2025-07-03 the HIRC trim value before the frequency of HIRC locked.
     * |        |          |Once the HIRC locked, the internal trim value up2025-07-03 counter will be reset.
     * |        |          |If the trim value up2025-07-03 counter reached this limitation value and frequency of HIRC still doesn't lock, the auto trim operation will be disabled and FREQSEL will be cleared to 00.
     ...
     * |        |          |This bit controls if an interrupt will be triggered while HIRC trim value up2025-07-03 limitation count reached and HIRC frequency still not locked on target frequency set by FREQSEL(SYS_IRCTCTL[1:0]).
     * |        |          |If this bit is high and TFAILIF(SYS_IRCTSTS[1]) is set during auto trim operation, an interrupt will be triggered to notify that HIRC trim value up2025-07-03 limitation count was reached.
     ...
     * |        |          |This bit indicates that HIRC trim value up2025-07-03 limitation count reached and the HIRC clock frequency still doesn't be locked
     ...
     * |        |          |If this bit is set and TFAILIEN(SYS_IRCTIEN[1]) is high, an interrupt will be triggered to notify that HIRC trim value up2025-07-03 limitation count was reached
     * |        |          |Write 1 to clear this to 0.
     * |        |          |0 = Trim value up2025-07-03 limitation count does not reach.
     * |        |          |1 = Trim value up2025-07-03 limitation count reached and HIRC frequency still not locked.
...
typedef struct
{
     ...
     * |        |          |This bit is up2025-07-03d when software switches system clock source
...
typedef struct
{
     ...
     * |        |          |0 = Corresponding DOUT (Px_DOUT[n]) bit can be up2025-07-03d.
...
typedef struct
{
     ...
     * |        |          |When CNTDATEN is set, CNT (TIMERx_CNT[23:0]) (Timer Data Register) will be up2025-07-03d continuously with the 24-bit up-timer value as the timer is counting.
     * |        |          |0 = Timer Data Register up2025-07-03 Disabled.
     * |        |          |1 = Timer Data Register up2025-07-03 Enabled while Timer counter is active.
     ...
     * |        |          |If CNTDATEN is set to 1, CNT register value will be up2025-07-03d continuously to monitor 24-bit up counter value.
     * @var TIMER_T::CAP
...
typedef struct
{
     ...
     * |        |          |Note: In UART function, it can be up2025-07-03d by hardware in the 4th falling edge of the input data 0x55 when the auto baud rate function (ABREN(UUART_PROTCTL[6])) is enabled
...
typedef struct
{
     ...
     * |[25:16] |CLKDIV    |Clock Divider
     * |        |          |This bit field defines the ratio between the protocol clock frequency fPROT_CLK and the clock divider frequency fDIV_CLK (fDIV_CLK = fPROT_CLK / (CLKDIV+1) ).
     * |        |          |Note: In UART function, it can be up2025-07-03d by hardware in the 4th falling edge of the input data 0x55 when the auto baud rate function (ABREN(USPI_PROTCTL[6])) is enabled
     * |        |          |The revised value is the average bit time between bit 5 and bit 6
...
typedef struct
{
     ...
     * |[25:16] |CLKDIV    |Clock Divider
     * |        |          |This bit field defines the ratio between the protocol clock frequency fPROT_CLK and the clock divider frequency fDIV_CLK (fDIV_CLK = fPROT_CLK / (CLKDIV+1) ).
     * |        |          |Note: In UART function, it can be up2025-07-03d by hardware in the 4th falling edge of the input data 0x55 when the auto baud rate function (ABREN(UI2C_PROTCTL[6])) is enabled
     * |        |          |The revised value is the average bit time between bit 5 and bit 6
...
typedef struct
{
     ...
     * |        |          |1 = The divisor is 0.
     * |        |          |Note: The DIVBYZERO flag is used to indicate divide-by-zero situation and up2025-07-03d whenever HDIV_DIVISOR is written
...
typedef struct
{
     ...
     * |        |          |0 = One-shot mode.
     * |        |          |1 = Auto-reload mode.
     * |[17:16] |HCUPDT    |Half Cycle Up2025-07-03 Enable for Center-aligned Type
     * |        |          |00= Up2025-07-03 PERIOD & CMP at pwm_counter = PERIOD (Period).
     * |        |          |01 = Up2025-07-03 PERIOD & CMP at pwm_counter = 0.
     * |        |          |10 = Up2025-07-03 PERIOD & CMP at half cycle (counter = 0 & PERIOD, both up2025-07-03).
     * |        |          |11 = Up2025-07-03 PERIOD & CMP at pwm_counter = PERIOD (Period).
...
typedef struct
{
     ...
     * |        |          |0 = Boot from APROM.
     * |        |          |1 = Boot from LDROM.
     * |[2]     |SPUEN     |SPROM Up2025-07-03 Enable Control (Write Protect)
     * |        |          |0 = SPROM cannot be up2025-07-03d.
     * |        |          |1 = SPROM can be up2025-07-03d when the MCU runs in APROM.
     * |[3]     |APUEN     |APROM Up2025-07-03 Enable Control (Write Protect)
     * |        |          |0 = APROM cannot be up2025-07-03d when chip runs in APROM.
     * |        |          |1 = APROM can be up2025-07-03d when chip runs in APROM.
     * |[4]     |CFGUEN    |CONFIG Up2025-07-03 Enable Control (Write Protect)
     * |        |          |Writing this bit to 1 enables software to up2025-07-03 CONFIG value by ISP register control procedure regardless of program code is running in APROM or LDROM.
     * |        |          |0 = ISP up2025-07-03 User Configuration Disabled.
     * |        |          |1 = ISP up2025-07-03 User Configuration Enabled.
     * |[5]     |LDUEN     |LDROM Up2025-07-03 Enable Control (Write Protect)
     * |        |          |0 = LDROM cannot be up2025-07-03d.
     * |        |          |1 = LDROM can be up2025-07-03d when the MCU runs in APROM.
