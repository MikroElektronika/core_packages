...
typedef struct
{
     ...
     * |        |          |1 = PERIOD/CMPDAT will load to PBUF and CMPBUF immediately when software up2025-07-01 PERIOD/CMPDAT.
     ...
     * |        |          |Each bit n controls the corresponding BPWM channel n.
     * |        |          |0 = Capture function Disabled. RCAPDAT/FCAPDAT register will not be up2025-07-01d.
...
typedef struct
{
     ...
     * |        |          |The unused code '7' may be written by the CPU to check for up2025-07-01s.
...
typedef struct
{
     ...
     * |        |          |0 = boot from APROM
     * |        |          |1 = boot from LDROM
     * |[3]     |APUEN     |APROM Up2025-07-01 Enable Bit (Write Protect)
     * |        |          |0 = APROM cannot be up2025-07-01d when the chip runs in APROM.
     * |        |          |1 = APROM can be up2025-07-01d when the chip runs in APROM.
     * |[4]     |CFGUEN    |Config Up2025-07-01 Enable (Write Protect)
     * |        |          |Writing this bit to 1 enables s/w to up2025-07-01 Config value by ISP procedure regardless of program
     * |        |          |code is running in APROM or LDROM.
     * |        |          |0 = Config up2025-07-01 disable
     * |        |          |1 = Config up2025-07-01 enable
     * |[5]     |LDUEN     |LDROM Up2025-07-01 Enable (Write Protect)
     * |        |          |LDROM up2025-07-01 enable bit.
     * |        |          |0 = LDROM cannot be up2025-07-01d
     * |        |          |1 = LDROM can be up2025-07-01d when chip runs in APROM.
...
typedef struct
{
     ...
     * |        |          |0 = Corresponding GPIOx_DOUT[n] bit can be up2025-07-01d.
...
typedef struct
{
     ...
     * |        |          |        controller attempts to transmit data to a master, the transmit data register should be up2025-07-01d
...
typedef struct
{
     ...
     * |        |          |When TDR_EN is set, TDR (Timer Data Register) will be up2025-07-01d continuously with the 24-bit up-timer value as the timer is counting.
     * |        |          |0 = Timer Data Register up2025-07-01 Disabled.
     * |        |          |1 = Timer Data Register up2025-07-01 Enabled while Timer counter is active.
     ...
     * |        |          |If TDR_EN (TCSR[16]) is set to 1, TDR register will be up2025-07-01d continuously to monitor 24-bit up counter value.
     */
    __I  uint32_t  TDR;
...
typedef struct
{
     ...
     * |[3]     |LINS_DUM_EN|LIN Slave Divider Up2025-07-01 Method Enable Control
     * |        |          |0 = UA_BAUD up2025-07-01d is written by software (if no automatic resynchronization up2025-07-01 occurs at the same time).
     * |        |          |1 = UA_BAUD is up2025-07-01d at the next received character. User must set the bit before checksum reception.
     * |        |          |Note1: This bit only valid when in LIN slave mode (LINS_EN (UA_LIN_CTL[0]) = 1).
...
typedef struct
{
     ...
     * |        |          |WWDTCVAL will be up2025-07-01d continuously to monitor 6-bit down counter value.
     */
