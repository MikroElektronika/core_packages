void _FloatToSignedIntegral()
{
    ...
    __me_ovfl:
        CMP        R0, #0                    // IF OVERFLOW, RETURN INFINITY
...
void _SignedIntegralToFloat()
{
    asm {
        ...
        BMI       __me_lab1
    __me_loop:
        SUBS      R2, R2, #1                // ADJUSTING THE EXPONENT, ACCORDINGLY
        LSLS      R1, R1, #1                // NORMALIZE THE MANTISSA
        BPL       __me_loop

    __me_lab1:
        ADDS      R1, R1, #0x00000080        // ADD 1/2 TO ROUND
        ITE       CS
...
void _LongDoubleToFloat()
{
    ...
    __me_ovfl:
        AND        R1, R1, #0x80000000      // OVERFLOW, RETURN +/- INF
        MOVS       R0, #0xFF
        LSL        R0, R0, #23
        ORRS       R0, R0, R1
    __me_lab_end:
...
void _FloatToLongDouble()
{
    ...
    __me_unfl:
        MOVS    R1, #0x0                // IF UNDERFLOW, RETURN 0
        MOVS    R0, #0x0
        B       __me_lab_end

    __me_ovfl:
        MOV     R0, #0                  // IF OVERFLOW, RETURN +/- INFINITY
...
void _Add_FP()
{
    ...
    __me_lab1:
        ORR     R4, R4, #0x80000000          // SET IMPLIED ONE IN MANTISSA
    ...
    __me_loop:
        SUB        R3, R3, #1                // ADJUSTING THE EXPONENT AS NECESSARY
...
void _Add_DP()
{
    ...
    __me_lab1:
        ORRS        R7, R7, #0x20000000       // SET IMPLIED ONE IN MANTISSA
    ...
    __me_lab3:
        LSLS       R4, R1, #12                // BUILD INPUT #1 MANTISSA
        LSRS       R4, R4, #3
        ORR        R4, R4, R0, LSR #23
        LSL        R5, R0, #9
    ...
    __me_lab4:
        ORR        R4, R4, #0x20000000        // SET IMPLIED ONE IN MANTISSA
    ...
    __me_no_sft:
        ADDS       R5, R5, R8                 // ADD IT TO THE FIRST MANTISSA
        ADCS       R4, R4, R7
    ...
    __me_ovfl:
        ADDS       R6, R6, #2                 // NORMALIZE THE EXPONENT
        ITTT       LE
    ...
    __me_lab9:
        LSLS       R7, R4, #20                // REPACK THE MANTISSA INTO
...
void _Div_DP()
{
    ...
    __me_fdivb:
        CMP        R3, R5               // IF DIVIDEND IS LARGER THAN DIVISOR,
        BNE        __me_lab1
        CMP        R2, R6
    __me_lab1:
        BCC        __me_lab2
        ...
        B        __me_lab3
    __me_lab2:
        ADCS        R0, R0, R0          // ELSE, SHIFT A 0 INTO THE QUOTIENT
        ADCS        R1, R1, R1
    __me_lab3:
