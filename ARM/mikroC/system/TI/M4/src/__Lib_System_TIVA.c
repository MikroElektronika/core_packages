...
static void InitialSetUpRCCRCC2(){
 ...
 volatile unsigned long Fosc_kHz;
  ...
  SYSCTL_MOSCCTL  = ulMOSCCTL & ~(1ul << SYSCTL_MOSCCTL_CVAL); // ne ukljuchuj clock validator dok ne prodje vreme za stabilizaciju main osc
  ...
  if (ulMOSCCTL.SYSCTL_MOSCCTL_CVAL) // kad se main clock stabilizovao ukljuchi validaciju ako treba
  ...
  if (ulRSCLKCFG & (1ul << SYSCTL_RSCLKCFG_USEPLL)) {    /* if PLL1 enabled*/
