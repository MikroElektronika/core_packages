...
{
    unsigned long volatile ulPCR_PROC_CLK_CNTRL, ulVBAT_CLOCK_EN;
    unsigned short cnt;
    ulPCR_PROC_CLK_CNTRL = 12345678;
    ulVBAT_CLOCK_EN = 12345679;

    // Suggested by Microchip.
    // Bit[1] = 1 Disable Write Buffer.
    NVIC_ACTLR |= 0x02;

    if ( ( ulPCR_PROC_CLK_CNTRL == 1 ) || ( ulPCR_PROC_CLK_CNTRL == 4 ) ||
         ( ulPCR_PROC_CLK_CNTRL == 16 ) || ( ulPCR_PROC_CLK_CNTRL == 48 ) ) {
        PCR_PROC_CLK_CNTRL = ulPCR_PROC_CLK_CNTRL;
    } else { // 0 must not be written to PCR_PROC_CLK_CNTRL.
        PCR_PROC_CLK_CNTRL = 1;
    }

    if ( ulVBAT_CLOCK_EN & 0x2 ) { // If we are using the 32K clock, wait for stabilization.
        // The XOSEL bit should be correctly configured by firmware before the 32K_EN bit is assserted.
        VBAT_CLOCK_EN = ulVBAT_CLOCK_EN;
        Delay_100ms();
        while ( DET32K_IN_bit == 0 )
            ; // Was the 32K detected after the reset?
        while ( ( PCR_CHIP_OSC_ID & _OSC_LOCK_MASK ) == 0 )
            ; // Is the OSC locked?
    }
}
