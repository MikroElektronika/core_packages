...
static void InitialSetUpRCCRCC2(){
  ...
  if((RCM->SRS0 & RCM_SRS0_WAKEUP_MASK) != 0x00)
  {
    ...
    SIM_SCGC6 |= SIM_SCGC6_RTC_MASK;
        if ((RTC_CR & RTC_CR_OSCE_MASK) == 0x00)
  ...
  // K64 code
  if ((ulMCG_MODE & 0x10) == 0) {
    // za K64 nemamo HSRUN, set default RUN mode
    SMC_PMCTRL &= ~SMC_PMCTRL_RUNM_MASK; // clear [5:6] bits, set RUN mode as default
    // wait for run mode
    while(SMC_PMSTAT != 0x01);
  } else if((ulMCG_MODE & 0x10)) {
    // provera freq. ako je freq>80MHz idemo u HSRUN, u suprotnom RUN mode
    if (Fosc_kHz > 80000) {
      // kod K22 imamo HSRUN
  ...
  SIM_SOPT2   = ((SIM_SOPT2 & (uint32_t)(~(SIM_SOPT2_PLLFLLSEL_MASK | SIM_SOPT2_USBSRC_MASK))) | (ulSIM_SOPT2 & (SIM_SOPT2_PLLFLLSEL_MASK | SIM_SOPT2_USBSRC_MASK)));
