...
#define SYSTEM_SIM_CLKDIV1_VALUE     0x01040000ul         // SIM_CLKDIV1 //
// SIM_SOPT1: OSC32KSEL=2,RAMSIZE=0 //
#define SYSTEM_SIM_SOPT1_VALUE       0x000C0000ul         // SIM_SOPT1 //
// SIM_SOPT2: TRACECLKSEL=0,CLKOUTSEL=0//
...
#define ICIALLU 0xE000EF50
 ...
 {
     uint8_t clockDiv1, clockDiv2, clockDiv4;
     ...
     //clockDiv3 = (SIM_CLKDIV1 & SIM_CLKDIV1_OUTDIV3_MASK) >> SIM_CLKDIV1_OUTDIV3_SHIFT;  //MK4x nema ovo
     ...
     //SIM_Clocks->FLEXBUS_Frequency  = mcgOutClockFrequency / (clockDiv3 + 1);  //MK4x nema ovo
...
static void InitialSetUpRCCRCC2(){
    volatile uint32_t ulOSC_CR, ulOSC_OSC_CR, ulSIM_SOPT1, ulSIM_SOPT2, ulSIM_CLKDIV1,
   ...
   if((RCM_SRS0 & RCM_SRS0_WAKEUP_MASK) != 0x00)
   {
       if((PMC_REGSC & PMC_REGSC_ACKISO_MASK) != 0x00)
       {
    ...
    if((ulMCG_MODE & 0x10)) 
    {
        // provera freq. ako je freq > 100MHz idemo u HSRUN, u suprotnom RUN mode
        if (Fosc_kHz > 100000) 
        {
    ...
    {   // MCG_MODE
        // Set MCG and OSC
        if ((ulOSC_CR & OSC_CR_ERCLKEN_MASK) != 0x00) 
        {
            // SIM_SCGC5: PORTA=1
            SIM_SCGC5 |= SIM_SCGC5_PORTA_MASK;
            // PORTA_PCR18: ISF=0,MUX=0
