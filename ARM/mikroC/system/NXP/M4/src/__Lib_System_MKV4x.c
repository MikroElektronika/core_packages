...
typedef struct
{
    ...
    __IO uint32_t SOPT1CFG;         /*!< SOPT1 Configuration Register, offset:                   */
    uint8_t       RESERVED_0[ 4092 ];
    __IO uint32_t SOPT2;            /*!< System Op  tions Register 2, offset: 0x1004             */
...
#define SYSTEM_SIM_CLKDIV1_VALUE 0x01040000ul
/* SIM_SOPT1: OSC32KSEL=2,RAMSIZE=0 */
#define SYSTEM_SIM_SOPT1_VALUE   0x000C0000ul
...
static void InitialSetUpRCCRCC2()
{
    ...
    // Checking for HSRUN support mode
    if ( ( ulMCG_MODE & 0x10 ) ) {
        // Checking freq. if freq > 100MHz then HSRUN, else RUN mode
        if ( Fosc_kHz > 100000 ) {
