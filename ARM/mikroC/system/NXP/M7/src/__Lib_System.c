...
typedef struct
{
    __IOM uint32_t ISER[8ul];               /*!< Offset: 0x000 (R/W)  Interrupt Set Enable Register */
          uint32_t RESERVED0[24ul];
    __IOM uint32_t ICER[8ul];               /*!< Offset: 0x080 (R/W)  Interrupt Clear Enable Register */
          uint32_t RSERVED1[24ul];
    __IOM uint32_t ISPR[8ul];               /*!< Offset: 0x100 (R/W)  Interrupt Set Pending Register */
          uint32_t RESERVED2[24ul];
    __IOM uint32_t ICPR[8ul];               /*!< Offset: 0x180 (R/W)  Interrupt Clear Pending Register */
          uint32_t RESERVED3[24ul];
    __IOM uint32_t IABR[8ul];               /*!< Offset: 0x200 (R/W)  Interrupt Active bit Register */
          uint32_t RESERVED4[56ul];
    __IOM uint8_t  IP[240ul];               /*!< Offset: 0x300 (R/W)  Interrupt Priority Register (8Bit wide) */
          uint32_t RESERVED5[644ul];
...
#define CCSIDR  0xE000ED80          // Cache size ID register address.
#define CSSELR  0xE000ED84          // Cache size selection register address.
#define DCISW   0xE000EF60          // Cache maintenance op address: data cache clean and invalidate by set/way.
#define ICIALLU 0xE000EF50

static void SCB_EnableCache()
{
    ...
    asm {
        LDR R11, =0xE000ED14
        LDR R0, [R11]
        ORR R0, R0, #131072   ; Set CCR.IC field
        STR R0, [R11]
    ...
    asm {
        MOV R0, #0x0                    // Select level 1 data cache.
        LDR R11, =CSSELR
        STR R0, [R11]
        DSB                             // Ensure write to CSSELR before proceeding
        LDR R11, =CCSIDR                // from CCSIDR.
        LDR R2, [R11]                   // Read data cache size information.
        AND R1, R2, #0x7                // Cache line size.
        ADD R7, R1, #0x4                // Number of words in a cache line.
        UBFX R4, R2, #3, #10            // Number of ways-1 of data cache.
        UBFX R2, R2, #13, #15           // Number of set-1 of data cache.
        CLZ R6, R4                      // Calculate bit offset for way in DCISW.
        LDR R11, =DCISW                 // Invalidate cache by set/way.

    inv_loop1:                          // For each set
        MOV R1, R4                      // Number of ways-1.
    ...
    asm {
        LDR R11, =0xE000ED14
        LDR R0, [R11]
        ORR R0, R0, #65536    ; Set CCR.DC field
        STR R0, [R11]
...
void SIM_GetClocksFrequency(SIM_ClocksTypeDef* SIM_Clocks)
{
    ...
    SIM_Clocks->FAST_PERIPHERAL_Frequency = mcgOutClockFrequency / ( clockDiv2 + 1 );
...
{
    volatile uint32_t ulOSC0_CR, ulOSC0_OSC_CR, ulSIM_SOPT1, ulSIM_SOPT2, ulSIM_CLKDIV1,
             ulMCG_C1, ulMCG_C2, ulMCG_C4, ulMCG_C5, ulMCG_C6, ulMCG_SC, ulWDOG_STCTRLH,
             ulMCG_MODE;
    ...
    ulOSC0_CR       = 12345678;
    ulOSC0_OSC_CR   = 12345679;
    ulSIM_SOPT1     = 12345680;
    ulSIM_SOPT2     = 12345681;
    ulSIM_CLKDIV1   = 12345682;
    ulMCG_C1        = 12345683;
    ulMCG_C2        = 12345684;
    ulMCG_C4        = 12345685;
    ulMCG_C5        = 12345686;
    ...
    SMC_PMPROT = SYSTEM_SMC_PMPROT_VALUE; // Enable all modes.

    /* Check if there is support for HSRUN mode */
    if( ( ulMCG_MODE & 0x10 ) ) {
        /* Frequency check. If the frequency is > 160MHz, go to HSRUN, otherwise RUN mode */
        if ( Fosc_kHz > 160000 ) {
    ...
    if ( ( mcg_mode == MCG_MODE_FEI ) || ( mcg_mode == MCG_MODE_FBI ) || ( mcg_mode == MCG_MODE_BLPI ) ) {
        /* Set MCG and OSC */
        if ( ( ( ulOSC0_CR & OSC_CR_ERCLKEN_MASK ) != 0x00 ) || ( ( ( ulMCG_C5 & MCG_C5_PLLCLKEN0_MASK ) != 0x00 ) ) ) {
        ...
        /* Set fast clock internal reference divider */
        MCG_SC = ulMCG_SC;

        /* Set clock source selection, FLL ext. reference divider, int. reference enable etc. */
        MCG_C1 = ulMCG_C1;
        ...
        /* Set freq. range, ext. and int. reference selection etc. excluding trim bits; low power bit is set later */
        MCG_C2 = ( MCG_C2 & ( uint8_t )( ~MCG_C2_FCFTRIM_MASK ) ) | ( ulMCG_C2 & ( uint8_t )( ~MCG_C2_LP_MASK ) );

        /* Set FLL output; trim values not changed */
        MCG_C4 = ( ulMCG_C4 & ( uint8_t )( ~( MCG_C4_FCTRIM_MASK | MCG_C4_SCFTRIM_MASK ) ) ) | ( MCG_C4 & ( MCG_C4_FCTRIM_MASK | MCG_C4_SCFTRIM_MASK ) );

        /* Set OSCERCLK enable, oscillator capacitor load */
        OSC0_CR = ulOSC0_CR;
        OSC0_OSC_DIV = ulOSC0_OSC_CR;
        ...
        /* Set MCG and OSC */
        if ( ( ulOSC0_CR & OSC_CR_ERCLKEN_MASK ) != 0x00 ) {
            SIM_SCGC5 |= SIM_SCGC5_PORTA_MASK;
            PORTA_PCR18 &= ( uint32_t )~( uint32_t )( ( PORT_PCR_ISF_MASK | PORT_PCR_MUX( 0x07 ) ) );
            if ( ( ulMCG_C2 & MCG_C2_EREFS_MASK ) != 0x00) {
                PORTA_PCR19 &= ( uint32_t )~( uint32_t )( ( PORT_PCR_ISF_MASK | PORT_PCR_MUX( 0x07 ) ) );
        ...
        if (mcg_mode == MCG_MODE_PEE)
            /* Set clock source selection, FLL ext. reference divider, int. reference enable etc. - PBE mode */
            MCG_C1 = ulMCG_C1 | MCG_C1_CLKS(0x02);
        else
            /* Set clock source selection, FLL ext. reference divider, int. reference enable etc. */
            MCG_C1 = ulMCG_C1;
        ...
        if ( ( ulMCG_C2 & MCG_C2_EREFS_MASK ) != 0x00 ) {
            ...
            while ( ( MCG_S & MCG_S_OSCINIT0_MASK ) == 0x00 ) {
            }
        }
        /* Check that the source of the FLL reference clock is the requested one */
        if ( ( ulMCG_C1 & MCG_C1_IREFS_MASK ) != 0x00 ) {
    ...
    /* Set PLL settings, PLL reference divider etc. */
    MCG_C5 = ulMCG_C5 & ( uint8_t )( ~MCG_C5_PLLCLKEN0_MASK );

    /* Set PLL select, VCO divider etc. */
    MCG_C6 = ulMCG_C6 & ( uint8_t )~( MCG_C6_PLLS_MASK );
    if ( ulMCG_C5 & MCG_C5_PLLCLKEN0_MASK ) {
        /* PLL clock enable in mode other than PEE or PBE */
    ...
    if ( mcg_mode == MCG_MODE_BLPE)  {
        ...
        while ( ( MCG_S & MCG_S_LOCK0_MASK ) == 0x00 ) {
        }
        if ( mcg_mode == MCG_MODE_PEE ) {
    ...
    if ( ( mcg_mode == MCG_MODE_FEI ) || ( mcg_mode == MCG_MODE_FEE ) ) {
        ...
        while ( ( MCG_S & MCG_S_CLKST_MASK ) != 0x00 ) {
        }

        /* Use LPTMR to wait for 1ms for FLL clock stabilization */
        /* Alow software control of LPMTR */
        SIM_SCGC5 |= SIM_SCGC5_LPTMR_MASK;

        /* Default 1 LPO tick */
        LPTMR0_CMR = LPTMR_CMR_COMPARE( 0 );
