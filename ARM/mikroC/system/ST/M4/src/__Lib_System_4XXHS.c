...
static void EnableOverdriveMode(){
  ...
  while((PWR_CSR & (1ul << B16)) == 0)  //ODRDY
  ;
  //enable overdrive switching
  PWR_CR |= (1ul << B17);     //ODSWEN
  //wait for overdrive switch ready flag to be set
  while((PWR_CSR & (1ul << B17)) == 0)   //ODSWRDY
  ;
}
...
static void InitialSetUpRCCRCC2(){
 ...
 unsigned long Fosc_kHz;
  ...
  FLASH_ACR.B8 = 1;      // PRFTEN Prefetch enable
  FLASH_ACR.B9 = 1;      // ICEN Instruction cache enable
