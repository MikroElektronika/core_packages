...
static void InitialSetUpRCCRCC2(){
 ...
 unsigned long Fosc_kHz;
  ...
  ulRCC_BDCR      = 12345688;
  ulRCC_CSR       = 12345689;
  ulPWR_D3CR      = 12345690;   // ulVOLTAGE_RANGE = 12345691;
  Fosc_kHz        = 12345677;
  ...
  PWR_D3CR = ulPWR_D3CR; //upis internog VCORE napona


  if (_VOLTAGE_RANGE == VR_1150_1260){
    if (Fosc_kHz > 225000)
      Flash_ACR &= 0x24;  //latency = 4, WRHIGHFREQ = 0b10
    else if (Fosc_kHz > 210000)
      Flash_ACR &= 0x23;  //latency = 3, WRHIGHFREQ = 0b10
    else if (Fosc_kHz > 185000)
      Flash_ACR &= 0x22; // latency = 2, WRHIGHFREQ = 0b10
    else if (Fosc_kHz > 140000)
      Flash_ACR &= 0x12; // latency = 2, WRHIGHFREQ = 0b01
    else if (Fosc_kHz > 70000)
      Flash_ACR &= 0x11; // latency = 1, WRHIGHFREQ = 0b01
    else
      Flash_ACR &= 0;    // latency = 0, WRHIGHFREQ = 0b00
  }
  ...
  RCC_CR = (ulRCC_CR & 0x000FF3BF);               /* do not start PLLs yet */
  RCC_PLLCKSELR = ulRCC_PLLCKSELR;              /* set PLL prescaler register */
  RCC_PLLCFGR  = ulRCC_PLLCFGR;                 /* set clock configuration register */
  RCC_CSR = ulRCC_CSR;                          /* set LSI clock */
  RCC_BDCR = ulRCC_BDCR;                        /* set LSE clock */
  RCC_D1CFGR = ulRCC_D1CFGR;                     /* set clock domain 1 */
  RCC_D2CFGR = ulRCC_D2CFGR;                     /* set clock domain 2 */
  RCC_D3CFGR = ulRCC_D3CFGR;                     /* set clock domain 3 */
  RCC_PLL1DIVR = ulRCC_PLL1DIVR;                /* set PLL1 configuration register */
  RCC_PLL2DIVR = ulRCC_PLL2DIVR;                /* set PLL2 configuration register */
  RCC_PLL3DIVR = ulRCC_PLL3DIVR;                /* set PLL3 configuration register */
  ...
  if (ulRCC_CR & (1ul << PLL1ON)) {                /* if PLL1 enabled*/
    RCC_CR |= (1ul << PLL1ON);                              /* PLL1 On */
    while ((RCC_CR & (1ul << PLL1RDY)) == 0)
      ;         /* Wait for PLL1RDY = 1 (PLL1 is ready)*/
  }

  if (ulRCC_CR & (1ul << PLL2ON)) {                /* if PLL2 enabled*/
    RCC_CR |= (1ul << PLL2ON);                              /* PLL2 On */
    while ((RCC_CR & (1ul << PLL2RDY)) == 0)
      ;         /* Wait for PLL2RDY = 1 (PLL2 is ready)*/
  }

  if (ulRCC_CR & (1ul << PLL3ON)) {                /* if PLL3 enabled*/
    RCC_CR |= (1ul << PLL3ON);                              /* PLL3 On */
