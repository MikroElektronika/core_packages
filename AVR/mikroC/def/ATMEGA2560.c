// Individual bit access constants
const char B0 = 0;
const char B1 = 1;
const char B2 = 2;
const char B3 = 3;
const char B4 = 4;
const char B5 = 5;
const char B6 = 6;
const char B7 = 7;

const unsigned long __FLASH_SIZE = 0x00040000;

// Flash page size in program words
const int __FLASH_PAGE_SIZE = 128;

const unsigned short ICS_AUTO                  =     0;
const unsigned short ICS_OFF                   =     3;

// Addres offset from Output to Input (port) registers
const signed short   OUTPUT_TO_INPUT_OFFSET    =     -2;

// Rx space registers
sfr rx unsigned short R0  absolute 0x00;
sfr rx unsigned short R1  absolute 0x01;
sfr rx unsigned short R2  absolute 0x02;
sfr rx unsigned short R3  absolute 0x03;
sfr rx unsigned short R4  absolute 0x04;
sfr rx unsigned short R5  absolute 0x05;
sfr rx unsigned short R6  absolute 0x06;
sfr rx unsigned short R7  absolute 0x07;
sfr rx unsigned short R8  absolute 0x08;
sfr rx unsigned short R9  absolute 0x09;
sfr rx unsigned short R10 absolute 0x0A;
sfr rx unsigned short R11 absolute 0x0B;
sfr rx unsigned short R12 absolute 0x0C;
sfr rx unsigned short R13 absolute 0x0D;
sfr rx unsigned short R14 absolute 0x0E;
sfr rx unsigned short R15 absolute 0x0F;
sfr rx unsigned short R16 absolute 0x10;
sfr rx unsigned short R17 absolute 0x11;
sfr rx unsigned short R18 absolute 0x12;
sfr rx unsigned short R19 absolute 0x13;
sfr rx unsigned short R20 absolute 0x14;
sfr rx unsigned short R21 absolute 0x15;
sfr rx unsigned short R22 absolute 0x16;
sfr rx unsigned short R23 absolute 0x17;
sfr rx unsigned short R24 absolute 0x18;
sfr rx unsigned short R25 absolute 0x19;
sfr rx unsigned short R26 absolute 0x1A;
sfr rx unsigned short R27 absolute 0x1B;
sfr rx unsigned short R28 absolute 0x1C;
sfr rx unsigned short R29 absolute 0x1D;
sfr rx unsigned short R30 absolute 0x1E;
sfr rx unsigned short R31 absolute 0x1F;

// X, Y and Z registers
sfr rx unsigned short XL absolute 0x1A;
sfr rx unsigned short XH absolute 0x1B;
sfr rx unsigned short YL absolute 0x1C;
sfr rx unsigned short YH absolute 0x1D;
sfr rx unsigned short ZL absolute 0x1E;
sfr rx unsigned short ZH absolute 0x1F;
sfr rx unsigned int   X  absolute 0x1A;
sfr rx unsigned int   Y  absolute 0x1C;
sfr rx unsigned int   Z  absolute 0x1E;

const unsigned int IVT_ADDR_RESET            = 0x0000;
const unsigned int IVT_ADDR_INT0             = 0x0002;
const unsigned int IVT_ADDR_INT1             = 0x0004;
const unsigned int IVT_ADDR_INT2             = 0x0006;
const unsigned int IVT_ADDR_INT3             = 0x0008;
const unsigned int IVT_ADDR_INT4             = 0x000A;
const unsigned int IVT_ADDR_INT5             = 0x000C;
const unsigned int IVT_ADDR_INT6             = 0x000E;
const unsigned int IVT_ADDR_INT7             = 0x0010;
const unsigned int IVT_ADDR_PCINT0           = 0x0012;
const unsigned int IVT_ADDR_PCINT1           = 0x0014;
const unsigned int IVT_ADDR_PCINT2           = 0x0016;
const unsigned int IVT_ADDR_WDT              = 0x0018;
const unsigned int IVT_ADDR_TIMER2_COMPA     = 0x001A;
const unsigned int IVT_ADDR_TIMER2_COMPB     = 0x001C;
const unsigned int IVT_ADDR_TIMER2_OVF       = 0x001E;
const unsigned int IVT_ADDR_TIMER1_CAPT      = 0x0020;
const unsigned int IVT_ADDR_TIMER1_COMPA     = 0x0022;
const unsigned int IVT_ADDR_TIMER1_COMPB     = 0x0024;
const unsigned int IVT_ADDR_TIMER1_COMPC     = 0x0026;
const unsigned int IVT_ADDR_TIMER1_OVF       = 0x0028;
const unsigned int IVT_ADDR_TIMER0_COMPA     = 0x002A;
const unsigned int IVT_ADDR_TIMER0_COMPB     = 0x002C;
const unsigned int IVT_ADDR_TIMER0_OVF       = 0x002E;
const unsigned int IVT_ADDR_SPI__STC         = 0x0030;
const unsigned int IVT_ADDR_USART0__RX       = 0x0032;
const unsigned int IVT_ADDR_USART0__UDRE     = 0x0034;
const unsigned int IVT_ADDR_USART0__TX       = 0x0036;
const unsigned int IVT_ADDR_ANALOG_COMP      = 0x0038;
const unsigned int IVT_ADDR_ADC              = 0x003A;
const unsigned int IVT_ADDR_EE_READY         = 0x003C;
const unsigned int IVT_ADDR_TIMER3_CAPT      = 0x003E;
const unsigned int IVT_ADDR_TIMER3_COMPA     = 0x0040;
const unsigned int IVT_ADDR_TIMER3_COMPB     = 0x0042;
const unsigned int IVT_ADDR_TIMER3_COMPC     = 0x0044;
const unsigned int IVT_ADDR_TIMER3_OVF       = 0x0046;
const unsigned int IVT_ADDR_USART1__RX       = 0x0048;
const unsigned int IVT_ADDR_USART1__UDRE     = 0x004A;
const unsigned int IVT_ADDR_USART1__TX       = 0x004C;
const unsigned int IVT_ADDR_TWI              = 0x004E;
const unsigned int IVT_ADDR_SPM_READY        = 0x0050;
const unsigned int IVT_ADDR_TIMER4_CAPT      = 0x0052;
const unsigned int IVT_ADDR_TIMER4_COMPA     = 0x0054;
const unsigned int IVT_ADDR_TIMER4_COMPB     = 0x0056;
const unsigned int IVT_ADDR_TIMER4_COMPC     = 0x0058;
const unsigned int IVT_ADDR_TIMER4_OVF       = 0x005A;
const unsigned int IVT_ADDR_TIMER5_CAPT      = 0x005C;
const unsigned int IVT_ADDR_TIMER5_COMPA     = 0x005E;
const unsigned int IVT_ADDR_TIMER5_COMPB     = 0x0060;
const unsigned int IVT_ADDR_TIMER5_COMPC     = 0x0062;
const unsigned int IVT_ADDR_TIMER5_OVF       = 0x0064;
const unsigned int IVT_ADDR_USART2__RX       = 0x0066;
const unsigned int IVT_ADDR_USART2__UDRE     = 0x0068;
const unsigned int IVT_ADDR_USART2__TX       = 0x006A;
const unsigned int IVT_ADDR_USART3__RX       = 0x006C;
const unsigned int IVT_ADDR_USART3__UDRE     = 0x006E;
const unsigned int IVT_ADDR_USART3__TX       = 0x0070;


sfr data unsigned short volatile UDR3 absolute 0x136;
    sbit  UDR3_0_bit at UDR3.B0;
    const register unsigned short int UDR3_0 = 0;
    sbit  UDR3_1_bit at UDR3.B1;
    const register unsigned short int UDR3_1 = 1;
    sbit  UDR3_2_bit at UDR3.B2;
    const register unsigned short int UDR3_2 = 2;
    sbit  UDR3_3_bit at UDR3.B3;
    const register unsigned short int UDR3_3 = 3;
    sbit  UDR3_4_bit at UDR3.B4;
    const register unsigned short int UDR3_4 = 4;
    sbit  UDR3_5_bit at UDR3.B5;
    const register unsigned short int UDR3_5 = 5;
    sbit  UDR3_6_bit at UDR3.B6;
    const register unsigned short int UDR3_6 = 6;
    sbit  UDR3_7_bit at UDR3.B7;
    const register unsigned short int UDR3_7 = 7;

sfr data unsigned short UBRR3H absolute 0x135;
    sbit  UBRR8_bit at UBRR3H.B0;
    const register unsigned short int UBRR8 = 0;
    sbit  UBRR9_bit at UBRR3H.B1;
    const register unsigned short int UBRR9 = 1;
    sbit  UBRR10_bit at UBRR3H.B2;
    const register unsigned short int UBRR10 = 2;
    sbit  UBRR11_bit at UBRR3H.B3;
    const register unsigned short int UBRR11 = 3;

sfr data unsigned short UBRR3L absolute 0x134;
    sbit  UBRR0_bit at UBRR3L.B0;
    const register unsigned short int UBRR0 = 0;
    sbit  UBRR1_bit at UBRR3L.B1;
    const register unsigned short int UBRR1 = 1;
    sbit  UBRR2_bit at UBRR3L.B2;
    const register unsigned short int UBRR2 = 2;
    sbit  UBRR3_bit at UBRR3L.B3;
    const register unsigned short int UBRR3 = 3;
    sbit  UBRR4_bit at UBRR3L.B4;
    const register unsigned short int UBRR4 = 4;
    sbit  UBRR5_bit at UBRR3L.B5;
    const register unsigned short int UBRR5 = 5;
    sbit  UBRR6_bit at UBRR3L.B6;
    const register unsigned short int UBRR6 = 6;
    sbit  UBRR7_bit at UBRR3L.B7;
    const register unsigned short int UBRR7 = 7;

sfr data unsigned short volatile UCSR3C absolute 0x132;
    sbit  UCPOL3_bit at UCSR3C.B0;
    const register unsigned short int UCPOL3 = 0;
    sbit  UCSZ30_bit at UCSR3C.B1;
    const register unsigned short int UCSZ30 = 1;
    sbit  UCSZ31_bit at UCSR3C.B2;
    const register unsigned short int UCSZ31 = 2;
    sbit  USBS3_bit at UCSR3C.B3;
    const register unsigned short int USBS3 = 3;
    sbit  UPM30_bit at UCSR3C.B4;
    const register unsigned short int UPM30 = 4;
    sbit  UPM31_bit at UCSR3C.B5;
    const register unsigned short int UPM31 = 5;
    sbit  UMSEL30_bit at UCSR3C.B6;
    const register unsigned short int UMSEL30 = 6;
    sbit  UMSEL31_bit at UCSR3C.B7;
    const register unsigned short int UMSEL31 = 7;

sfr data unsigned short volatile UCSR3B absolute 0x131;
    sbit  TXB83_bit at UCSR3B.B0;
    const register unsigned short int TXB83 = 0;
    sbit  RXB83_bit at UCSR3B.B1;
    const register unsigned short int RXB83 = 1;
    sbit  UCSZ32_bit at UCSR3B.B2;
    const register unsigned short int UCSZ32 = 2;
    sbit  TXEN3_bit at UCSR3B.B3;
    const register unsigned short int TXEN3 = 3;
    sbit  RXEN3_bit at UCSR3B.B4;
    const register unsigned short int RXEN3 = 4;
    sbit  UDRIE3_bit at UCSR3B.B5;
    const register unsigned short int UDRIE3 = 5;
    sbit  TXCIE3_bit at UCSR3B.B6;
    const register unsigned short int TXCIE3 = 6;
    sbit  RXCIE3_bit at UCSR3B.B7;
    const register unsigned short int RXCIE3 = 7;

sfr data unsigned short volatile UCSR3A absolute 0x130;
    sbit  MPCM3_bit at UCSR3A.B0;
    const register unsigned short int MPCM3 = 0;
    sbit  U2X3_bit at UCSR3A.B1;
    const register unsigned short int U2X3 = 1;
    sbit  UPE3_bit at UCSR3A.B2;
    const register unsigned short int UPE3 = 2;
    sbit  DOR3_bit at UCSR3A.B3;
    const register unsigned short int DOR3 = 3;
    sbit  FE3_bit at UCSR3A.B4;
    const register unsigned short int FE3 = 4;
    sbit  UDRE3_bit at UCSR3A.B5;
    const register unsigned short int UDRE3 = 5;
    sbit  TXC3_bit at UCSR3A.B6;
    const register unsigned short int TXC3 = 6;
    sbit  RXC3_bit at UCSR3A.B7;
    const register unsigned short int RXC3 = 7;

sfr data unsigned short OCR5CH absolute 0x12D;
    sbit  OCR5CH0_bit at OCR5CH.B0;
    const register unsigned short int OCR5CH0 = 0;
    sbit  OCR5CH1_bit at OCR5CH.B1;
    const register unsigned short int OCR5CH1 = 1;
    sbit  OCR5CH2_bit at OCR5CH.B2;
    const register unsigned short int OCR5CH2 = 2;
    sbit  OCR5CH3_bit at OCR5CH.B3;
    const register unsigned short int OCR5CH3 = 3;
    sbit  OCR5CH4_bit at OCR5CH.B4;
    const register unsigned short int OCR5CH4 = 4;
    sbit  OCR5CH5_bit at OCR5CH.B5;
    const register unsigned short int OCR5CH5 = 5;
    sbit  OCR5CH6_bit at OCR5CH.B6;
    const register unsigned short int OCR5CH6 = 6;
    sbit  OCR5CH7_bit at OCR5CH.B7;
    const register unsigned short int OCR5CH7 = 7;

sfr data unsigned short OCR5CL absolute 0x12C;
    sbit  OCR5CL0_bit at OCR5CL.B0;
    const register unsigned short int OCR5CL0 = 0;
    sbit  OCR5CL1_bit at OCR5CL.B1;
    const register unsigned short int OCR5CL1 = 1;
    sbit  OCR5CL2_bit at OCR5CL.B2;
    const register unsigned short int OCR5CL2 = 2;
    sbit  OCR5CL3_bit at OCR5CL.B3;
    const register unsigned short int OCR5CL3 = 3;
    sbit  OCR5CL4_bit at OCR5CL.B4;
    const register unsigned short int OCR5CL4 = 4;
    sbit  OCR5CL5_bit at OCR5CL.B5;
    const register unsigned short int OCR5CL5 = 5;
    sbit  OCR5CL6_bit at OCR5CL.B6;
    const register unsigned short int OCR5CL6 = 6;
    sbit  OCR5CL7_bit at OCR5CL.B7;
    const register unsigned short int OCR5CL7 = 7;

sfr data unsigned short OCR5BH absolute 0x12B;
    sbit  OCR5BH0_bit at OCR5BH.B0;
    const register unsigned short int OCR5BH0 = 0;
    sbit  OCR5BH1_bit at OCR5BH.B1;
    const register unsigned short int OCR5BH1 = 1;
    sbit  OCR5BH2_bit at OCR5BH.B2;
    const register unsigned short int OCR5BH2 = 2;
    sbit  OCR5BH3_bit at OCR5BH.B3;
    const register unsigned short int OCR5BH3 = 3;
    sbit  OCR5BH4_bit at OCR5BH.B4;
    const register unsigned short int OCR5BH4 = 4;
    sbit  OCR5BH5_bit at OCR5BH.B5;
    const register unsigned short int OCR5BH5 = 5;
    sbit  OCR5BH6_bit at OCR5BH.B6;
    const register unsigned short int OCR5BH6 = 6;
    sbit  OCR5BH7_bit at OCR5BH.B7;
    const register unsigned short int OCR5BH7 = 7;

sfr data unsigned short OCR5BL absolute 0x12A;
    sbit  OCR5BL0_bit at OCR5BL.B0;
    const register unsigned short int OCR5BL0 = 0;
    sbit  OCR5BL1_bit at OCR5BL.B1;
    const register unsigned short int OCR5BL1 = 1;
    sbit  OCR5BL2_bit at OCR5BL.B2;
    const register unsigned short int OCR5BL2 = 2;
    sbit  OCR5BL3_bit at OCR5BL.B3;
    const register unsigned short int OCR5BL3 = 3;
    sbit  OCR5BL4_bit at OCR5BL.B4;
    const register unsigned short int OCR5BL4 = 4;
    sbit  OCR5BL5_bit at OCR5BL.B5;
    const register unsigned short int OCR5BL5 = 5;
    sbit  OCR5BL6_bit at OCR5BL.B6;
    const register unsigned short int OCR5BL6 = 6;
    sbit  OCR5BL7_bit at OCR5BL.B7;
    const register unsigned short int OCR5BL7 = 7;

sfr data unsigned short OCR5AH absolute 0x129;
    sbit  OCR5AH0_bit at OCR5AH.B0;
    const register unsigned short int OCR5AH0 = 0;
    sbit  OCR5AH1_bit at OCR5AH.B1;
    const register unsigned short int OCR5AH1 = 1;
    sbit  OCR5AH2_bit at OCR5AH.B2;
    const register unsigned short int OCR5AH2 = 2;
    sbit  OCR5AH3_bit at OCR5AH.B3;
    const register unsigned short int OCR5AH3 = 3;
    sbit  OCR5AH4_bit at OCR5AH.B4;
    const register unsigned short int OCR5AH4 = 4;
    sbit  OCR5AH5_bit at OCR5AH.B5;
    const register unsigned short int OCR5AH5 = 5;
    sbit  OCR5AH6_bit at OCR5AH.B6;
    const register unsigned short int OCR5AH6 = 6;
    sbit  OCR5AH7_bit at OCR5AH.B7;
    const register unsigned short int OCR5AH7 = 7;

sfr data unsigned short OCR5AL absolute 0x128;
    sbit  OCR5AL0_bit at OCR5AL.B0;
    const register unsigned short int OCR5AL0 = 0;
    sbit  OCR5AL1_bit at OCR5AL.B1;
    const register unsigned short int OCR5AL1 = 1;
    sbit  OCR5AL2_bit at OCR5AL.B2;
    const register unsigned short int OCR5AL2 = 2;
    sbit  OCR5AL3_bit at OCR5AL.B3;
    const register unsigned short int OCR5AL3 = 3;
    sbit  OCR5AL4_bit at OCR5AL.B4;
    const register unsigned short int OCR5AL4 = 4;
    sbit  OCR5AL5_bit at OCR5AL.B5;
    const register unsigned short int OCR5AL5 = 5;
    sbit  OCR5AL6_bit at OCR5AL.B6;
    const register unsigned short int OCR5AL6 = 6;
    sbit  OCR5AL7_bit at OCR5AL.B7;
    const register unsigned short int OCR5AL7 = 7;

sfr data unsigned short volatile ICR5H absolute 0x127;
    sbit  ICR5H0_bit at ICR5H.B0;
    const register unsigned short int ICR5H0 = 0;
    sbit  ICR5H1_bit at ICR5H.B1;
    const register unsigned short int ICR5H1 = 1;
    sbit  ICR5H2_bit at ICR5H.B2;
    const register unsigned short int ICR5H2 = 2;
    sbit  ICR5H3_bit at ICR5H.B3;
    const register unsigned short int ICR5H3 = 3;
    sbit  ICR5H4_bit at ICR5H.B4;
    const register unsigned short int ICR5H4 = 4;
    sbit  ICR5H5_bit at ICR5H.B5;
    const register unsigned short int ICR5H5 = 5;
    sbit  ICR5H6_bit at ICR5H.B6;
    const register unsigned short int ICR5H6 = 6;
    sbit  ICR5H7_bit at ICR5H.B7;
    const register unsigned short int ICR5H7 = 7;

sfr data unsigned short volatile ICR5L absolute 0x126;
    sbit  ICR5L0_bit at ICR5L.B0;
    const register unsigned short int ICR5L0 = 0;
    sbit  ICR5L1_bit at ICR5L.B1;
    const register unsigned short int ICR5L1 = 1;
    sbit  ICR5L2_bit at ICR5L.B2;
    const register unsigned short int ICR5L2 = 2;
    sbit  ICR5L3_bit at ICR5L.B3;
    const register unsigned short int ICR5L3 = 3;
    sbit  ICR5L4_bit at ICR5L.B4;
    const register unsigned short int ICR5L4 = 4;
    sbit  ICR5L5_bit at ICR5L.B5;
    const register unsigned short int ICR5L5 = 5;
    sbit  ICR5L6_bit at ICR5L.B6;
    const register unsigned short int ICR5L6 = 6;
    sbit  ICR5L7_bit at ICR5L.B7;
    const register unsigned short int ICR5L7 = 7;

sfr data unsigned short volatile TCNT5H absolute 0x125;
    sbit  TCNT5H0_bit at TCNT5H.B0;
    const register unsigned short int TCNT5H0 = 0;
    sbit  TCNT5H1_bit at TCNT5H.B1;
    const register unsigned short int TCNT5H1 = 1;
    sbit  TCNT5H2_bit at TCNT5H.B2;
    const register unsigned short int TCNT5H2 = 2;
    sbit  TCNT5H3_bit at TCNT5H.B3;
    const register unsigned short int TCNT5H3 = 3;
    sbit  TCNT5H4_bit at TCNT5H.B4;
    const register unsigned short int TCNT5H4 = 4;
    sbit  TCNT5H5_bit at TCNT5H.B5;
    const register unsigned short int TCNT5H5 = 5;
    sbit  TCNT5H6_bit at TCNT5H.B6;
    const register unsigned short int TCNT5H6 = 6;
    sbit  TCNT5H7_bit at TCNT5H.B7;
    const register unsigned short int TCNT5H7 = 7;

sfr data unsigned short volatile TCNT5L absolute 0x124;
    sbit  TCNT5L0_bit at TCNT5L.B0;
    const register unsigned short int TCNT5L0 = 0;
    sbit  TCNT5L1_bit at TCNT5L.B1;
    const register unsigned short int TCNT5L1 = 1;
    sbit  TCNT5L2_bit at TCNT5L.B2;
    const register unsigned short int TCNT5L2 = 2;
    sbit  TCNT5L3_bit at TCNT5L.B3;
    const register unsigned short int TCNT5L3 = 3;
    sbit  TCNT5L4_bit at TCNT5L.B4;
    const register unsigned short int TCNT5L4 = 4;
    sbit  TCNT5L5_bit at TCNT5L.B5;
    const register unsigned short int TCNT5L5 = 5;
    sbit  TCNT5L6_bit at TCNT5L.B6;
    const register unsigned short int TCNT5L6 = 6;
    sbit  TCNT5L7_bit at TCNT5L.B7;
    const register unsigned short int TCNT5L7 = 7;

sfr data unsigned short volatile TCCR5C absolute 0x122;
    sbit  FOC5C_bit at TCCR5C.B5;
    const register unsigned short int FOC5C = 5;
    sbit  FOC5B_bit at TCCR5C.B6;
    const register unsigned short int FOC5B = 6;
    sbit  FOC5A_bit at TCCR5C.B7;
    const register unsigned short int FOC5A = 7;

sfr data unsigned short volatile TCCR5B absolute 0x121;
    sbit  CS50_bit at TCCR5B.B0;
    const register unsigned short int CS50 = 0;
    sbit  CS51_bit at TCCR5B.B1;
    const register unsigned short int CS51 = 1;
    sbit  CS52_bit at TCCR5B.B2;
    const register unsigned short int CS52 = 2;
    sbit  WGM52_bit at TCCR5B.B3;
    const register unsigned short int WGM52 = 3;
    sbit  WGM53_bit at TCCR5B.B4;
    const register unsigned short int WGM53 = 4;
    sbit  ICES5_bit at TCCR5B.B6;
    const register unsigned short int ICES5 = 6;
    sbit  ICNC5_bit at TCCR5B.B7;
    const register unsigned short int ICNC5 = 7;

sfr data unsigned short volatile TCCR5A absolute 0x120;
    sbit  WGM50_bit at TCCR5A.B0;
    const register unsigned short int WGM50 = 0;
    sbit  WGM51_bit at TCCR5A.B1;
    const register unsigned short int WGM51 = 1;
    sbit  COM5C0_bit at TCCR5A.B2;
    const register unsigned short int COM5C0 = 2;
    sbit  COM5C1_bit at TCCR5A.B3;
    const register unsigned short int COM5C1 = 3;
    sbit  COM5B0_bit at TCCR5A.B4;
    const register unsigned short int COM5B0 = 4;
    sbit  COM5B1_bit at TCCR5A.B5;
    const register unsigned short int COM5B1 = 5;
    sbit  COM5A0_bit at TCCR5A.B6;
    const register unsigned short int COM5A0 = 6;
    sbit  COM5A1_bit at TCCR5A.B7;
    const register unsigned short int COM5A1 = 7;

sfr data unsigned short PORTL absolute 0x10B;
    sbit  PORTL0_bit at PORTL.B0;
    const register unsigned short int PORTL0 = 0;
    sbit  PORTL1_bit at PORTL.B1;
    const register unsigned short int PORTL1 = 1;
    sbit  PORTL2_bit at PORTL.B2;
    const register unsigned short int PORTL2 = 2;
    sbit  PORTL3_bit at PORTL.B3;
    const register unsigned short int PORTL3 = 3;
    sbit  PORTL4_bit at PORTL.B4;
    const register unsigned short int PORTL4 = 4;
    sbit  PORTL5_bit at PORTL.B5;
    const register unsigned short int PORTL5 = 5;
    sbit  PORTL6_bit at PORTL.B6;
    const register unsigned short int PORTL6 = 6;
    sbit  PORTL7_bit at PORTL.B7;
    const register unsigned short int PORTL7 = 7;

sfr data unsigned short DDRL absolute 0x10A;
    sbit  DDL0_bit at DDRL.B0;
    const register unsigned short int DDL0 = 0;
    sbit  DDL1_bit at DDRL.B1;
    const register unsigned short int DDL1 = 1;
    sbit  DDL2_bit at DDRL.B2;
    const register unsigned short int DDL2 = 2;
    sbit  DDL3_bit at DDRL.B3;
    const register unsigned short int DDL3 = 3;
    sbit  DDL4_bit at DDRL.B4;
    const register unsigned short int DDL4 = 4;
    sbit  DDL5_bit at DDRL.B5;
    const register unsigned short int DDL5 = 5;
    sbit  DDL6_bit at DDRL.B6;
    const register unsigned short int DDL6 = 6;
    sbit  DDL7_bit at DDRL.B7;
    const register unsigned short int DDL7 = 7;

sfr data unsigned short volatile PINL absolute 0x109;
    sbit  PINL0_bit at PINL.B0;
    const register unsigned short int PINL0 = 0;
    sbit  PINL1_bit at PINL.B1;
    const register unsigned short int PINL1 = 1;
    sbit  PINL2_bit at PINL.B2;
    const register unsigned short int PINL2 = 2;
    sbit  PINL3_bit at PINL.B3;
    const register unsigned short int PINL3 = 3;
    sbit  PINL4_bit at PINL.B4;
    const register unsigned short int PINL4 = 4;
    sbit  PINL5_bit at PINL.B5;
    const register unsigned short int PINL5 = 5;
    sbit  PINL6_bit at PINL.B6;
    const register unsigned short int PINL6 = 6;
    sbit  PINL7_bit at PINL.B7;
    const register unsigned short int PINL7 = 7;

sfr data unsigned short PORTK absolute 0x108;
    sbit  PORTK0_bit at PORTK.B0;
    const register unsigned short int PORTK0 = 0;
    sbit  PORTK1_bit at PORTK.B1;
    const register unsigned short int PORTK1 = 1;
    sbit  PORTK2_bit at PORTK.B2;
    const register unsigned short int PORTK2 = 2;
    sbit  PORTK3_bit at PORTK.B3;
    const register unsigned short int PORTK3 = 3;
    sbit  PORTK4_bit at PORTK.B4;
    const register unsigned short int PORTK4 = 4;
    sbit  PORTK5_bit at PORTK.B5;
    const register unsigned short int PORTK5 = 5;
    sbit  PORTK6_bit at PORTK.B6;
    const register unsigned short int PORTK6 = 6;
    sbit  PORTK7_bit at PORTK.B7;
    const register unsigned short int PORTK7 = 7;

sfr data unsigned short DDRK absolute 0x107;
    sbit  DDK0_bit at DDRK.B0;
    const register unsigned short int DDK0 = 0;
    sbit  DDK1_bit at DDRK.B1;
    const register unsigned short int DDK1 = 1;
    sbit  DDK2_bit at DDRK.B2;
    const register unsigned short int DDK2 = 2;
    sbit  DDK3_bit at DDRK.B3;
    const register unsigned short int DDK3 = 3;
    sbit  DDK4_bit at DDRK.B4;
    const register unsigned short int DDK4 = 4;
    sbit  DDK5_bit at DDRK.B5;
    const register unsigned short int DDK5 = 5;
    sbit  DDK6_bit at DDRK.B6;
    const register unsigned short int DDK6 = 6;
    sbit  DDK7_bit at DDRK.B7;
    const register unsigned short int DDK7 = 7;

sfr data unsigned short volatile PINK absolute 0x106;
    sbit  PINK0_bit at PINK.B0;
    const register unsigned short int PINK0 = 0;
    sbit  PINK1_bit at PINK.B1;
    const register unsigned short int PINK1 = 1;
    sbit  PINK2_bit at PINK.B2;
    const register unsigned short int PINK2 = 2;
    sbit  PINK3_bit at PINK.B3;
    const register unsigned short int PINK3 = 3;
    sbit  PINK4_bit at PINK.B4;
    const register unsigned short int PINK4 = 4;
    sbit  PINK5_bit at PINK.B5;
    const register unsigned short int PINK5 = 5;
    sbit  PINK6_bit at PINK.B6;
    const register unsigned short int PINK6 = 6;
    sbit  PINK7_bit at PINK.B7;
    const register unsigned short int PINK7 = 7;

sfr data unsigned short PORTJ absolute 0x105;
    sbit  PORTJ0_bit at PORTJ.B0;
    const register unsigned short int PORTJ0 = 0;
    sbit  PORTJ1_bit at PORTJ.B1;
    const register unsigned short int PORTJ1 = 1;
    sbit  PORTJ2_bit at PORTJ.B2;
    const register unsigned short int PORTJ2 = 2;
    sbit  PORTJ3_bit at PORTJ.B3;
    const register unsigned short int PORTJ3 = 3;
    sbit  PORTJ4_bit at PORTJ.B4;
    const register unsigned short int PORTJ4 = 4;
    sbit  PORTJ5_bit at PORTJ.B5;
    const register unsigned short int PORTJ5 = 5;
    sbit  PORTJ6_bit at PORTJ.B6;
    const register unsigned short int PORTJ6 = 6;
    sbit  PORTJ7_bit at PORTJ.B7;
    const register unsigned short int PORTJ7 = 7;

sfr data unsigned short DDRJ absolute 0x104;
    sbit  DDJ0_bit at DDRJ.B0;
    const register unsigned short int DDJ0 = 0;
    sbit  DDJ1_bit at DDRJ.B1;
    const register unsigned short int DDJ1 = 1;
    sbit  DDJ2_bit at DDRJ.B2;
    const register unsigned short int DDJ2 = 2;
    sbit  DDJ3_bit at DDRJ.B3;
    const register unsigned short int DDJ3 = 3;
    sbit  DDJ4_bit at DDRJ.B4;
    const register unsigned short int DDJ4 = 4;
    sbit  DDJ5_bit at DDRJ.B5;
    const register unsigned short int DDJ5 = 5;
    sbit  DDJ6_bit at DDRJ.B6;
    const register unsigned short int DDJ6 = 6;
    sbit  DDJ7_bit at DDRJ.B7;
    const register unsigned short int DDJ7 = 7;

sfr data unsigned short volatile PINJ absolute 0x103;
    sbit  PINJ0_bit at PINJ.B0;
    const register unsigned short int PINJ0 = 0;
    sbit  PINJ1_bit at PINJ.B1;
    const register unsigned short int PINJ1 = 1;
    sbit  PINJ2_bit at PINJ.B2;
    const register unsigned short int PINJ2 = 2;
    sbit  PINJ3_bit at PINJ.B3;
    const register unsigned short int PINJ3 = 3;
    sbit  PINJ4_bit at PINJ.B4;
    const register unsigned short int PINJ4 = 4;
    sbit  PINJ5_bit at PINJ.B5;
    const register unsigned short int PINJ5 = 5;
    sbit  PINJ6_bit at PINJ.B6;
    const register unsigned short int PINJ6 = 6;
    sbit  PINJ7_bit at PINJ.B7;
    const register unsigned short int PINJ7 = 7;

sfr data unsigned short PORTH absolute 0x102;
    sbit  PORTH0_bit at PORTH.B0;
    const register unsigned short int PORTH0 = 0;
    sbit  PORTH1_bit at PORTH.B1;
    const register unsigned short int PORTH1 = 1;
    sbit  PORTH2_bit at PORTH.B2;
    const register unsigned short int PORTH2 = 2;
    sbit  PORTH3_bit at PORTH.B3;
    const register unsigned short int PORTH3 = 3;
    sbit  PORTH4_bit at PORTH.B4;
    const register unsigned short int PORTH4 = 4;
    sbit  PORTH5_bit at PORTH.B5;
    const register unsigned short int PORTH5 = 5;
    sbit  PORTH6_bit at PORTH.B6;
    const register unsigned short int PORTH6 = 6;
    sbit  PORTH7_bit at PORTH.B7;
    const register unsigned short int PORTH7 = 7;

sfr data unsigned short DDRH absolute 0x101;
    sbit  DDH0_bit at DDRH.B0;
    const register unsigned short int DDH0 = 0;
    sbit  DDH1_bit at DDRH.B1;
    const register unsigned short int DDH1 = 1;
    sbit  DDH2_bit at DDRH.B2;
    const register unsigned short int DDH2 = 2;
    sbit  DDH3_bit at DDRH.B3;
    const register unsigned short int DDH3 = 3;
    sbit  DDH4_bit at DDRH.B4;
    const register unsigned short int DDH4 = 4;
    sbit  DDH5_bit at DDRH.B5;
    const register unsigned short int DDH5 = 5;
    sbit  DDH6_bit at DDRH.B6;
    const register unsigned short int DDH6 = 6;
    sbit  DDH7_bit at DDRH.B7;
    const register unsigned short int DDH7 = 7;

sfr data unsigned short volatile PINH absolute 0x100;
    sbit  PINH0_bit at PINH.B0;
    const register unsigned short int PINH0 = 0;
    sbit  PINH1_bit at PINH.B1;
    const register unsigned short int PINH1 = 1;
    sbit  PINH2_bit at PINH.B2;
    const register unsigned short int PINH2 = 2;
    sbit  PINH3_bit at PINH.B3;
    const register unsigned short int PINH3 = 3;
    sbit  PINH4_bit at PINH.B4;
    const register unsigned short int PINH4 = 4;
    sbit  PINH5_bit at PINH.B5;
    const register unsigned short int PINH5 = 5;
    sbit  PINH6_bit at PINH.B6;
    const register unsigned short int PINH6 = 6;
    sbit  PINH7_bit at PINH.B7;
    const register unsigned short int PINH7 = 7;

sfr data unsigned short volatile UDR2 absolute 0xD6;
    sbit  UDR2_0_bit at UDR2.B0;
    const register unsigned short int UDR2_0 = 0;
    sbit  UDR2_1_bit at UDR2.B1;
    const register unsigned short int UDR2_1 = 1;
    sbit  UDR2_2_bit at UDR2.B2;
    const register unsigned short int UDR2_2 = 2;
    sbit  UDR2_3_bit at UDR2.B3;
    const register unsigned short int UDR2_3 = 3;
    sbit  UDR2_4_bit at UDR2.B4;
    const register unsigned short int UDR2_4 = 4;
    sbit  UDR2_5_bit at UDR2.B5;
    const register unsigned short int UDR2_5 = 5;
    sbit  UDR2_6_bit at UDR2.B6;
    const register unsigned short int UDR2_6 = 6;
    sbit  UDR2_7_bit at UDR2.B7;
    const register unsigned short int UDR2_7 = 7;

sfr data unsigned short UBRR2H absolute 0xD5;
    sbit  UBRR8_UBRR2H_bit at UBRR2H.B0;
    sbit  UBRR9_UBRR2H_bit at UBRR2H.B1;
    sbit  UBRR10_UBRR2H_bit at UBRR2H.B2;
    sbit  UBRR11_UBRR2H_bit at UBRR2H.B3;

sfr data unsigned short UBRR2L absolute 0xD4;
    sbit  UBRR0_UBRR2L_bit at UBRR2L.B0;
    sbit  UBRR1_UBRR2L_bit at UBRR2L.B1;
    sbit  UBRR2_UBRR2L_bit at UBRR2L.B2;
    sbit  UBRR3_UBRR2L_bit at UBRR2L.B3;
    sbit  UBRR4_UBRR2L_bit at UBRR2L.B4;
    sbit  UBRR5_UBRR2L_bit at UBRR2L.B5;
    sbit  UBRR6_UBRR2L_bit at UBRR2L.B6;
    sbit  UBRR7_UBRR2L_bit at UBRR2L.B7;

sfr data unsigned short volatile UCSR2C absolute 0xD2;
    sbit  UCPOL2_bit at UCSR2C.B0;
    const register unsigned short int UCPOL2 = 0;
    sbit  UCSZ20_bit at UCSR2C.B1;
    const register unsigned short int UCSZ20 = 1;
    sbit  UCSZ21_bit at UCSR2C.B2;
    const register unsigned short int UCSZ21 = 2;
    sbit  USBS2_bit at UCSR2C.B3;
    const register unsigned short int USBS2 = 3;
    sbit  UPM20_bit at UCSR2C.B4;
    const register unsigned short int UPM20 = 4;
    sbit  UPM21_bit at UCSR2C.B5;
    const register unsigned short int UPM21 = 5;
    sbit  UMSEL20_bit at UCSR2C.B6;
    const register unsigned short int UMSEL20 = 6;
    sbit  UMSEL21_bit at UCSR2C.B7;
    const register unsigned short int UMSEL21 = 7;

sfr data unsigned short volatile UCSR2B absolute 0xD1;
    sbit  TXB82_bit at UCSR2B.B0;
    const register unsigned short int TXB82 = 0;
    sbit  RXB82_bit at UCSR2B.B1;
    const register unsigned short int RXB82 = 1;
    sbit  UCSZ22_bit at UCSR2B.B2;
    const register unsigned short int UCSZ22 = 2;
    sbit  TXEN2_bit at UCSR2B.B3;
    const register unsigned short int TXEN2 = 3;
    sbit  RXEN2_bit at UCSR2B.B4;
    const register unsigned short int RXEN2 = 4;
    sbit  UDRIE2_bit at UCSR2B.B5;
    const register unsigned short int UDRIE2 = 5;
    sbit  TXCIE2_bit at UCSR2B.B6;
    const register unsigned short int TXCIE2 = 6;
    sbit  RXCIE2_bit at UCSR2B.B7;
    const register unsigned short int RXCIE2 = 7;

sfr data unsigned short volatile UCSR2A absolute 0xD0;
    sbit  MPCM2_bit at UCSR2A.B0;
    const register unsigned short int MPCM2 = 0;
    sbit  U2X2_bit at UCSR2A.B1;
    const register unsigned short int U2X2 = 1;
    sbit  UPE2_bit at UCSR2A.B2;
    const register unsigned short int UPE2 = 2;
    sbit  DOR2_bit at UCSR2A.B3;
    const register unsigned short int DOR2 = 3;
    sbit  FE2_bit at UCSR2A.B4;
    const register unsigned short int FE2 = 4;
    sbit  UDRE2_bit at UCSR2A.B5;
    const register unsigned short int UDRE2 = 5;
    sbit  TXC2_bit at UCSR2A.B6;
    const register unsigned short int TXC2 = 6;
    sbit  RXC2_bit at UCSR2A.B7;
    const register unsigned short int RXC2 = 7;

sfr data unsigned short volatile UDR1 absolute 0xCE;
    sbit  UDR1_0_bit at UDR1.B0;
    const register unsigned short int UDR1_0 = 0;
    sbit  UDR1_1_bit at UDR1.B1;
    const register unsigned short int UDR1_1 = 1;
    sbit  UDR1_2_bit at UDR1.B2;
    const register unsigned short int UDR1_2 = 2;
    sbit  UDR1_3_bit at UDR1.B3;
    const register unsigned short int UDR1_3 = 3;
    sbit  UDR1_4_bit at UDR1.B4;
    const register unsigned short int UDR1_4 = 4;
    sbit  UDR1_5_bit at UDR1.B5;
    const register unsigned short int UDR1_5 = 5;
    sbit  UDR1_6_bit at UDR1.B6;
    const register unsigned short int UDR1_6 = 6;
    sbit  UDR1_7_bit at UDR1.B7;
    const register unsigned short int UDR1_7 = 7;

sfr data unsigned short UBRR1H absolute 0xCD;
    sbit  UBRR_8_bit at UBRR1H.B0;
    const register unsigned short int UBRR_8 = 0;
    sbit  UBRR_9_bit at UBRR1H.B1;
    const register unsigned short int UBRR_9 = 1;
    sbit  UBRR_10_bit at UBRR1H.B2;
    const register unsigned short int UBRR_10 = 2;
    sbit  UBRR_11_bit at UBRR1H.B3;
    const register unsigned short int UBRR_11 = 3;

sfr data unsigned short UBRR1L absolute 0xCC;
    sbit  UBRR_0_bit at UBRR1L.B0;
    const register unsigned short int UBRR_0 = 0;
    sbit  UBRR_1_bit at UBRR1L.B1;
    const register unsigned short int UBRR_1 = 1;
    sbit  UBRR_2_bit at UBRR1L.B2;
    const register unsigned short int UBRR_2 = 2;
    sbit  UBRR_3_bit at UBRR1L.B3;
    const register unsigned short int UBRR_3 = 3;
    sbit  UBRR_4_bit at UBRR1L.B4;
    const register unsigned short int UBRR_4 = 4;
    sbit  UBRR_5_bit at UBRR1L.B5;
    const register unsigned short int UBRR_5 = 5;
    sbit  UBRR_6_bit at UBRR1L.B6;
    const register unsigned short int UBRR_6 = 6;
    sbit  UBRR_7_bit at UBRR1L.B7;
    const register unsigned short int UBRR_7 = 7;

sfr data unsigned short volatile UCSR1C absolute 0xCA;
    sbit  UCPOL1_bit at UCSR1C.B0;
    const register unsigned short int UCPOL1 = 0;
    sbit  UCSZ10_bit at UCSR1C.B1;
    const register unsigned short int UCSZ10 = 1;
    sbit  UCSZ11_bit at UCSR1C.B2;
    const register unsigned short int UCSZ11 = 2;
    sbit  USBS1_bit at UCSR1C.B3;
    const register unsigned short int USBS1 = 3;
    sbit  UPM10_bit at UCSR1C.B4;
    const register unsigned short int UPM10 = 4;
    sbit  UPM11_bit at UCSR1C.B5;
    const register unsigned short int UPM11 = 5;
    sbit  UMSEL10_bit at UCSR1C.B6;
    const register unsigned short int UMSEL10 = 6;
    sbit  UMSEL11_bit at UCSR1C.B7;
    const register unsigned short int UMSEL11 = 7;

sfr data unsigned short volatile UCSR1B absolute 0xC9;
    sbit  TXB81_bit at UCSR1B.B0;
    const register unsigned short int TXB81 = 0;
    sbit  RXB81_bit at UCSR1B.B1;
    const register unsigned short int RXB81 = 1;
    sbit  UCSZ12_bit at UCSR1B.B2;
    const register unsigned short int UCSZ12 = 2;
    sbit  TXEN1_bit at UCSR1B.B3;
    const register unsigned short int TXEN1 = 3;
    sbit  RXEN1_bit at UCSR1B.B4;
    const register unsigned short int RXEN1 = 4;
    sbit  UDRIE1_bit at UCSR1B.B5;
    const register unsigned short int UDRIE1 = 5;
    sbit  TXCIE1_bit at UCSR1B.B6;
    const register unsigned short int TXCIE1 = 6;
    sbit  RXCIE1_bit at UCSR1B.B7;
    const register unsigned short int RXCIE1 = 7;

sfr data unsigned short volatile UCSR1A absolute 0xC8;
    sbit  MPCM1_bit at UCSR1A.B0;
    const register unsigned short int MPCM1 = 0;
    sbit  U2X1_bit at UCSR1A.B1;
    const register unsigned short int U2X1 = 1;
    sbit  UPE1_bit at UCSR1A.B2;
    const register unsigned short int UPE1 = 2;
    sbit  DOR1_bit at UCSR1A.B3;
    const register unsigned short int DOR1 = 3;
    sbit  FE1_bit at UCSR1A.B4;
    const register unsigned short int FE1 = 4;
    sbit  UDRE1_bit at UCSR1A.B5;
    const register unsigned short int UDRE1 = 5;
    sbit  TXC1_bit at UCSR1A.B6;
    const register unsigned short int TXC1 = 6;
    sbit  RXC1_bit at UCSR1A.B7;
    const register unsigned short int RXC1 = 7;

sfr data unsigned short volatile UDR0 absolute 0xC6;
    sbit  UDR0_0_bit at UDR0.B0;
    const register unsigned short int UDR0_0 = 0;
    sbit  UDR0_1_bit at UDR0.B1;
    const register unsigned short int UDR0_1 = 1;
    sbit  UDR0_2_bit at UDR0.B2;
    const register unsigned short int UDR0_2 = 2;
    sbit  UDR0_3_bit at UDR0.B3;
    const register unsigned short int UDR0_3 = 3;
    sbit  UDR0_4_bit at UDR0.B4;
    const register unsigned short int UDR0_4 = 4;
    sbit  UDR0_5_bit at UDR0.B5;
    const register unsigned short int UDR0_5 = 5;
    sbit  UDR0_6_bit at UDR0.B6;
    const register unsigned short int UDR0_6 = 6;
    sbit  UDR0_7_bit at UDR0.B7;
    const register unsigned short int UDR0_7 = 7;

sfr data unsigned short UBRR0H absolute 0xC5;
    sbit  UBRR8_UBRR0H_bit at UBRR0H.B0;
    sbit  UBRR9_UBRR0H_bit at UBRR0H.B1;
    sbit  UBRR10_UBRR0H_bit at UBRR0H.B2;
    sbit  UBRR11_UBRR0H_bit at UBRR0H.B3;

sfr data unsigned short UBRR0L absolute 0xC4;
    sbit  UBRR0_UBRR0L_bit at UBRR0L.B0;
    sbit  UBRR1_UBRR0L_bit at UBRR0L.B1;
    sbit  UBRR2_UBRR0L_bit at UBRR0L.B2;
    sbit  UBRR3_UBRR0L_bit at UBRR0L.B3;
    sbit  UBRR4_UBRR0L_bit at UBRR0L.B4;
    sbit  UBRR5_UBRR0L_bit at UBRR0L.B5;
    sbit  UBRR6_UBRR0L_bit at UBRR0L.B6;
    sbit  UBRR7_UBRR0L_bit at UBRR0L.B7;

sfr data unsigned short volatile UCSR0C absolute 0xC2;
    sbit  UCPOL0_bit at UCSR0C.B0;
    const register unsigned short int UCPOL0 = 0;
    sbit  UCSZ00_bit at UCSR0C.B1;
    const register unsigned short int UCSZ00 = 1;
    sbit  UCSZ01_bit at UCSR0C.B2;
    const register unsigned short int UCSZ01 = 2;
    sbit  USBS0_bit at UCSR0C.B3;
    const register unsigned short int USBS0 = 3;
    sbit  UPM00_bit at UCSR0C.B4;
    const register unsigned short int UPM00 = 4;
    sbit  UPM01_bit at UCSR0C.B5;
    const register unsigned short int UPM01 = 5;
    sbit  UMSEL00_bit at UCSR0C.B6;
    const register unsigned short int UMSEL00 = 6;
    sbit  UMSEL01_bit at UCSR0C.B7;
    const register unsigned short int UMSEL01 = 7;

sfr data unsigned short volatile UCSR0B absolute 0xC1;
    sbit  TXB80_bit at UCSR0B.B0;
    const register unsigned short int TXB80 = 0;
    sbit  RXB80_bit at UCSR0B.B1;
    const register unsigned short int RXB80 = 1;
    sbit  UCSZ02_bit at UCSR0B.B2;
    const register unsigned short int UCSZ02 = 2;
    sbit  TXEN0_bit at UCSR0B.B3;
    const register unsigned short int TXEN0 = 3;
    sbit  RXEN0_bit at UCSR0B.B4;
    const register unsigned short int RXEN0 = 4;
    sbit  UDRIE0_bit at UCSR0B.B5;
    const register unsigned short int UDRIE0 = 5;
    sbit  TXCIE0_bit at UCSR0B.B6;
    const register unsigned short int TXCIE0 = 6;
    sbit  RXCIE0_bit at UCSR0B.B7;
    const register unsigned short int RXCIE0 = 7;

sfr data unsigned short volatile UCSR0A absolute 0xC0;
    sbit  MPCM0_bit at UCSR0A.B0;
    const register unsigned short int MPCM0 = 0;
    sbit  U2X0_bit at UCSR0A.B1;
    const register unsigned short int U2X0 = 1;
    sbit  UPE0_bit at UCSR0A.B2;
    const register unsigned short int UPE0 = 2;
    sbit  DOR0_bit at UCSR0A.B3;
    const register unsigned short int DOR0 = 3;
    sbit  FE0_bit at UCSR0A.B4;
    const register unsigned short int FE0 = 4;
    sbit  UDRE0_bit at UCSR0A.B5;
    const register unsigned short int UDRE0 = 5;
    sbit  TXC0_bit at UCSR0A.B6;
    const register unsigned short int TXC0 = 6;
    sbit  RXC0_bit at UCSR0A.B7;
    const register unsigned short int RXC0 = 7;

sfr data unsigned short TWAMR absolute 0xBD;
    sbit  TWAM0_bit at TWAMR.B1;
    const register unsigned short int TWAM0 = 1;
    sbit  TWAM1_bit at TWAMR.B2;
    const register unsigned short int TWAM1 = 2;
    sbit  TWAM2_bit at TWAMR.B3;
    const register unsigned short int TWAM2 = 3;
    sbit  TWAM3_bit at TWAMR.B4;
    const register unsigned short int TWAM3 = 4;
    sbit  TWAM4_bit at TWAMR.B5;
    const register unsigned short int TWAM4 = 5;
    sbit  TWAM5_bit at TWAMR.B6;
    const register unsigned short int TWAM5 = 6;
    sbit  TWAM6_bit at TWAMR.B7;
    const register unsigned short int TWAM6 = 7;

sfr data unsigned short volatile TWCR absolute 0xBC;
    sbit  TWIE_bit at TWCR.B0;
    const register unsigned short int TWIE = 0;
    sbit  TWEN_bit at TWCR.B2;
    const register unsigned short int TWEN = 2;
    sbit  TWWC_bit at TWCR.B3;
    const register unsigned short int TWWC = 3;
    sbit  TWSTO_bit at TWCR.B4;
    const register unsigned short int TWSTO = 4;
    sbit  TWSTA_bit at TWCR.B5;
    const register unsigned short int TWSTA = 5;
    sbit  TWEA_bit at TWCR.B6;
    const register unsigned short int TWEA = 6;
    sbit  TWINT_bit at TWCR.B7;
    const register unsigned short int TWINT = 7;

sfr data unsigned short volatile TWDR absolute 0xBB;
    sbit  TWD0_bit at TWDR.B0;
    const register unsigned short int TWD0 = 0;
    sbit  TWD1_bit at TWDR.B1;
    const register unsigned short int TWD1 = 1;
    sbit  TWD2_bit at TWDR.B2;
    const register unsigned short int TWD2 = 2;
    sbit  TWD3_bit at TWDR.B3;
    const register unsigned short int TWD3 = 3;
    sbit  TWD4_bit at TWDR.B4;
    const register unsigned short int TWD4 = 4;
    sbit  TWD5_bit at TWDR.B5;
    const register unsigned short int TWD5 = 5;
    sbit  TWD6_bit at TWDR.B6;
    const register unsigned short int TWD6 = 6;
    sbit  TWD7_bit at TWDR.B7;
    const register unsigned short int TWD7 = 7;

sfr data unsigned short TWAR absolute 0xBA;
    sbit  TWGCE_bit at TWAR.B0;
    const register unsigned short int TWGCE = 0;
    sbit  TWA0_bit at TWAR.B1;
    const register unsigned short int TWA0 = 1;
    sbit  TWA1_bit at TWAR.B2;
    const register unsigned short int TWA1 = 2;
    sbit  TWA2_bit at TWAR.B3;
    const register unsigned short int TWA2 = 3;
    sbit  TWA3_bit at TWAR.B4;
    const register unsigned short int TWA3 = 4;
    sbit  TWA4_bit at TWAR.B5;
    const register unsigned short int TWA4 = 5;
    sbit  TWA5_bit at TWAR.B6;
    const register unsigned short int TWA5 = 6;
    sbit  TWA6_bit at TWAR.B7;
    const register unsigned short int TWA6 = 7;

sfr data unsigned short volatile TWSR absolute 0xB9;
    sbit  TWPS0_bit at TWSR.B0;
    const register unsigned short int TWPS0 = 0;
    sbit  TWPS1_bit at TWSR.B1;
    const register unsigned short int TWPS1 = 1;
    sbit  TWS3_bit at TWSR.B3;
    const register unsigned short int TWS3 = 3;
    sbit  TWS4_bit at TWSR.B4;
    const register unsigned short int TWS4 = 4;
    sbit  TWS5_bit at TWSR.B5;
    const register unsigned short int TWS5 = 5;
    sbit  TWS6_bit at TWSR.B6;
    const register unsigned short int TWS6 = 6;
    sbit  TWS7_bit at TWSR.B7;
    const register unsigned short int TWS7 = 7;

sfr data unsigned short TWBR absolute 0xB8;
    sbit  TWBR0_bit at TWBR.B0;
    const register unsigned short int TWBR0 = 0;
    sbit  TWBR1_bit at TWBR.B1;
    const register unsigned short int TWBR1 = 1;
    sbit  TWBR2_bit at TWBR.B2;
    const register unsigned short int TWBR2 = 2;
    sbit  TWBR3_bit at TWBR.B3;
    const register unsigned short int TWBR3 = 3;
    sbit  TWBR4_bit at TWBR.B4;
    const register unsigned short int TWBR4 = 4;
    sbit  TWBR5_bit at TWBR.B5;
    const register unsigned short int TWBR5 = 5;
    sbit  TWBR6_bit at TWBR.B6;
    const register unsigned short int TWBR6 = 6;
    sbit  TWBR7_bit at TWBR.B7;
    const register unsigned short int TWBR7 = 7;

sfr data unsigned short volatile ASSR absolute 0xB6;
    sbit  TCR2BUB_bit at ASSR.B0;
    const register unsigned short int TCR2BUB = 0;
    sbit  TCR2AUB_bit at ASSR.B1;
    const register unsigned short int TCR2AUB = 1;
    sbit  OCR2BUB_bit at ASSR.B2;
    const register unsigned short int OCR2BUB = 2;
    sbit  OCR2AUB_bit at ASSR.B3;
    const register unsigned short int OCR2AUB = 3;
    sbit  TCN2UB_bit at ASSR.B4;
    const register unsigned short int TCN2UB = 4;
    sbit  AS2_bit at ASSR.B5;
    const register unsigned short int AS2 = 5;
    sbit  EXCLK_bit at ASSR.B6;
    const register unsigned short int EXCLK = 6;

sfr data unsigned short OCR2B absolute 0xB4;
    sbit  OCR2B_0_bit at OCR2B.B0;
    const register unsigned short int OCR2B_0 = 0;
    sbit  OCR2B_1_bit at OCR2B.B1;
    const register unsigned short int OCR2B_1 = 1;
    sbit  OCR2B_2_bit at OCR2B.B2;
    const register unsigned short int OCR2B_2 = 2;
    sbit  OCR2B_3_bit at OCR2B.B3;
    const register unsigned short int OCR2B_3 = 3;
    sbit  OCR2B_4_bit at OCR2B.B4;
    const register unsigned short int OCR2B_4 = 4;
    sbit  OCR2B_5_bit at OCR2B.B5;
    const register unsigned short int OCR2B_5 = 5;
    sbit  OCR2B_6_bit at OCR2B.B6;
    const register unsigned short int OCR2B_6 = 6;
    sbit  OCR2B_7_bit at OCR2B.B7;
    const register unsigned short int OCR2B_7 = 7;

sfr data unsigned short OCR2A absolute 0xB3;
    sbit  OCR2A_0_bit at OCR2A.B0;
    const register unsigned short int OCR2A_0 = 0;
    sbit  OCR2A_1_bit at OCR2A.B1;
    const register unsigned short int OCR2A_1 = 1;
    sbit  OCR2A_2_bit at OCR2A.B2;
    const register unsigned short int OCR2A_2 = 2;
    sbit  OCR2A_3_bit at OCR2A.B3;
    const register unsigned short int OCR2A_3 = 3;
    sbit  OCR2A_4_bit at OCR2A.B4;
    const register unsigned short int OCR2A_4 = 4;
    sbit  OCR2A_5_bit at OCR2A.B5;
    const register unsigned short int OCR2A_5 = 5;
    sbit  OCR2A_6_bit at OCR2A.B6;
    const register unsigned short int OCR2A_6 = 6;
    sbit  OCR2A_7_bit at OCR2A.B7;
    const register unsigned short int OCR2A_7 = 7;

sfr data unsigned short volatile TCNT2 absolute 0xB2;
    sbit  TCNT2_0_bit at TCNT2.B0;
    const register unsigned short int TCNT2_0 = 0;
    sbit  TCNT2_1_bit at TCNT2.B1;
    const register unsigned short int TCNT2_1 = 1;
    sbit  TCNT2_2_bit at TCNT2.B2;
    const register unsigned short int TCNT2_2 = 2;
    sbit  TCNT2_3_bit at TCNT2.B3;
    const register unsigned short int TCNT2_3 = 3;
    sbit  TCNT2_4_bit at TCNT2.B4;
    const register unsigned short int TCNT2_4 = 4;
    sbit  TCNT2_5_bit at TCNT2.B5;
    const register unsigned short int TCNT2_5 = 5;
    sbit  TCNT2_6_bit at TCNT2.B6;
    const register unsigned short int TCNT2_6 = 6;
    sbit  TCNT2_7_bit at TCNT2.B7;
    const register unsigned short int TCNT2_7 = 7;

sfr data unsigned short TCCR2B absolute 0xB1;
    sbit  CS20_bit at TCCR2B.B0;
    const register unsigned short int CS20 = 0;
    sbit  CS21_bit at TCCR2B.B1;
    const register unsigned short int CS21 = 1;
    sbit  CS22_bit at TCCR2B.B2;
    const register unsigned short int CS22 = 2;
    sbit  WGM22_bit at TCCR2B.B3;
    const register unsigned short int WGM22 = 3;
    sbit  FOC2B_bit at TCCR2B.B6;
    const register unsigned short int FOC2B = 6;
    sbit  FOC2A_bit at TCCR2B.B7;
    const register unsigned short int FOC2A = 7;

sfr data unsigned short TCCR2A absolute 0xB0;
    sbit  WGM20_bit at TCCR2A.B0;
    const register unsigned short int WGM20 = 0;
    sbit  WGM21_bit at TCCR2A.B1;
    const register unsigned short int WGM21 = 1;
    sbit  COM2B0_bit at TCCR2A.B4;
    const register unsigned short int COM2B0 = 4;
    sbit  COM2B1_bit at TCCR2A.B5;
    const register unsigned short int COM2B1 = 5;
    sbit  COM2A0_bit at TCCR2A.B6;
    const register unsigned short int COM2A0 = 6;
    sbit  COM2A1_bit at TCCR2A.B7;
    const register unsigned short int COM2A1 = 7;

sfr data unsigned short OCR4CH absolute 0xAD;
    sbit  OCR4CH0_bit at OCR4CH.B0;
    const register unsigned short int OCR4CH0 = 0;
    sbit  OCR4CH1_bit at OCR4CH.B1;
    const register unsigned short int OCR4CH1 = 1;
    sbit  OCR4CH2_bit at OCR4CH.B2;
    const register unsigned short int OCR4CH2 = 2;
    sbit  OCR4CH3_bit at OCR4CH.B3;
    const register unsigned short int OCR4CH3 = 3;
    sbit  OCR4CH4_bit at OCR4CH.B4;
    const register unsigned short int OCR4CH4 = 4;
    sbit  OCR4CH5_bit at OCR4CH.B5;
    const register unsigned short int OCR4CH5 = 5;
    sbit  OCR4CH6_bit at OCR4CH.B6;
    const register unsigned short int OCR4CH6 = 6;
    sbit  OCR4CH7_bit at OCR4CH.B7;
    const register unsigned short int OCR4CH7 = 7;

sfr data unsigned short OCR4CL absolute 0xAC;
    sbit  OCR4CL0_bit at OCR4CL.B0;
    const register unsigned short int OCR4CL0 = 0;
    sbit  OCR4CL1_bit at OCR4CL.B1;
    const register unsigned short int OCR4CL1 = 1;
    sbit  OCR4CL2_bit at OCR4CL.B2;
    const register unsigned short int OCR4CL2 = 2;
    sbit  OCR4CL3_bit at OCR4CL.B3;
    const register unsigned short int OCR4CL3 = 3;
    sbit  OCR4CL4_bit at OCR4CL.B4;
    const register unsigned short int OCR4CL4 = 4;
    sbit  OCR4CL5_bit at OCR4CL.B5;
    const register unsigned short int OCR4CL5 = 5;
    sbit  OCR4CL6_bit at OCR4CL.B6;
    const register unsigned short int OCR4CL6 = 6;
    sbit  OCR4CL7_bit at OCR4CL.B7;
    const register unsigned short int OCR4CL7 = 7;

sfr data unsigned short OCR4BH absolute 0xAB;
    sbit  OCR4BH0_bit at OCR4BH.B0;
    const register unsigned short int OCR4BH0 = 0;
    sbit  OCR4BH1_bit at OCR4BH.B1;
    const register unsigned short int OCR4BH1 = 1;
    sbit  OCR4BH2_bit at OCR4BH.B2;
    const register unsigned short int OCR4BH2 = 2;
    sbit  OCR4BH3_bit at OCR4BH.B3;
    const register unsigned short int OCR4BH3 = 3;
    sbit  OCR4BH4_bit at OCR4BH.B4;
    const register unsigned short int OCR4BH4 = 4;
    sbit  OCR4BH5_bit at OCR4BH.B5;
    const register unsigned short int OCR4BH5 = 5;
    sbit  OCR4BH6_bit at OCR4BH.B6;
    const register unsigned short int OCR4BH6 = 6;
    sbit  OCR4BH7_bit at OCR4BH.B7;
    const register unsigned short int OCR4BH7 = 7;

sfr data unsigned short OCR4BL absolute 0xAA;
    sbit  OCR4BL0_bit at OCR4BL.B0;
    const register unsigned short int OCR4BL0 = 0;
    sbit  OCR4BL1_bit at OCR4BL.B1;
    const register unsigned short int OCR4BL1 = 1;
    sbit  OCR4BL2_bit at OCR4BL.B2;
    const register unsigned short int OCR4BL2 = 2;
    sbit  OCR4BL3_bit at OCR4BL.B3;
    const register unsigned short int OCR4BL3 = 3;
    sbit  OCR4BL4_bit at OCR4BL.B4;
    const register unsigned short int OCR4BL4 = 4;
    sbit  OCR4BL5_bit at OCR4BL.B5;
    const register unsigned short int OCR4BL5 = 5;
    sbit  OCR4BL6_bit at OCR4BL.B6;
    const register unsigned short int OCR4BL6 = 6;
    sbit  OCR4BL7_bit at OCR4BL.B7;
    const register unsigned short int OCR4BL7 = 7;

sfr data unsigned short OCR4AH absolute 0xA9;
    sbit  OCR4AH0_bit at OCR4AH.B0;
    const register unsigned short int OCR4AH0 = 0;
    sbit  OCR4AH1_bit at OCR4AH.B1;
    const register unsigned short int OCR4AH1 = 1;
    sbit  OCR4AH2_bit at OCR4AH.B2;
    const register unsigned short int OCR4AH2 = 2;
    sbit  OCR4AH3_bit at OCR4AH.B3;
    const register unsigned short int OCR4AH3 = 3;
    sbit  OCR4AH4_bit at OCR4AH.B4;
    const register unsigned short int OCR4AH4 = 4;
    sbit  OCR4AH5_bit at OCR4AH.B5;
    const register unsigned short int OCR4AH5 = 5;
    sbit  OCR4AH6_bit at OCR4AH.B6;
    const register unsigned short int OCR4AH6 = 6;
    sbit  OCR4AH7_bit at OCR4AH.B7;
    const register unsigned short int OCR4AH7 = 7;

sfr data unsigned short OCR4AL absolute 0xA8;
    sbit  OCR4AL0_bit at OCR4AL.B0;
    const register unsigned short int OCR4AL0 = 0;
    sbit  OCR4AL1_bit at OCR4AL.B1;
    const register unsigned short int OCR4AL1 = 1;
    sbit  OCR4AL2_bit at OCR4AL.B2;
    const register unsigned short int OCR4AL2 = 2;
    sbit  OCR4AL3_bit at OCR4AL.B3;
    const register unsigned short int OCR4AL3 = 3;
    sbit  OCR4AL4_bit at OCR4AL.B4;
    const register unsigned short int OCR4AL4 = 4;
    sbit  OCR4AL5_bit at OCR4AL.B5;
    const register unsigned short int OCR4AL5 = 5;
    sbit  OCR4AL6_bit at OCR4AL.B6;
    const register unsigned short int OCR4AL6 = 6;
    sbit  OCR4AL7_bit at OCR4AL.B7;
    const register unsigned short int OCR4AL7 = 7;

sfr data unsigned short volatile ICR4H absolute 0xA7;
    sbit  ICR4H0_bit at ICR4H.B0;
    const register unsigned short int ICR4H0 = 0;
    sbit  ICR4H1_bit at ICR4H.B1;
    const register unsigned short int ICR4H1 = 1;
    sbit  ICR4H2_bit at ICR4H.B2;
    const register unsigned short int ICR4H2 = 2;
    sbit  ICR4H3_bit at ICR4H.B3;
    const register unsigned short int ICR4H3 = 3;
    sbit  ICR4H4_bit at ICR4H.B4;
    const register unsigned short int ICR4H4 = 4;
    sbit  ICR4H5_bit at ICR4H.B5;
    const register unsigned short int ICR4H5 = 5;
    sbit  ICR4H6_bit at ICR4H.B6;
    const register unsigned short int ICR4H6 = 6;
    sbit  ICR4H7_bit at ICR4H.B7;
    const register unsigned short int ICR4H7 = 7;

sfr data unsigned short volatile ICR4L absolute 0xA6;
    sbit  ICR4L0_bit at ICR4L.B0;
    const register unsigned short int ICR4L0 = 0;
    sbit  ICR4L1_bit at ICR4L.B1;
    const register unsigned short int ICR4L1 = 1;
    sbit  ICR4L2_bit at ICR4L.B2;
    const register unsigned short int ICR4L2 = 2;
    sbit  ICR4L3_bit at ICR4L.B3;
    const register unsigned short int ICR4L3 = 3;
    sbit  ICR4L4_bit at ICR4L.B4;
    const register unsigned short int ICR4L4 = 4;
    sbit  ICR4L5_bit at ICR4L.B5;
    const register unsigned short int ICR4L5 = 5;
    sbit  ICR4L6_bit at ICR4L.B6;
    const register unsigned short int ICR4L6 = 6;
    sbit  ICR4L7_bit at ICR4L.B7;
    const register unsigned short int ICR4L7 = 7;

sfr data unsigned short volatile TCNT4H absolute 0xA5;
    sbit  TCNT4H0_bit at TCNT4H.B0;
    const register unsigned short int TCNT4H0 = 0;
    sbit  TCNT4H1_bit at TCNT4H.B1;
    const register unsigned short int TCNT4H1 = 1;
    sbit  TCNT4H2_bit at TCNT4H.B2;
    const register unsigned short int TCNT4H2 = 2;
    sbit  TCNT4H3_bit at TCNT4H.B3;
    const register unsigned short int TCNT4H3 = 3;
    sbit  TCNT4H4_bit at TCNT4H.B4;
    const register unsigned short int TCNT4H4 = 4;
    sbit  TCNT4H5_bit at TCNT4H.B5;
    const register unsigned short int TCNT4H5 = 5;
    sbit  TCNT4H6_bit at TCNT4H.B6;
    const register unsigned short int TCNT4H6 = 6;
    sbit  TCNT4H7_bit at TCNT4H.B7;
    const register unsigned short int TCNT4H7 = 7;

sfr data unsigned short volatile TCNT4L absolute 0xA4;
    sbit  TCNT4L0_bit at TCNT4L.B0;
    const register unsigned short int TCNT4L0 = 0;
    sbit  TCNT4L1_bit at TCNT4L.B1;
    const register unsigned short int TCNT4L1 = 1;
    sbit  TCNT4L2_bit at TCNT4L.B2;
    const register unsigned short int TCNT4L2 = 2;
    sbit  TCNT4L3_bit at TCNT4L.B3;
    const register unsigned short int TCNT4L3 = 3;
    sbit  TCNT4L4_bit at TCNT4L.B4;
    const register unsigned short int TCNT4L4 = 4;
    sbit  TCNT4L5_bit at TCNT4L.B5;
    const register unsigned short int TCNT4L5 = 5;
    sbit  TCNT4L6_bit at TCNT4L.B6;
    const register unsigned short int TCNT4L6 = 6;
    sbit  TCNT4L7_bit at TCNT4L.B7;
    const register unsigned short int TCNT4L7 = 7;

sfr data unsigned short volatile TCCR4C absolute 0xA2;
    sbit  FOC4C_bit at TCCR4C.B5;
    const register unsigned short int FOC4C = 5;
    sbit  FOC4B_bit at TCCR4C.B6;
    const register unsigned short int FOC4B = 6;
    sbit  FOC4A_bit at TCCR4C.B7;
    const register unsigned short int FOC4A = 7;

sfr data unsigned short volatile TCCR4B absolute 0xA1;
    sbit  CS40_bit at TCCR4B.B0;
    const register unsigned short int CS40 = 0;
    sbit  CS41_bit at TCCR4B.B1;
    const register unsigned short int CS41 = 1;
    sbit  CS42_bit at TCCR4B.B2;
    const register unsigned short int CS42 = 2;
    sbit  WGM42_bit at TCCR4B.B3;
    const register unsigned short int WGM42 = 3;
    sbit  WGM43_bit at TCCR4B.B4;
    const register unsigned short int WGM43 = 4;
    sbit  ICES4_bit at TCCR4B.B6;
    const register unsigned short int ICES4 = 6;
    sbit  ICNC4_bit at TCCR4B.B7;
    const register unsigned short int ICNC4 = 7;

sfr data unsigned short volatile TCCR4A absolute 0xA0;
    sbit  WGM40_bit at TCCR4A.B0;
    const register unsigned short int WGM40 = 0;
    sbit  WGM41_bit at TCCR4A.B1;
    const register unsigned short int WGM41 = 1;
    sbit  COM4C0_bit at TCCR4A.B2;
    const register unsigned short int COM4C0 = 2;
    sbit  COM4C1_bit at TCCR4A.B3;
    const register unsigned short int COM4C1 = 3;
    sbit  COM4B0_bit at TCCR4A.B4;
    const register unsigned short int COM4B0 = 4;
    sbit  COM4B1_bit at TCCR4A.B5;
    const register unsigned short int COM4B1 = 5;
    sbit  COM4A0_bit at TCCR4A.B6;
    const register unsigned short int COM4A0 = 6;
    sbit  COM4A1_bit at TCCR4A.B7;
    const register unsigned short int COM4A1 = 7;

sfr data unsigned short OCR3CH absolute 0x9D;
    sbit  OCR3CH0_bit at OCR3CH.B0;
    const register unsigned short int OCR3CH0 = 0;
    sbit  OCR3CH1_bit at OCR3CH.B1;
    const register unsigned short int OCR3CH1 = 1;
    sbit  OCR3CH2_bit at OCR3CH.B2;
    const register unsigned short int OCR3CH2 = 2;
    sbit  OCR3CH3_bit at OCR3CH.B3;
    const register unsigned short int OCR3CH3 = 3;
    sbit  OCR3CH4_bit at OCR3CH.B4;
    const register unsigned short int OCR3CH4 = 4;
    sbit  OCR3CH5_bit at OCR3CH.B5;
    const register unsigned short int OCR3CH5 = 5;
    sbit  OCR3CH6_bit at OCR3CH.B6;
    const register unsigned short int OCR3CH6 = 6;
    sbit  OCR3CH7_bit at OCR3CH.B7;
    const register unsigned short int OCR3CH7 = 7;

sfr data unsigned short OCR3CL absolute 0x9C;
    sbit  OCR3CL0_bit at OCR3CL.B0;
    const register unsigned short int OCR3CL0 = 0;
    sbit  OCR3CL1_bit at OCR3CL.B1;
    const register unsigned short int OCR3CL1 = 1;
    sbit  OCR3CL2_bit at OCR3CL.B2;
    const register unsigned short int OCR3CL2 = 2;
    sbit  OCR3CL3_bit at OCR3CL.B3;
    const register unsigned short int OCR3CL3 = 3;
    sbit  OCR3CL4_bit at OCR3CL.B4;
    const register unsigned short int OCR3CL4 = 4;
    sbit  OCR3CL5_bit at OCR3CL.B5;
    const register unsigned short int OCR3CL5 = 5;
    sbit  OCR3CL6_bit at OCR3CL.B6;
    const register unsigned short int OCR3CL6 = 6;
    sbit  OCR3CL7_bit at OCR3CL.B7;
    const register unsigned short int OCR3CL7 = 7;

sfr data unsigned short OCR3BH absolute 0x9B;
    sbit  OCR3BH0_bit at OCR3BH.B0;
    const register unsigned short int OCR3BH0 = 0;
    sbit  OCR3BH1_bit at OCR3BH.B1;
    const register unsigned short int OCR3BH1 = 1;
    sbit  OCR3BH2_bit at OCR3BH.B2;
    const register unsigned short int OCR3BH2 = 2;
    sbit  OCR3BH3_bit at OCR3BH.B3;
    const register unsigned short int OCR3BH3 = 3;
    sbit  OCR3BH4_bit at OCR3BH.B4;
    const register unsigned short int OCR3BH4 = 4;
    sbit  OCR3BH5_bit at OCR3BH.B5;
    const register unsigned short int OCR3BH5 = 5;
    sbit  OCR3BH6_bit at OCR3BH.B6;
    const register unsigned short int OCR3BH6 = 6;
    sbit  OCR3BH7_bit at OCR3BH.B7;
    const register unsigned short int OCR3BH7 = 7;

sfr data unsigned short OCR3BL absolute 0x9A;
    sbit  OCR3BL0_bit at OCR3BL.B0;
    const register unsigned short int OCR3BL0 = 0;
    sbit  OCR3BL1_bit at OCR3BL.B1;
    const register unsigned short int OCR3BL1 = 1;
    sbit  OCR3BL2_bit at OCR3BL.B2;
    const register unsigned short int OCR3BL2 = 2;
    sbit  OCR3BL3_bit at OCR3BL.B3;
    const register unsigned short int OCR3BL3 = 3;
    sbit  OCR3BL4_bit at OCR3BL.B4;
    const register unsigned short int OCR3BL4 = 4;
    sbit  OCR3BL5_bit at OCR3BL.B5;
    const register unsigned short int OCR3BL5 = 5;
    sbit  OCR3BL6_bit at OCR3BL.B6;
    const register unsigned short int OCR3BL6 = 6;
    sbit  OCR3BL7_bit at OCR3BL.B7;
    const register unsigned short int OCR3BL7 = 7;

sfr data unsigned short OCR3AH absolute 0x99;
    sbit  OCR3AH0_bit at OCR3AH.B0;
    const register unsigned short int OCR3AH0 = 0;
    sbit  OCR3AH1_bit at OCR3AH.B1;
    const register unsigned short int OCR3AH1 = 1;
    sbit  OCR3AH2_bit at OCR3AH.B2;
    const register unsigned short int OCR3AH2 = 2;
    sbit  OCR3AH3_bit at OCR3AH.B3;
    const register unsigned short int OCR3AH3 = 3;
    sbit  OCR3AH4_bit at OCR3AH.B4;
    const register unsigned short int OCR3AH4 = 4;
    sbit  OCR3AH5_bit at OCR3AH.B5;
    const register unsigned short int OCR3AH5 = 5;
    sbit  OCR3AH6_bit at OCR3AH.B6;
    const register unsigned short int OCR3AH6 = 6;
    sbit  OCR3AH7_bit at OCR3AH.B7;
    const register unsigned short int OCR3AH7 = 7;

sfr data unsigned short OCR3AL absolute 0x98;
    sbit  OCR3AL0_bit at OCR3AL.B0;
    const register unsigned short int OCR3AL0 = 0;
    sbit  OCR3AL1_bit at OCR3AL.B1;
    const register unsigned short int OCR3AL1 = 1;
    sbit  OCR3AL2_bit at OCR3AL.B2;
    const register unsigned short int OCR3AL2 = 2;
    sbit  OCR3AL3_bit at OCR3AL.B3;
    const register unsigned short int OCR3AL3 = 3;
    sbit  OCR3AL4_bit at OCR3AL.B4;
    const register unsigned short int OCR3AL4 = 4;
    sbit  OCR3AL5_bit at OCR3AL.B5;
    const register unsigned short int OCR3AL5 = 5;
    sbit  OCR3AL6_bit at OCR3AL.B6;
    const register unsigned short int OCR3AL6 = 6;
    sbit  OCR3AL7_bit at OCR3AL.B7;
    const register unsigned short int OCR3AL7 = 7;

sfr data unsigned short volatile ICR3H absolute 0x97;
    sbit  ICR3H0_bit at ICR3H.B0;
    const register unsigned short int ICR3H0 = 0;
    sbit  ICR3H1_bit at ICR3H.B1;
    const register unsigned short int ICR3H1 = 1;
    sbit  ICR3H2_bit at ICR3H.B2;
    const register unsigned short int ICR3H2 = 2;
    sbit  ICR3H3_bit at ICR3H.B3;
    const register unsigned short int ICR3H3 = 3;
    sbit  ICR3H4_bit at ICR3H.B4;
    const register unsigned short int ICR3H4 = 4;
    sbit  ICR3H5_bit at ICR3H.B5;
    const register unsigned short int ICR3H5 = 5;
    sbit  ICR3H6_bit at ICR3H.B6;
    const register unsigned short int ICR3H6 = 6;
    sbit  ICR3H7_bit at ICR3H.B7;
    const register unsigned short int ICR3H7 = 7;

sfr data unsigned short volatile ICR3L absolute 0x96;
    sbit  ICR3L0_bit at ICR3L.B0;
    const register unsigned short int ICR3L0 = 0;
    sbit  ICR3L1_bit at ICR3L.B1;
    const register unsigned short int ICR3L1 = 1;
    sbit  ICR3L2_bit at ICR3L.B2;
    const register unsigned short int ICR3L2 = 2;
    sbit  ICR3L3_bit at ICR3L.B3;
    const register unsigned short int ICR3L3 = 3;
    sbit  ICR3L4_bit at ICR3L.B4;
    const register unsigned short int ICR3L4 = 4;
    sbit  ICR3L5_bit at ICR3L.B5;
    const register unsigned short int ICR3L5 = 5;
    sbit  ICR3L6_bit at ICR3L.B6;
    const register unsigned short int ICR3L6 = 6;
    sbit  ICR3L7_bit at ICR3L.B7;
    const register unsigned short int ICR3L7 = 7;

sfr data unsigned short volatile TCNT3H absolute 0x95;
    sbit  TCNT3H0_bit at TCNT3H.B0;
    const register unsigned short int TCNT3H0 = 0;
    sbit  TCNT3H1_bit at TCNT3H.B1;
    const register unsigned short int TCNT3H1 = 1;
    sbit  TCNT3H2_bit at TCNT3H.B2;
    const register unsigned short int TCNT3H2 = 2;
    sbit  TCNT3H3_bit at TCNT3H.B3;
    const register unsigned short int TCNT3H3 = 3;
    sbit  TCNT3H4_bit at TCNT3H.B4;
    const register unsigned short int TCNT3H4 = 4;
    sbit  TCNT3H5_bit at TCNT3H.B5;
    const register unsigned short int TCNT3H5 = 5;
    sbit  TCNT3H6_bit at TCNT3H.B6;
    const register unsigned short int TCNT3H6 = 6;
    sbit  TCNT3H7_bit at TCNT3H.B7;
    const register unsigned short int TCNT3H7 = 7;

sfr data unsigned short volatile TCNT3L absolute 0x94;
    sbit  TCNT3L0_bit at TCNT3L.B0;
    const register unsigned short int TCNT3L0 = 0;
    sbit  TCNT3L1_bit at TCNT3L.B1;
    const register unsigned short int TCNT3L1 = 1;
    sbit  TCNT3L2_bit at TCNT3L.B2;
    const register unsigned short int TCNT3L2 = 2;
    sbit  TCNT3L3_bit at TCNT3L.B3;
    const register unsigned short int TCNT3L3 = 3;
    sbit  TCNT3L4_bit at TCNT3L.B4;
    const register unsigned short int TCNT3L4 = 4;
    sbit  TCNT3L5_bit at TCNT3L.B5;
    const register unsigned short int TCNT3L5 = 5;
    sbit  TCNT3L6_bit at TCNT3L.B6;
    const register unsigned short int TCNT3L6 = 6;
    sbit  TCNT3L7_bit at TCNT3L.B7;
    const register unsigned short int TCNT3L7 = 7;

sfr data unsigned short volatile TCCR3C absolute 0x92;
    sbit  FOC3C_bit at TCCR3C.B5;
    const register unsigned short int FOC3C = 5;
    sbit  FOC3B_bit at TCCR3C.B6;
    const register unsigned short int FOC3B = 6;
    sbit  FOC3A_bit at TCCR3C.B7;
    const register unsigned short int FOC3A = 7;

sfr data unsigned short volatile TCCR3B absolute 0x91;
    sbit  CS30_bit at TCCR3B.B0;
    const register unsigned short int CS30 = 0;
    sbit  CS31_bit at TCCR3B.B1;
    const register unsigned short int CS31 = 1;
    sbit  CS32_bit at TCCR3B.B2;
    const register unsigned short int CS32 = 2;
    sbit  WGM32_bit at TCCR3B.B3;
    const register unsigned short int WGM32 = 3;
    sbit  WGM33_bit at TCCR3B.B4;
    const register unsigned short int WGM33 = 4;
    sbit  ICES3_bit at TCCR3B.B6;
    const register unsigned short int ICES3 = 6;
    sbit  ICNC3_bit at TCCR3B.B7;
    const register unsigned short int ICNC3 = 7;

sfr data unsigned short volatile TCCR3A absolute 0x90;
    sbit  WGM30_bit at TCCR3A.B0;
    const register unsigned short int WGM30 = 0;
    sbit  WGM31_bit at TCCR3A.B1;
    const register unsigned short int WGM31 = 1;
    sbit  COM3C0_bit at TCCR3A.B2;
    const register unsigned short int COM3C0 = 2;
    sbit  COM3C1_bit at TCCR3A.B3;
    const register unsigned short int COM3C1 = 3;
    sbit  COM3B0_bit at TCCR3A.B4;
    const register unsigned short int COM3B0 = 4;
    sbit  COM3B1_bit at TCCR3A.B5;
    const register unsigned short int COM3B1 = 5;
    sbit  COM3A0_bit at TCCR3A.B6;
    const register unsigned short int COM3A0 = 6;
    sbit  COM3A1_bit at TCCR3A.B7;
    const register unsigned short int COM3A1 = 7;

sfr data unsigned short OCR1CH absolute 0x8D;
    sbit  OCR1CH0_bit at OCR1CH.B0;
    const register unsigned short int OCR1CH0 = 0;
    sbit  OCR1CH1_bit at OCR1CH.B1;
    const register unsigned short int OCR1CH1 = 1;
    sbit  OCR1CH2_bit at OCR1CH.B2;
    const register unsigned short int OCR1CH2 = 2;
    sbit  OCR1CH3_bit at OCR1CH.B3;
    const register unsigned short int OCR1CH3 = 3;
    sbit  OCR1CH4_bit at OCR1CH.B4;
    const register unsigned short int OCR1CH4 = 4;
    sbit  OCR1CH5_bit at OCR1CH.B5;
    const register unsigned short int OCR1CH5 = 5;
    sbit  OCR1CH6_bit at OCR1CH.B6;
    const register unsigned short int OCR1CH6 = 6;
    sbit  OCR1CH7_bit at OCR1CH.B7;
    const register unsigned short int OCR1CH7 = 7;

sfr data unsigned short OCR1CL absolute 0x8C;
    sbit  OCR1CL0_bit at OCR1CL.B0;
    const register unsigned short int OCR1CL0 = 0;
    sbit  OCR1CL1_bit at OCR1CL.B1;
    const register unsigned short int OCR1CL1 = 1;
    sbit  OCR1CL2_bit at OCR1CL.B2;
    const register unsigned short int OCR1CL2 = 2;
    sbit  OCR1CL3_bit at OCR1CL.B3;
    const register unsigned short int OCR1CL3 = 3;
    sbit  OCR1CL4_bit at OCR1CL.B4;
    const register unsigned short int OCR1CL4 = 4;
    sbit  OCR1CL5_bit at OCR1CL.B5;
    const register unsigned short int OCR1CL5 = 5;
    sbit  OCR1CL6_bit at OCR1CL.B6;
    const register unsigned short int OCR1CL6 = 6;
    sbit  OCR1CL7_bit at OCR1CL.B7;
    const register unsigned short int OCR1CL7 = 7;

sfr data unsigned short OCR1BH absolute 0x8B;
    sbit  OCR1BH0_bit at OCR1BH.B0;
    const register unsigned short int OCR1BH0 = 0;
    sbit  OCR1BH1_bit at OCR1BH.B1;
    const register unsigned short int OCR1BH1 = 1;
    sbit  OCR1BH2_bit at OCR1BH.B2;
    const register unsigned short int OCR1BH2 = 2;
    sbit  OCR1BH3_bit at OCR1BH.B3;
    const register unsigned short int OCR1BH3 = 3;
    sbit  OCR1BH4_bit at OCR1BH.B4;
    const register unsigned short int OCR1BH4 = 4;
    sbit  OCR1BH5_bit at OCR1BH.B5;
    const register unsigned short int OCR1BH5 = 5;
    sbit  OCR1BH6_bit at OCR1BH.B6;
    const register unsigned short int OCR1BH6 = 6;
    sbit  OCR1BH7_bit at OCR1BH.B7;
    const register unsigned short int OCR1BH7 = 7;

sfr data unsigned short OCR1BL absolute 0x8A;
    sbit  OCR1BL0_bit at OCR1BL.B0;
    const register unsigned short int OCR1BL0 = 0;
    sbit  OCR1BL1_bit at OCR1BL.B1;
    const register unsigned short int OCR1BL1 = 1;
    sbit  OCR1BL2_bit at OCR1BL.B2;
    const register unsigned short int OCR1BL2 = 2;
    sbit  OCR1BL3_bit at OCR1BL.B3;
    const register unsigned short int OCR1BL3 = 3;
    sbit  OCR1BL4_bit at OCR1BL.B4;
    const register unsigned short int OCR1BL4 = 4;
    sbit  OCR1BL5_bit at OCR1BL.B5;
    const register unsigned short int OCR1BL5 = 5;
    sbit  OCR1BL6_bit at OCR1BL.B6;
    const register unsigned short int OCR1BL6 = 6;
    sbit  OCR1BL7_bit at OCR1BL.B7;
    const register unsigned short int OCR1BL7 = 7;

sfr data unsigned short OCR1AH absolute 0x89;
    sbit  OCR1AH0_bit at OCR1AH.B0;
    const register unsigned short int OCR1AH0 = 0;
    sbit  OCR1AH1_bit at OCR1AH.B1;
    const register unsigned short int OCR1AH1 = 1;
    sbit  OCR1AH2_bit at OCR1AH.B2;
    const register unsigned short int OCR1AH2 = 2;
    sbit  OCR1AH3_bit at OCR1AH.B3;
    const register unsigned short int OCR1AH3 = 3;
    sbit  OCR1AH4_bit at OCR1AH.B4;
    const register unsigned short int OCR1AH4 = 4;
    sbit  OCR1AH5_bit at OCR1AH.B5;
    const register unsigned short int OCR1AH5 = 5;
    sbit  OCR1AH6_bit at OCR1AH.B6;
    const register unsigned short int OCR1AH6 = 6;
    sbit  OCR1AH7_bit at OCR1AH.B7;
    const register unsigned short int OCR1AH7 = 7;

sfr data unsigned short OCR1AL absolute 0x88;
    sbit  OCR1AL0_bit at OCR1AL.B0;
    const register unsigned short int OCR1AL0 = 0;
    sbit  OCR1AL1_bit at OCR1AL.B1;
    const register unsigned short int OCR1AL1 = 1;
    sbit  OCR1AL2_bit at OCR1AL.B2;
    const register unsigned short int OCR1AL2 = 2;
    sbit  OCR1AL3_bit at OCR1AL.B3;
    const register unsigned short int OCR1AL3 = 3;
    sbit  OCR1AL4_bit at OCR1AL.B4;
    const register unsigned short int OCR1AL4 = 4;
    sbit  OCR1AL5_bit at OCR1AL.B5;
    const register unsigned short int OCR1AL5 = 5;
    sbit  OCR1AL6_bit at OCR1AL.B6;
    const register unsigned short int OCR1AL6 = 6;
    sbit  OCR1AL7_bit at OCR1AL.B7;
    const register unsigned short int OCR1AL7 = 7;

sfr data unsigned short volatile ICR1H absolute 0x87;
    sbit  ICR1H0_bit at ICR1H.B0;
    const register unsigned short int ICR1H0 = 0;
    sbit  ICR1H1_bit at ICR1H.B1;
    const register unsigned short int ICR1H1 = 1;
    sbit  ICR1H2_bit at ICR1H.B2;
    const register unsigned short int ICR1H2 = 2;
    sbit  ICR1H3_bit at ICR1H.B3;
    const register unsigned short int ICR1H3 = 3;
    sbit  ICR1H4_bit at ICR1H.B4;
    const register unsigned short int ICR1H4 = 4;
    sbit  ICR1H5_bit at ICR1H.B5;
    const register unsigned short int ICR1H5 = 5;
    sbit  ICR1H6_bit at ICR1H.B6;
    const register unsigned short int ICR1H6 = 6;
    sbit  ICR1H7_bit at ICR1H.B7;
    const register unsigned short int ICR1H7 = 7;

sfr data unsigned short volatile ICR1L absolute 0x86;
    sbit  ICR1L0_bit at ICR1L.B0;
    const register unsigned short int ICR1L0 = 0;
    sbit  ICR1L1_bit at ICR1L.B1;
    const register unsigned short int ICR1L1 = 1;
    sbit  ICR1L2_bit at ICR1L.B2;
    const register unsigned short int ICR1L2 = 2;
    sbit  ICR1L3_bit at ICR1L.B3;
    const register unsigned short int ICR1L3 = 3;
    sbit  ICR1L4_bit at ICR1L.B4;
    const register unsigned short int ICR1L4 = 4;
    sbit  ICR1L5_bit at ICR1L.B5;
    const register unsigned short int ICR1L5 = 5;
    sbit  ICR1L6_bit at ICR1L.B6;
    const register unsigned short int ICR1L6 = 6;
    sbit  ICR1L7_bit at ICR1L.B7;
    const register unsigned short int ICR1L7 = 7;

sfr data unsigned short volatile TCNT1H absolute 0x85;
    sbit  TCNT1H0_bit at TCNT1H.B0;
    const register unsigned short int TCNT1H0 = 0;
    sbit  TCNT1H1_bit at TCNT1H.B1;
    const register unsigned short int TCNT1H1 = 1;
    sbit  TCNT1H2_bit at TCNT1H.B2;
    const register unsigned short int TCNT1H2 = 2;
    sbit  TCNT1H3_bit at TCNT1H.B3;
    const register unsigned short int TCNT1H3 = 3;
    sbit  TCNT1H4_bit at TCNT1H.B4;
    const register unsigned short int TCNT1H4 = 4;
    sbit  TCNT1H5_bit at TCNT1H.B5;
    const register unsigned short int TCNT1H5 = 5;
    sbit  TCNT1H6_bit at TCNT1H.B6;
    const register unsigned short int TCNT1H6 = 6;
    sbit  TCNT1H7_bit at TCNT1H.B7;
    const register unsigned short int TCNT1H7 = 7;

sfr data unsigned short volatile TCNT1L absolute 0x84;
    sbit  TCNT1L0_bit at TCNT1L.B0;
    const register unsigned short int TCNT1L0 = 0;
    sbit  TCNT1L1_bit at TCNT1L.B1;
    const register unsigned short int TCNT1L1 = 1;
    sbit  TCNT1L2_bit at TCNT1L.B2;
    const register unsigned short int TCNT1L2 = 2;
    sbit  TCNT1L3_bit at TCNT1L.B3;
    const register unsigned short int TCNT1L3 = 3;
    sbit  TCNT1L4_bit at TCNT1L.B4;
    const register unsigned short int TCNT1L4 = 4;
    sbit  TCNT1L5_bit at TCNT1L.B5;
    const register unsigned short int TCNT1L5 = 5;
    sbit  TCNT1L6_bit at TCNT1L.B6;
    const register unsigned short int TCNT1L6 = 6;
    sbit  TCNT1L7_bit at TCNT1L.B7;
    const register unsigned short int TCNT1L7 = 7;

sfr data unsigned short volatile TCCR1C absolute 0x82;
    sbit  FOC1C_bit at TCCR1C.B5;
    const register unsigned short int FOC1C = 5;
    sbit  FOC1B_bit at TCCR1C.B6;
    const register unsigned short int FOC1B = 6;
    sbit  FOC1A_bit at TCCR1C.B7;
    const register unsigned short int FOC1A = 7;

sfr data unsigned short TCCR1B absolute 0x81;
    sbit  CS10_bit at TCCR1B.B0;
    const register unsigned short int CS10 = 0;
    sbit  CS11_bit at TCCR1B.B1;
    const register unsigned short int CS11 = 1;
    sbit  CS12_bit at TCCR1B.B2;
    const register unsigned short int CS12 = 2;
    sbit  WGM12_bit at TCCR1B.B3;
    const register unsigned short int WGM12 = 3;
    sbit  WGM13_bit at TCCR1B.B4;
    const register unsigned short int WGM13 = 4;
    sbit  ICES1_bit at TCCR1B.B6;
    const register unsigned short int ICES1 = 6;
    sbit  ICNC1_bit at TCCR1B.B7;
    const register unsigned short int ICNC1 = 7;

sfr data unsigned short TCCR1A absolute 0x80;
    sbit  WGM10_bit at TCCR1A.B0;
    const register unsigned short int WGM10 = 0;
    sbit  WGM11_bit at TCCR1A.B1;
    const register unsigned short int WGM11 = 1;
    sbit  COM1C0_bit at TCCR1A.B2;
    const register unsigned short int COM1C0 = 2;
    sbit  COM1C1_bit at TCCR1A.B3;
    const register unsigned short int COM1C1 = 3;
    sbit  COM1B0_bit at TCCR1A.B4;
    const register unsigned short int COM1B0 = 4;
    sbit  COM1B1_bit at TCCR1A.B5;
    const register unsigned short int COM1B1 = 5;
    sbit  COM1A0_bit at TCCR1A.B6;
    const register unsigned short int COM1A0 = 6;
    sbit  COM1A1_bit at TCCR1A.B7;
    const register unsigned short int COM1A1 = 7;

sfr data unsigned short DIDR1 absolute 0x7F;
    sbit  AIN0D_bit at DIDR1.B0;
    const register unsigned short int AIN0D = 0;
    sbit  AIN1D_bit at DIDR1.B1;
    const register unsigned short int AIN1D = 1;

sfr data unsigned short DIDR0 absolute 0x7E;
    sbit  ADC0D_bit at DIDR0.B0;
    const register unsigned short int ADC0D = 0;
    sbit  ADC1D_bit at DIDR0.B1;
    const register unsigned short int ADC1D = 1;
    sbit  ADC2D_bit at DIDR0.B2;
    const register unsigned short int ADC2D = 2;
    sbit  ADC3D_bit at DIDR0.B3;
    const register unsigned short int ADC3D = 3;
    sbit  ADC4D_bit at DIDR0.B4;
    const register unsigned short int ADC4D = 4;
    sbit  ADC5D_bit at DIDR0.B5;
    const register unsigned short int ADC5D = 5;
    sbit  ADC6D_bit at DIDR0.B6;
    const register unsigned short int ADC6D = 6;
    sbit  ADC7D_bit at DIDR0.B7;
    const register unsigned short int ADC7D = 7;

sfr data unsigned short DIDR2 absolute 0x7D;
    sbit  ADC8D_bit at DIDR2.B0;
    const register unsigned short int ADC8D = 0;
    sbit  ADC9D_bit at DIDR2.B1;
    const register unsigned short int ADC9D = 1;
    sbit  ADC10D_bit at DIDR2.B2;
    const register unsigned short int ADC10D = 2;
    sbit  ADC11D_bit at DIDR2.B3;
    const register unsigned short int ADC11D = 3;
    sbit  ADC12D_bit at DIDR2.B4;
    const register unsigned short int ADC12D = 4;
    sbit  ADC13D_bit at DIDR2.B5;
    const register unsigned short int ADC13D = 5;
    sbit  ADC14D_bit at DIDR2.B6;
    const register unsigned short int ADC14D = 6;
    sbit  ADC15D_bit at DIDR2.B7;
    const register unsigned short int ADC15D = 7;

sfr data unsigned short ADMUX absolute 0x7C;
    sbit  MUX0_bit at ADMUX.B0;
    const register unsigned short int MUX0 = 0;
    sbit  MUX1_bit at ADMUX.B1;
    const register unsigned short int MUX1 = 1;
    sbit  MUX2_bit at ADMUX.B2;
    const register unsigned short int MUX2 = 2;
    sbit  MUX3_bit at ADMUX.B3;
    const register unsigned short int MUX3 = 3;
    sbit  MUX4_bit at ADMUX.B4;
    const register unsigned short int MUX4 = 4;
    sbit  ADLAR_bit at ADMUX.B5;
    const register unsigned short int ADLAR = 5;
    sbit  REFS0_bit at ADMUX.B6;
    const register unsigned short int REFS0 = 6;
    sbit  REFS1_bit at ADMUX.B7;
    const register unsigned short int REFS1 = 7;

sfr data unsigned short ADCSRB absolute 0x7B;
    sbit  ACME_bit at ADCSRB.B6;
    const register unsigned short int ACME = 6;
    sbit  ADTS0_bit at ADCSRB.B0;
    const register unsigned short int ADTS0 = 0;
    sbit  ADTS1_bit at ADCSRB.B1;
    const register unsigned short int ADTS1 = 1;
    sbit  ADTS2_bit at ADCSRB.B2;
    const register unsigned short int ADTS2 = 2;
    sbit  MUX5_bit at ADCSRB.B3;
    const register unsigned short int MUX5 = 3;

sfr data unsigned short volatile ADCSRA absolute 0x7A;
    sbit  ADPS0_bit at ADCSRA.B0;
    const register unsigned short int ADPS0 = 0;
    sbit  ADPS1_bit at ADCSRA.B1;
    const register unsigned short int ADPS1 = 1;
    sbit  ADPS2_bit at ADCSRA.B2;
    const register unsigned short int ADPS2 = 2;
    sbit  ADIE_bit at ADCSRA.B3;
    const register unsigned short int ADIE = 3;
    sbit  ADIF_bit at ADCSRA.B4;
    const register unsigned short int ADIF = 4;
    sbit  ADATE_bit at ADCSRA.B5;
    const register unsigned short int ADATE = 5;
    sbit  ADSC_bit at ADCSRA.B6;
    const register unsigned short int ADSC = 6;
    sbit  ADEN_bit at ADCSRA.B7;
    const register unsigned short int ADEN = 7;

sfr data unsigned short volatile ADCH absolute 0x79;
    sbit  ADCH0_bit at ADCH.B0;
    const register unsigned short int ADCH0 = 0;
    sbit  ADCH1_bit at ADCH.B1;
    const register unsigned short int ADCH1 = 1;
    sbit  ADCH2_bit at ADCH.B2;
    const register unsigned short int ADCH2 = 2;
    sbit  ADCH3_bit at ADCH.B3;
    const register unsigned short int ADCH3 = 3;
    sbit  ADCH4_bit at ADCH.B4;
    const register unsigned short int ADCH4 = 4;
    sbit  ADCH5_bit at ADCH.B5;
    const register unsigned short int ADCH5 = 5;
    sbit  ADCH6_bit at ADCH.B6;
    const register unsigned short int ADCH6 = 6;
    sbit  ADCH7_bit at ADCH.B7;
    const register unsigned short int ADCH7 = 7;

sfr data unsigned short volatile ADCL absolute 0x78;
    sbit  ADCL0_bit at ADCL.B0;
    const register unsigned short int ADCL0 = 0;
    sbit  ADCL1_bit at ADCL.B1;
    const register unsigned short int ADCL1 = 1;
    sbit  ADCL2_bit at ADCL.B2;
    const register unsigned short int ADCL2 = 2;
    sbit  ADCL3_bit at ADCL.B3;
    const register unsigned short int ADCL3 = 3;
    sbit  ADCL4_bit at ADCL.B4;
    const register unsigned short int ADCL4 = 4;
    sbit  ADCL5_bit at ADCL.B5;
    const register unsigned short int ADCL5 = 5;
    sbit  ADCL6_bit at ADCL.B6;
    const register unsigned short int ADCL6 = 6;
    sbit  ADCL7_bit at ADCL.B7;
    const register unsigned short int ADCL7 = 7;

sfr data unsigned short XMCRB absolute 0x75;
    sbit  XMM0_bit at XMCRB.B0;
    const register unsigned short int XMM0 = 0;
    sbit  XMM1_bit at XMCRB.B1;
    const register unsigned short int XMM1 = 1;
    sbit  XMM2_bit at XMCRB.B2;
    const register unsigned short int XMM2 = 2;
    sbit  XMBK_bit at XMCRB.B7;
    const register unsigned short int XMBK = 7;

sfr data unsigned short XMCRA absolute 0x74;
    sbit  SRW00_bit at XMCRA.B0;
    const register unsigned short int SRW00 = 0;
    sbit  SRW01_bit at XMCRA.B1;
    const register unsigned short int SRW01 = 1;
    sbit  SRW10_bit at XMCRA.B2;
    const register unsigned short int SRW10 = 2;
    sbit  SRW11_bit at XMCRA.B3;
    const register unsigned short int SRW11 = 3;
    sbit  SRL0_bit at XMCRA.B4;
    const register unsigned short int SRL0 = 4;
    sbit  SRL1_bit at XMCRA.B5;
    const register unsigned short int SRL1 = 5;
    sbit  SRL2_bit at XMCRA.B6;
    const register unsigned short int SRL2 = 6;
    sbit  SRE_bit at XMCRA.B7;
    const register unsigned short int SRE = 7;

sfr data unsigned short TIMSK5 absolute 0x73;
    sbit  TOIE5_bit at TIMSK5.B0;
    const register unsigned short int TOIE5 = 0;
    sbit  OCIE5A_bit at TIMSK5.B1;
    const register unsigned short int OCIE5A = 1;
    sbit  OCIE5B_bit at TIMSK5.B2;
    const register unsigned short int OCIE5B = 2;
    sbit  OCIE5C_bit at TIMSK5.B3;
    const register unsigned short int OCIE5C = 3;
    sbit  ICIE5_bit at TIMSK5.B5;
    const register unsigned short int ICIE5 = 5;

sfr data unsigned short TIMSK4 absolute 0x72;
    sbit  TOIE4_bit at TIMSK4.B0;
    const register unsigned short int TOIE4 = 0;
    sbit  OCIE4A_bit at TIMSK4.B1;
    const register unsigned short int OCIE4A = 1;
    sbit  OCIE4B_bit at TIMSK4.B2;
    const register unsigned short int OCIE4B = 2;
    sbit  OCIE4C_bit at TIMSK4.B3;
    const register unsigned short int OCIE4C = 3;
    sbit  ICIE4_bit at TIMSK4.B5;
    const register unsigned short int ICIE4 = 5;

sfr data unsigned short TIMSK3 absolute 0x71;
    sbit  TOIE3_bit at TIMSK3.B0;
    const register unsigned short int TOIE3 = 0;
    sbit  OCIE3A_bit at TIMSK3.B1;
    const register unsigned short int OCIE3A = 1;
    sbit  OCIE3B_bit at TIMSK3.B2;
    const register unsigned short int OCIE3B = 2;
    sbit  OCIE3C_bit at TIMSK3.B3;
    const register unsigned short int OCIE3C = 3;
    sbit  ICIE3_bit at TIMSK3.B5;
    const register unsigned short int ICIE3 = 5;

sfr data unsigned short TIMSK2 absolute 0x70;
    sbit  TOIE2_bit at TIMSK2.B0;
    const register unsigned short int TOIE2 = 0;
    sbit  OCIE2A_bit at TIMSK2.B1;
    const register unsigned short int OCIE2A = 1;
    sbit  OCIE2B_bit at TIMSK2.B2;
    const register unsigned short int OCIE2B = 2;

sfr data unsigned short TIMSK1 absolute 0x6F;
    sbit  TOIE1_bit at TIMSK1.B0;
    const register unsigned short int TOIE1 = 0;
    sbit  OCIE1A_bit at TIMSK1.B1;
    const register unsigned short int OCIE1A = 1;
    sbit  OCIE1B_bit at TIMSK1.B2;
    const register unsigned short int OCIE1B = 2;
    sbit  OCIE1C_bit at TIMSK1.B3;
    const register unsigned short int OCIE1C = 3;
    sbit  ICIE1_bit at TIMSK1.B5;
    const register unsigned short int ICIE1 = 5;

sfr data unsigned short TIMSK0 absolute 0x6E;
    sbit  TOIE0_bit at TIMSK0.B0;
    const register unsigned short int TOIE0 = 0;
    sbit  OCIE0A_bit at TIMSK0.B1;
    const register unsigned short int OCIE0A = 1;
    sbit  OCIE0B_bit at TIMSK0.B2;
    const register unsigned short int OCIE0B = 2;

sfr data unsigned short PCMSK2 absolute 0x6D;
    sbit  PCINT16_bit at PCMSK2.B0;
    const register unsigned short int PCINT16 = 0;
    sbit  PCINT17_bit at PCMSK2.B1;
    const register unsigned short int PCINT17 = 1;
    sbit  PCINT18_bit at PCMSK2.B2;
    const register unsigned short int PCINT18 = 2;
    sbit  PCINT19_bit at PCMSK2.B3;
    const register unsigned short int PCINT19 = 3;
    sbit  PCINT20_bit at PCMSK2.B4;
    const register unsigned short int PCINT20 = 4;
    sbit  PCINT21_bit at PCMSK2.B5;
    const register unsigned short int PCINT21 = 5;
    sbit  PCINT22_bit at PCMSK2.B6;
    const register unsigned short int PCINT22 = 6;
    sbit  PCINT23_bit at PCMSK2.B7;
    const register unsigned short int PCINT23 = 7;

sfr data unsigned short PCMSK1 absolute 0x6C;
    sbit  PCINT8_bit at PCMSK1.B0;
    const register unsigned short int PCINT8 = 0;
    sbit  PCINT9_bit at PCMSK1.B1;
    const register unsigned short int PCINT9 = 1;
    sbit  PCINT10_bit at PCMSK1.B2;
    const register unsigned short int PCINT10 = 2;
    sbit  PCINT11_bit at PCMSK1.B3;
    const register unsigned short int PCINT11 = 3;
    sbit  PCINT12_bit at PCMSK1.B4;
    const register unsigned short int PCINT12 = 4;
    sbit  PCINT13_bit at PCMSK1.B5;
    const register unsigned short int PCINT13 = 5;
    sbit  PCINT14_bit at PCMSK1.B6;
    const register unsigned short int PCINT14 = 6;
    sbit  PCINT15_bit at PCMSK1.B7;
    const register unsigned short int PCINT15 = 7;

sfr data unsigned short PCMSK0 absolute 0x6B;
    sbit  PCINT0_bit at PCMSK0.B0;
    const register unsigned short int PCINT0 = 0;
    sbit  PCINT1_bit at PCMSK0.B1;
    const register unsigned short int PCINT1 = 1;
    sbit  PCINT2_bit at PCMSK0.B2;
    const register unsigned short int PCINT2 = 2;
    sbit  PCINT3_bit at PCMSK0.B3;
    const register unsigned short int PCINT3 = 3;
    sbit  PCINT4_bit at PCMSK0.B4;
    const register unsigned short int PCINT4 = 4;
    sbit  PCINT5_bit at PCMSK0.B5;
    const register unsigned short int PCINT5 = 5;
    sbit  PCINT6_bit at PCMSK0.B6;
    const register unsigned short int PCINT6 = 6;
    sbit  PCINT7_bit at PCMSK0.B7;
    const register unsigned short int PCINT7 = 7;

sfr data unsigned short volatile EICRB absolute 0x6A;
    sbit  ISC40_bit at EICRB.B0;
    const register unsigned short int ISC40 = 0;
    sbit  ISC41_bit at EICRB.B1;
    const register unsigned short int ISC41 = 1;
    sbit  ISC50_bit at EICRB.B2;
    const register unsigned short int ISC50 = 2;
    sbit  ISC51_bit at EICRB.B3;
    const register unsigned short int ISC51 = 3;
    sbit  ISC60_bit at EICRB.B4;
    const register unsigned short int ISC60 = 4;
    sbit  ISC61_bit at EICRB.B5;
    const register unsigned short int ISC61 = 5;
    sbit  ISC70_bit at EICRB.B6;
    const register unsigned short int ISC70 = 6;
    sbit  ISC71_bit at EICRB.B7;
    const register unsigned short int ISC71 = 7;

sfr data unsigned short volatile EICRA absolute 0x69;
    sbit  ISC00_bit at EICRA.B0;
    const register unsigned short int ISC00 = 0;
    sbit  ISC01_bit at EICRA.B1;
    const register unsigned short int ISC01 = 1;
    sbit  ISC10_bit at EICRA.B2;
    const register unsigned short int ISC10 = 2;
    sbit  ISC11_bit at EICRA.B3;
    const register unsigned short int ISC11 = 3;
    sbit  ISC20_bit at EICRA.B4;
    const register unsigned short int ISC20 = 4;
    sbit  ISC21_bit at EICRA.B5;
    const register unsigned short int ISC21 = 5;
    sbit  ISC30_bit at EICRA.B6;
    const register unsigned short int ISC30 = 6;
    sbit  ISC31_bit at EICRA.B7;
    const register unsigned short int ISC31 = 7;

sfr data unsigned short PCICR absolute 0x68;
    sbit  PCIE0_bit at PCICR.B0;
    const register unsigned short int PCIE0 = 0;
    sbit  PCIE1_bit at PCICR.B1;
    const register unsigned short int PCIE1 = 1;
    sbit  PCIE2_bit at PCICR.B2;
    const register unsigned short int PCIE2 = 2;

sfr data unsigned short OSCCAL absolute 0x66;
    sbit  CAL0_bit at OSCCAL.B0;
    const register unsigned short int CAL0 = 0;
    sbit  CAL1_bit at OSCCAL.B1;
    const register unsigned short int CAL1 = 1;
    sbit  CAL2_bit at OSCCAL.B2;
    const register unsigned short int CAL2 = 2;
    sbit  CAL3_bit at OSCCAL.B3;
    const register unsigned short int CAL3 = 3;
    sbit  CAL4_bit at OSCCAL.B4;
    const register unsigned short int CAL4 = 4;
    sbit  CAL5_bit at OSCCAL.B5;
    const register unsigned short int CAL5 = 5;
    sbit  CAL6_bit at OSCCAL.B6;
    const register unsigned short int CAL6 = 6;
    sbit  CAL7_bit at OSCCAL.B7;
    const register unsigned short int CAL7 = 7;

sfr data unsigned short PRR1 absolute 0x65;
    sbit  PRUSART1_bit at PRR1.B0;
    const register unsigned short int PRUSART1 = 0;
    sbit  PRUSART2_bit at PRR1.B1;
    const register unsigned short int PRUSART2 = 1;
    sbit  PRUSART3_bit at PRR1.B2;
    const register unsigned short int PRUSART3 = 2;
    sbit  PRTIM3_bit at PRR1.B3;
    const register unsigned short int PRTIM3 = 3;
    sbit  PRTIM4_bit at PRR1.B4;
    const register unsigned short int PRTIM4 = 4;
    sbit  PRTIM5_bit at PRR1.B5;
    const register unsigned short int PRTIM5 = 5;

sfr data unsigned short PRR0 absolute 0x64;
    sbit  PRADC_bit at PRR0.B0;
    const register unsigned short int PRADC = 0;
    sbit  PRUSART0_bit at PRR0.B1;
    const register unsigned short int PRUSART0 = 1;
    sbit  PRSPI_bit at PRR0.B2;
    const register unsigned short int PRSPI = 2;
    sbit  PRTIM1_bit at PRR0.B3;
    const register unsigned short int PRTIM1 = 3;
    sbit  PRTIM0_bit at PRR0.B5;
    const register unsigned short int PRTIM0 = 5;
    sbit  PRTIM2_bit at PRR0.B6;
    const register unsigned short int PRTIM2 = 6;
    sbit  PRTWI_bit at PRR0.B7;
    const register unsigned short int PRTWI = 7;

sfr data unsigned short volatile CLKPR absolute 0x61;
    sbit  CLKPS0_bit at CLKPR.B0;
    const register unsigned short int CLKPS0 = 0;
    sbit  CLKPS1_bit at CLKPR.B1;
    const register unsigned short int CLKPS1 = 1;
    sbit  CLKPS2_bit at CLKPR.B2;
    const register unsigned short int CLKPS2 = 2;
    sbit  CLKPS3_bit at CLKPR.B3;
    const register unsigned short int CLKPS3 = 3;
    sbit  CLKPCE_bit at CLKPR.B7;
    const register unsigned short int CLKPCE = 7;

sfr data unsigned short volatile WDTCSR absolute 0x60;
    sbit  WDP0_bit at WDTCSR.B0;
    const register unsigned short int WDP0 = 0;
    sbit  WDP1_bit at WDTCSR.B1;
    const register unsigned short int WDP1 = 1;
    sbit  WDP2_bit at WDTCSR.B2;
    const register unsigned short int WDP2 = 2;
    sbit  WDE_bit at WDTCSR.B3;
    const register unsigned short int WDE = 3;
    sbit  WDCE_bit at WDTCSR.B4;
    const register unsigned short int WDCE = 4;
    sbit  WDP3_bit at WDTCSR.B5;
    const register unsigned short int WDP3 = 5;
    sbit  WDIE_bit at WDTCSR.B6;
    const register unsigned short int WDIE = 6;
    sbit  WDIF_bit at WDTCSR.B7;
    const register unsigned short int WDIF = 7;

sfr io unsigned short volatile SREG absolute 0x5F;
    sbit  SREG_C_bit at SREG.B0;
    const register unsigned short int SREG_C = 0;
    sbit  SREG_Z_bit at SREG.B1;
    const register unsigned short int SREG_Z = 1;
    sbit  SREG_N_bit at SREG.B2;
    const register unsigned short int SREG_N = 2;
    sbit  SREG_V_bit at SREG.B3;
    const register unsigned short int SREG_V = 3;
    sbit  SREG_S_bit at SREG.B4;
    const register unsigned short int SREG_S = 4;
    sbit  SREG_H_bit at SREG.B5;
    const register unsigned short int SREG_H = 5;
    sbit  SREG_T_bit at SREG.B6;
    const register unsigned short int SREG_T = 6;
    sbit  SREG_I_bit at SREG.B7;
    const register unsigned short int SREG_I = 7;

sfr io unsigned short volatile SPH absolute 0x5E;
    sbit  SP8_bit at SPH.B0;
    const register unsigned short int SP8 = 0;
    sbit  SP9_bit at SPH.B1;
    const register unsigned short int SP9 = 1;
    sbit  SP10_bit at SPH.B2;
    const register unsigned short int SP10 = 2;
    sbit  SP11_bit at SPH.B3;
    const register unsigned short int SP11 = 3;
    sbit  SP12_bit at SPH.B4;
    const register unsigned short int SP12 = 4;
    sbit  SP13_bit at SPH.B5;
    const register unsigned short int SP13 = 5;
    sbit  SP14_bit at SPH.B6;
    const register unsigned short int SP14 = 6;
    sbit  SP15_bit at SPH.B7;
    const register unsigned short int SP15 = 7;

sfr io unsigned short volatile SPL absolute 0x5D;
    sbit  SP0_bit at SPL.B0;
    const register unsigned short int SP0 = 0;
    sbit  SP1_bit at SPL.B1;
    const register unsigned short int SP1 = 1;
    sbit  SP2_bit at SPL.B2;
    const register unsigned short int SP2 = 2;
    sbit  SP3_bit at SPL.B3;
    const register unsigned short int SP3 = 3;
    sbit  SP4_bit at SPL.B4;
    const register unsigned short int SP4 = 4;
    sbit  SP5_bit at SPL.B5;
    const register unsigned short int SP5 = 5;
    sbit  SP6_bit at SPL.B6;
    const register unsigned short int SP6 = 6;
    sbit  SP7_bit at SPL.B7;
    const register unsigned short int SP7 = 7;

sfr io unsigned short volatile EIND absolute 0x5C;
    sbit  EIND0_bit at EIND.B0;
    const register unsigned short int EIND0 = 0;

sfr io unsigned short volatile RAMPZ absolute 0x5B;
    sbit  RAMPZ0_bit at RAMPZ.B0;
    const register unsigned short int RAMPZ0 = 0;
    sbit  RAMPZ1_bit at RAMPZ.B1;
    const register unsigned short int RAMPZ1 = 1;

sfr io unsigned short volatile SPMCSR absolute 0x57;
    sbit  SPMEN_bit at SPMCSR.B0;
    const register unsigned short int SPMEN = 0;
    sbit  PGERS_bit at SPMCSR.B1;
    const register unsigned short int PGERS = 1;
    sbit  PGWRT_bit at SPMCSR.B2;
    const register unsigned short int PGWRT = 2;
    sbit  BLBSET_bit at SPMCSR.B3;
    const register unsigned short int BLBSET = 3;
    sbit  RWWSRE_bit at SPMCSR.B4;
    const register unsigned short int RWWSRE = 4;
    sbit  SIGRD_bit at SPMCSR.B5;
    const register unsigned short int SIGRD = 5;
    sbit  RWWSB_bit at SPMCSR.B6;
    const register unsigned short int RWWSB = 6;
    sbit  SPMIE_bit at SPMCSR.B7;
    const register unsigned short int SPMIE = 7;

sfr io unsigned short volatile MCUCR absolute 0x55;
    sbit  JTD_bit at MCUCR.B7;
    const register unsigned short int JTD = 7;
    sbit  IVCE_bit at MCUCR.B0;
    const register unsigned short int IVCE = 0;
    sbit  IVSEL_bit at MCUCR.B1;
    const register unsigned short int IVSEL = 1;
    sbit  PUD_bit at MCUCR.B4;
    const register unsigned short int PUD = 4;

sfr io unsigned short volatile MCUSR absolute 0x54;
    sbit  JTRF_bit at MCUSR.B4;
    const register unsigned short int JTRF = 4;
    sbit  PORF_bit at MCUSR.B0;
    const register unsigned short int PORF = 0;
    sbit  EXTRF_bit at MCUSR.B1;
    const register unsigned short int EXTRF = 1;
    sbit  BORF_bit at MCUSR.B2;
    const register unsigned short int BORF = 2;
    sbit  WDRF_bit at MCUSR.B3;
    const register unsigned short int WDRF = 3;

sfr io unsigned short SMCR absolute 0x53;
    sbit  SE_bit at SMCR.B0;
    const register unsigned short int SE = 0;
    sbit  SM0_bit at SMCR.B1;
    const register unsigned short int SM0 = 1;
    sbit  SM1_bit at SMCR.B2;
    const register unsigned short int SM1 = 2;
    sbit  SM2_bit at SMCR.B3;
    const register unsigned short int SM2 = 3;

sfr io unsigned short volatile OCDR absolute 0x51;
    sbit  OCDR0_bit at OCDR.B0;
    const register unsigned short int OCDR0 = 0;
    sbit  OCDR1_bit at OCDR.B1;
    const register unsigned short int OCDR1 = 1;
    sbit  OCDR2_bit at OCDR.B2;
    const register unsigned short int OCDR2 = 2;
    sbit  OCDR3_bit at OCDR.B3;
    const register unsigned short int OCDR3 = 3;
    sbit  OCDR4_bit at OCDR.B4;
    const register unsigned short int OCDR4 = 4;
    sbit  OCDR5_bit at OCDR.B5;
    const register unsigned short int OCDR5 = 5;
    sbit  OCDR6_bit at OCDR.B6;
    const register unsigned short int OCDR6 = 6;
    sbit  OCDR7_bit at OCDR.B7;
    const register unsigned short int OCDR7 = 7;

sfr io unsigned short volatile ACSR absolute 0x50;
    sbit  ACIS0_bit at ACSR.B0;
    const register unsigned short int ACIS0 = 0;
    sbit  ACIS1_bit at ACSR.B1;
    const register unsigned short int ACIS1 = 1;
    sbit  ACIC_bit at ACSR.B2;
    const register unsigned short int ACIC = 2;
    sbit  ACIE_bit at ACSR.B3;
    const register unsigned short int ACIE = 3;
    sbit  ACI_bit at ACSR.B4;
    const register unsigned short int ACI = 4;
    sbit  ACO_bit at ACSR.B5;
    const register unsigned short int ACO = 5;
    sbit  ACBG_bit at ACSR.B6;
    const register unsigned short int ACBG = 6;
    sbit  ACD_bit at ACSR.B7;
    const register unsigned short int ACD = 7;

sfr io unsigned short volatile SPDR absolute 0x4E;
    sbit  SPDR0_bit at SPDR.B0;
    const register unsigned short int SPDR0 = 0;
    sbit  SPDR1_bit at SPDR.B1;
    const register unsigned short int SPDR1 = 1;
    sbit  SPDR2_bit at SPDR.B2;
    const register unsigned short int SPDR2 = 2;
    sbit  SPDR3_bit at SPDR.B3;
    const register unsigned short int SPDR3 = 3;
    sbit  SPDR4_bit at SPDR.B4;
    const register unsigned short int SPDR4 = 4;
    sbit  SPDR5_bit at SPDR.B5;
    const register unsigned short int SPDR5 = 5;
    sbit  SPDR6_bit at SPDR.B6;
    const register unsigned short int SPDR6 = 6;
    sbit  SPDR7_bit at SPDR.B7;
    const register unsigned short int SPDR7 = 7;

sfr io unsigned short volatile SPSR absolute 0x4D;
    sbit  SPI2X_bit at SPSR.B0;
    const register unsigned short int SPI2X = 0;
    sbit  WCOL_bit at SPSR.B6;
    const register unsigned short int WCOL = 6;
    sbit  SPIF_bit at SPSR.B7;
    const register unsigned short int SPIF = 7;

sfr io unsigned short SPCR absolute 0x4C;
    sbit  SPR0_bit at SPCR.B0;
    const register unsigned short int SPR0 = 0;
    sbit  SPR1_bit at SPCR.B1;
    const register unsigned short int SPR1 = 1;
    sbit  CPHA_bit at SPCR.B2;
    const register unsigned short int CPHA = 2;
    sbit  CPOL_bit at SPCR.B3;
    const register unsigned short int CPOL = 3;
    sbit  MSTR_bit at SPCR.B4;
    const register unsigned short int MSTR = 4;
    sbit  DORD_bit at SPCR.B5;
    const register unsigned short int DORD = 5;
    sbit  SPE_bit at SPCR.B6;
    const register unsigned short int SPE = 6;
    sbit  SPIE_bit at SPCR.B7;
    const register unsigned short int SPIE = 7;

sfr io unsigned short volatile GPIOR2 absolute 0x4B;
    sbit  GPIOR20_bit at GPIOR2.B0;
    const register unsigned short int GPIOR20 = 0;
    sbit  GPIOR21_bit at GPIOR2.B1;
    const register unsigned short int GPIOR21 = 1;
    sbit  GPIOR22_bit at GPIOR2.B2;
    const register unsigned short int GPIOR22 = 2;
    sbit  GPIOR23_bit at GPIOR2.B3;
    const register unsigned short int GPIOR23 = 3;
    sbit  GPIOR24_bit at GPIOR2.B4;
    const register unsigned short int GPIOR24 = 4;
    sbit  GPIOR25_bit at GPIOR2.B5;
    const register unsigned short int GPIOR25 = 5;
    sbit  GPIOR26_bit at GPIOR2.B6;
    const register unsigned short int GPIOR26 = 6;
    sbit  GPIOR27_bit at GPIOR2.B7;
    const register unsigned short int GPIOR27 = 7;

sfr io unsigned short volatile GPIOR1 absolute 0x4A;
    sbit  GPIOR10_bit at GPIOR1.B0;
    const register unsigned short int GPIOR10 = 0;
    sbit  GPIOR11_bit at GPIOR1.B1;
    const register unsigned short int GPIOR11 = 1;
    sbit  GPIOR12_bit at GPIOR1.B2;
    const register unsigned short int GPIOR12 = 2;
    sbit  GPIOR13_bit at GPIOR1.B3;
    const register unsigned short int GPIOR13 = 3;
    sbit  GPIOR14_bit at GPIOR1.B4;
    const register unsigned short int GPIOR14 = 4;
    sbit  GPIOR15_bit at GPIOR1.B5;
    const register unsigned short int GPIOR15 = 5;
    sbit  GPIOR16_bit at GPIOR1.B6;
    const register unsigned short int GPIOR16 = 6;
    sbit  GPIOR17_bit at GPIOR1.B7;
    const register unsigned short int GPIOR17 = 7;

sfr io unsigned short OCR0B absolute 0x48;
    sbit  OCR0B_0_bit at OCR0B.B0;
    const register unsigned short int OCR0B_0 = 0;
    sbit  OCR0B_1_bit at OCR0B.B1;
    const register unsigned short int OCR0B_1 = 1;
    sbit  OCR0B_2_bit at OCR0B.B2;
    const register unsigned short int OCR0B_2 = 2;
    sbit  OCR0B_3_bit at OCR0B.B3;
    const register unsigned short int OCR0B_3 = 3;
    sbit  OCR0B_4_bit at OCR0B.B4;
    const register unsigned short int OCR0B_4 = 4;
    sbit  OCR0B_5_bit at OCR0B.B5;
    const register unsigned short int OCR0B_5 = 5;
    sbit  OCR0B_6_bit at OCR0B.B6;
    const register unsigned short int OCR0B_6 = 6;
    sbit  OCR0B_7_bit at OCR0B.B7;
    const register unsigned short int OCR0B_7 = 7;

sfr io unsigned short OCR0A absolute 0x47;
    sbit  OCR0A_0_bit at OCR0A.B0;
    const register unsigned short int OCR0A_0 = 0;
    sbit  OCR0A_1_bit at OCR0A.B1;
    const register unsigned short int OCR0A_1 = 1;
    sbit  OCR0A_2_bit at OCR0A.B2;
    const register unsigned short int OCR0A_2 = 2;
    sbit  OCR0A_3_bit at OCR0A.B3;
    const register unsigned short int OCR0A_3 = 3;
    sbit  OCR0A_4_bit at OCR0A.B4;
    const register unsigned short int OCR0A_4 = 4;
    sbit  OCR0A_5_bit at OCR0A.B5;
    const register unsigned short int OCR0A_5 = 5;
    sbit  OCR0A_6_bit at OCR0A.B6;
    const register unsigned short int OCR0A_6 = 6;
    sbit  OCR0A_7_bit at OCR0A.B7;
    const register unsigned short int OCR0A_7 = 7;

sfr io unsigned short volatile TCNT0 absolute 0x46;
    sbit  TCNT0_0_bit at TCNT0.B0;
    const register unsigned short int TCNT0_0 = 0;
    sbit  TCNT0_1_bit at TCNT0.B1;
    const register unsigned short int TCNT0_1 = 1;
    sbit  TCNT0_2_bit at TCNT0.B2;
    const register unsigned short int TCNT0_2 = 2;
    sbit  TCNT0_3_bit at TCNT0.B3;
    const register unsigned short int TCNT0_3 = 3;
    sbit  TCNT0_4_bit at TCNT0.B4;
    const register unsigned short int TCNT0_4 = 4;
    sbit  TCNT0_5_bit at TCNT0.B5;
    const register unsigned short int TCNT0_5 = 5;
    sbit  TCNT0_6_bit at TCNT0.B6;
    const register unsigned short int TCNT0_6 = 6;
    sbit  TCNT0_7_bit at TCNT0.B7;
    const register unsigned short int TCNT0_7 = 7;

sfr io unsigned short TCCR0B absolute 0x45;
    sbit  CS00_bit at TCCR0B.B0;
    const register unsigned short int CS00 = 0;
    sbit  CS01_bit at TCCR0B.B1;
    const register unsigned short int CS01 = 1;
    sbit  CS02_bit at TCCR0B.B2;
    const register unsigned short int CS02 = 2;
    sbit  WGM02_bit at TCCR0B.B3;
    const register unsigned short int WGM02 = 3;
    sbit  FOC0B_bit at TCCR0B.B6;
    const register unsigned short int FOC0B = 6;
    sbit  FOC0A_bit at TCCR0B.B7;
    const register unsigned short int FOC0A = 7;

sfr io unsigned short TCCR0A absolute 0x44;
    sbit  WGM00_bit at TCCR0A.B0;
    const register unsigned short int WGM00 = 0;
    sbit  WGM01_bit at TCCR0A.B1;
    const register unsigned short int WGM01 = 1;
    sbit  COM0B0_bit at TCCR0A.B4;
    const register unsigned short int COM0B0 = 4;
    sbit  COM0B1_bit at TCCR0A.B5;
    const register unsigned short int COM0B1 = 5;
    sbit  COM0A0_bit at TCCR0A.B6;
    const register unsigned short int COM0A0 = 6;
    sbit  COM0A1_bit at TCCR0A.B7;
    const register unsigned short int COM0A1 = 7;

sfr io unsigned short volatile GTCCR absolute 0x43;
    sbit  PSRSYNC_bit at GTCCR.B0;
    const register unsigned short int PSRSYNC = 0;
    sbit  TSM_bit at GTCCR.B7;
    const register unsigned short int TSM = 7;
    sbit  PSRASY_bit at GTCCR.B1;
    const register unsigned short int PSRASY = 1;

sfr io unsigned short EEARH absolute 0x42;
    sbit  EEAR8_bit at EEARH.B0;
    const register unsigned short int EEAR8 = 0;
    sbit  EEAR9_bit at EEARH.B1;
    const register unsigned short int EEAR9 = 1;
    sbit  EEAR10_bit at EEARH.B2;
    const register unsigned short int EEAR10 = 2;
    sbit  EEAR11_bit at EEARH.B3;
    const register unsigned short int EEAR11 = 3;

sfr io unsigned short EEARL absolute 0x41;
    sbit  EEAR0_bit at EEARL.B0;
    const register unsigned short int EEAR0 = 0;
    sbit  EEAR1_bit at EEARL.B1;
    const register unsigned short int EEAR1 = 1;
    sbit  EEAR2_bit at EEARL.B2;
    const register unsigned short int EEAR2 = 2;
    sbit  EEAR3_bit at EEARL.B3;
    const register unsigned short int EEAR3 = 3;
    sbit  EEAR4_bit at EEARL.B4;
    const register unsigned short int EEAR4 = 4;
    sbit  EEAR5_bit at EEARL.B5;
    const register unsigned short int EEAR5 = 5;
    sbit  EEAR6_bit at EEARL.B6;
    const register unsigned short int EEAR6 = 6;
    sbit  EEAR7_bit at EEARL.B7;
    const register unsigned short int EEAR7 = 7;

sfr io unsigned short volatile EEDR absolute 0x40;
    sbit  EEDR0_bit at EEDR.B0;
    const register unsigned short int EEDR0 = 0;
    sbit  EEDR1_bit at EEDR.B1;
    const register unsigned short int EEDR1 = 1;
    sbit  EEDR2_bit at EEDR.B2;
    const register unsigned short int EEDR2 = 2;
    sbit  EEDR3_bit at EEDR.B3;
    const register unsigned short int EEDR3 = 3;
    sbit  EEDR4_bit at EEDR.B4;
    const register unsigned short int EEDR4 = 4;
    sbit  EEDR5_bit at EEDR.B5;
    const register unsigned short int EEDR5 = 5;
    sbit  EEDR6_bit at EEDR.B6;
    const register unsigned short int EEDR6 = 6;
    sbit  EEDR7_bit at EEDR.B7;
    const register unsigned short int EEDR7 = 7;

sfr io unsigned short volatile EECR absolute 0x3F;
    sbit  EERE_bit at EECR.B0;
    const register unsigned short int EERE = 0;
    sbit  EEPE_bit at EECR.B1;
    const register unsigned short int EEPE = 1;
    sbit  EEMPE_bit at EECR.B2;
    const register unsigned short int EEMPE = 2;
    sbit  EERIE_bit at EECR.B3;
    const register unsigned short int EERIE = 3;
    sbit  EEPM0_bit at EECR.B4;
    const register unsigned short int EEPM0 = 4;
    sbit  EEPM1_bit at EECR.B5;
    const register unsigned short int EEPM1 = 5;

sfr io unsigned short volatile GPIOR0 absolute 0x3E;
    sbit  GPIOR00_bit at GPIOR0.B0;
    const register unsigned short int GPIOR00 = 0;
    sbit  GPIOR01_bit at GPIOR0.B1;
    const register unsigned short int GPIOR01 = 1;
    sbit  GPIOR02_bit at GPIOR0.B2;
    const register unsigned short int GPIOR02 = 2;
    sbit  GPIOR03_bit at GPIOR0.B3;
    const register unsigned short int GPIOR03 = 3;
    sbit  GPIOR04_bit at GPIOR0.B4;
    const register unsigned short int GPIOR04 = 4;
    sbit  GPIOR05_bit at GPIOR0.B5;
    const register unsigned short int GPIOR05 = 5;
    sbit  GPIOR06_bit at GPIOR0.B6;
    const register unsigned short int GPIOR06 = 6;
    sbit  GPIOR07_bit at GPIOR0.B7;
    const register unsigned short int GPIOR07 = 7;

sfr io unsigned short EIMSK absolute 0x3D;
    sbit  INT0_bit at EIMSK.B0;
    const register unsigned short int INT0 = 0;
    sbit  INT1_bit at EIMSK.B1;
    const register unsigned short int INT1 = 1;
    sbit  INT2_bit at EIMSK.B2;
    const register unsigned short int INT2 = 2;
    sbit  INT3_bit at EIMSK.B3;
    const register unsigned short int INT3 = 3;
    sbit  INT4_bit at EIMSK.B4;
    const register unsigned short int INT4 = 4;
    sbit  INT5_bit at EIMSK.B5;
    const register unsigned short int INT5 = 5;
    sbit  INT6_bit at EIMSK.B6;
    const register unsigned short int INT6 = 6;
    sbit  INT7_bit at EIMSK.B7;
    const register unsigned short int INT7 = 7;

sfr io unsigned short volatile EIFR absolute 0x3C;
    sbit  INTF0_bit at EIFR.B0;
    const register unsigned short int INTF0 = 0;
    sbit  INTF1_bit at EIFR.B1;
    const register unsigned short int INTF1 = 1;
    sbit  INTF2_bit at EIFR.B2;
    const register unsigned short int INTF2 = 2;
    sbit  INTF3_bit at EIFR.B3;
    const register unsigned short int INTF3 = 3;
    sbit  INTF4_bit at EIFR.B4;
    const register unsigned short int INTF4 = 4;
    sbit  INTF5_bit at EIFR.B5;
    const register unsigned short int INTF5 = 5;
    sbit  INTF6_bit at EIFR.B6;
    const register unsigned short int INTF6 = 6;
    sbit  INTF7_bit at EIFR.B7;
    const register unsigned short int INTF7 = 7;

sfr io unsigned short volatile PCIFR absolute 0x3B;
    sbit  PCIF0_bit at PCIFR.B0;
    const register unsigned short int PCIF0 = 0;
    sbit  PCIF1_bit at PCIFR.B1;
    const register unsigned short int PCIF1 = 1;
    sbit  PCIF2_bit at PCIFR.B2;
    const register unsigned short int PCIF2 = 2;

sfr io unsigned short volatile TIFR5 absolute 0x3A;
    sbit  TOV5_bit at TIFR5.B0;
    const register unsigned short int TOV5 = 0;
    sbit  OCF5A_bit at TIFR5.B1;
    const register unsigned short int OCF5A = 1;
    sbit  OCF5B_bit at TIFR5.B2;
    const register unsigned short int OCF5B = 2;
    sbit  OCF5C_bit at TIFR5.B3;
    const register unsigned short int OCF5C = 3;
    sbit  ICF5_bit at TIFR5.B5;
    const register unsigned short int ICF5 = 5;

sfr io unsigned short volatile TIFR4 absolute 0x39;
    sbit  TOV4_bit at TIFR4.B0;
    const register unsigned short int TOV4 = 0;
    sbit  OCF4A_bit at TIFR4.B1;
    const register unsigned short int OCF4A = 1;
    sbit  OCF4B_bit at TIFR4.B2;
    const register unsigned short int OCF4B = 2;
    sbit  OCF4C_bit at TIFR4.B3;
    const register unsigned short int OCF4C = 3;
    sbit  ICF4_bit at TIFR4.B5;
    const register unsigned short int ICF4 = 5;

sfr io unsigned short volatile TIFR3 absolute 0x38;
    sbit  TOV3_bit at TIFR3.B0;
    const register unsigned short int TOV3 = 0;
    sbit  OCF3A_bit at TIFR3.B1;
    const register unsigned short int OCF3A = 1;
    sbit  OCF3B_bit at TIFR3.B2;
    const register unsigned short int OCF3B = 2;
    sbit  OCF3C_bit at TIFR3.B3;
    const register unsigned short int OCF3C = 3;
    sbit  ICF3_bit at TIFR3.B5;
    const register unsigned short int ICF3 = 5;

sfr io unsigned short volatile TIFR2 absolute 0x37;
    sbit  TOV2_bit at TIFR2.B0;
    const register unsigned short int TOV2 = 0;
    sbit  OCF2A_bit at TIFR2.B1;
    const register unsigned short int OCF2A = 1;
    sbit  OCF2B_bit at TIFR2.B2;
    const register unsigned short int OCF2B = 2;

sfr io unsigned short volatile TIFR1 absolute 0x36;
    sbit  TOV1_bit at TIFR1.B0;
    const register unsigned short int TOV1 = 0;
    sbit  OCF1A_bit at TIFR1.B1;
    const register unsigned short int OCF1A = 1;
    sbit  OCF1B_bit at TIFR1.B2;
    const register unsigned short int OCF1B = 2;
    sbit  OCF1C_bit at TIFR1.B3;
    const register unsigned short int OCF1C = 3;
    sbit  ICF1_bit at TIFR1.B5;
    const register unsigned short int ICF1 = 5;

sfr io unsigned short volatile TIFR0 absolute 0x35;
    sbit  TOV0_bit at TIFR0.B0;
    const register unsigned short int TOV0 = 0;
    sbit  OCF0A_bit at TIFR0.B1;
    const register unsigned short int OCF0A = 1;
    sbit  OCF0B_bit at TIFR0.B2;
    const register unsigned short int OCF0B = 2;

sfr io unsigned short PORTG absolute 0x34;
    sbit  PORTG0_bit at PORTG.B0;
    const register unsigned short int PORTG0 = 0;
    sbit  PORTG1_bit at PORTG.B1;
    const register unsigned short int PORTG1 = 1;
    sbit  PORTG2_bit at PORTG.B2;
    const register unsigned short int PORTG2 = 2;
    sbit  PORTG3_bit at PORTG.B3;
    const register unsigned short int PORTG3 = 3;
    sbit  PORTG4_bit at PORTG.B4;
    const register unsigned short int PORTG4 = 4;
    sbit  PORTG5_bit at PORTG.B5;
    const register unsigned short int PORTG5 = 5;

sfr io unsigned short DDRG absolute 0x33;
    sbit  DDG0_bit at DDRG.B0;
    const register unsigned short int DDG0 = 0;
    sbit  DDG1_bit at DDRG.B1;
    const register unsigned short int DDG1 = 1;
    sbit  DDG2_bit at DDRG.B2;
    const register unsigned short int DDG2 = 2;
    sbit  DDG3_bit at DDRG.B3;
    const register unsigned short int DDG3 = 3;
    sbit  DDG4_bit at DDRG.B4;
    const register unsigned short int DDG4 = 4;
    sbit  DDG5_bit at DDRG.B5;
    const register unsigned short int DDG5 = 5;

sfr io unsigned short volatile PING absolute 0x32;
    sbit  PING0_bit at PING.B0;
    const register unsigned short int PING0 = 0;
    sbit  PING1_bit at PING.B1;
    const register unsigned short int PING1 = 1;
    sbit  PING2_bit at PING.B2;
    const register unsigned short int PING2 = 2;
    sbit  PING3_bit at PING.B3;
    const register unsigned short int PING3 = 3;
    sbit  PING4_bit at PING.B4;
    const register unsigned short int PING4 = 4;
    sbit  PING5_bit at PING.B5;
    const register unsigned short int PING5 = 5;

sfr io unsigned short PORTF absolute 0x31;
    sbit  PORTF0_bit at PORTF.B0;
    const register unsigned short int PORTF0 = 0;
    sbit  PORTF1_bit at PORTF.B1;
    const register unsigned short int PORTF1 = 1;
    sbit  PORTF2_bit at PORTF.B2;
    const register unsigned short int PORTF2 = 2;
    sbit  PORTF3_bit at PORTF.B3;
    const register unsigned short int PORTF3 = 3;
    sbit  PORTF4_bit at PORTF.B4;
    const register unsigned short int PORTF4 = 4;
    sbit  PORTF5_bit at PORTF.B5;
    const register unsigned short int PORTF5 = 5;
    sbit  PORTF6_bit at PORTF.B6;
    const register unsigned short int PORTF6 = 6;
    sbit  PORTF7_bit at PORTF.B7;
    const register unsigned short int PORTF7 = 7;

sfr io unsigned short DDRF absolute 0x30;
    sbit  DDF0_bit at DDRF.B0;
    const register unsigned short int DDF0 = 0;
    sbit  DDF1_bit at DDRF.B1;
    const register unsigned short int DDF1 = 1;
    sbit  DDF2_bit at DDRF.B2;
    const register unsigned short int DDF2 = 2;
    sbit  DDF3_bit at DDRF.B3;
    const register unsigned short int DDF3 = 3;
    sbit  DDF4_bit at DDRF.B4;
    const register unsigned short int DDF4 = 4;
    sbit  DDF5_bit at DDRF.B5;
    const register unsigned short int DDF5 = 5;
    sbit  DDF6_bit at DDRF.B6;
    const register unsigned short int DDF6 = 6;
    sbit  DDF7_bit at DDRF.B7;
    const register unsigned short int DDF7 = 7;

sfr io unsigned short volatile PINF absolute 0x2F;
    sbit  PINF0_bit at PINF.B0;
    const register unsigned short int PINF0 = 0;
    sbit  PINF1_bit at PINF.B1;
    const register unsigned short int PINF1 = 1;
    sbit  PINF2_bit at PINF.B2;
    const register unsigned short int PINF2 = 2;
    sbit  PINF3_bit at PINF.B3;
    const register unsigned short int PINF3 = 3;
    sbit  PINF4_bit at PINF.B4;
    const register unsigned short int PINF4 = 4;
    sbit  PINF5_bit at PINF.B5;
    const register unsigned short int PINF5 = 5;
    sbit  PINF6_bit at PINF.B6;
    const register unsigned short int PINF6 = 6;
    sbit  PINF7_bit at PINF.B7;
    const register unsigned short int PINF7 = 7;

sfr io unsigned short PORTE absolute 0x2E;
    sbit  PORTE0_bit at PORTE.B0;
    const register unsigned short int PORTE0 = 0;
    sbit  PORTE1_bit at PORTE.B1;
    const register unsigned short int PORTE1 = 1;
    sbit  PORTE2_bit at PORTE.B2;
    const register unsigned short int PORTE2 = 2;
    sbit  PORTE3_bit at PORTE.B3;
    const register unsigned short int PORTE3 = 3;
    sbit  PORTE4_bit at PORTE.B4;
    const register unsigned short int PORTE4 = 4;
    sbit  PORTE5_bit at PORTE.B5;
    const register unsigned short int PORTE5 = 5;
    sbit  PORTE6_bit at PORTE.B6;
    const register unsigned short int PORTE6 = 6;
    sbit  PORTE7_bit at PORTE.B7;
    const register unsigned short int PORTE7 = 7;

sfr io unsigned short DDRE absolute 0x2D;
    sbit  DDE0_bit at DDRE.B0;
    const register unsigned short int DDE0 = 0;
    sbit  DDE1_bit at DDRE.B1;
    const register unsigned short int DDE1 = 1;
    sbit  DDE2_bit at DDRE.B2;
    const register unsigned short int DDE2 = 2;
    sbit  DDE3_bit at DDRE.B3;
    const register unsigned short int DDE3 = 3;
    sbit  DDE4_bit at DDRE.B4;
    const register unsigned short int DDE4 = 4;
    sbit  DDE5_bit at DDRE.B5;
    const register unsigned short int DDE5 = 5;
    sbit  DDE6_bit at DDRE.B6;
    const register unsigned short int DDE6 = 6;
    sbit  DDE7_bit at DDRE.B7;
    const register unsigned short int DDE7 = 7;

sfr io unsigned short volatile PINE absolute 0x2C;
    sbit  PINE0_bit at PINE.B0;
    const register unsigned short int PINE0 = 0;
    sbit  PINE1_bit at PINE.B1;
    const register unsigned short int PINE1 = 1;
    sbit  PINE2_bit at PINE.B2;
    const register unsigned short int PINE2 = 2;
    sbit  PINE3_bit at PINE.B3;
    const register unsigned short int PINE3 = 3;
    sbit  PINE4_bit at PINE.B4;
    const register unsigned short int PINE4 = 4;
    sbit  PINE5_bit at PINE.B5;
    const register unsigned short int PINE5 = 5;
    sbit  PINE6_bit at PINE.B6;
    const register unsigned short int PINE6 = 6;
    sbit  PINE7_bit at PINE.B7;
    const register unsigned short int PINE7 = 7;

sfr io unsigned short PORTD absolute 0x2B;
    sbit  PORTD0_bit at PORTD.B0;
    const register unsigned short int PORTD0 = 0;
    sbit  PORTD1_bit at PORTD.B1;
    const register unsigned short int PORTD1 = 1;
    sbit  PORTD2_bit at PORTD.B2;
    const register unsigned short int PORTD2 = 2;
    sbit  PORTD3_bit at PORTD.B3;
    const register unsigned short int PORTD3 = 3;
    sbit  PORTD4_bit at PORTD.B4;
    const register unsigned short int PORTD4 = 4;
    sbit  PORTD5_bit at PORTD.B5;
    const register unsigned short int PORTD5 = 5;
    sbit  PORTD6_bit at PORTD.B6;
    const register unsigned short int PORTD6 = 6;
    sbit  PORTD7_bit at PORTD.B7;
    const register unsigned short int PORTD7 = 7;

sfr io unsigned short DDRD absolute 0x2A;
    sbit  DDD0_bit at DDRD.B0;
    const register unsigned short int DDD0 = 0;
    sbit  DDD1_bit at DDRD.B1;
    const register unsigned short int DDD1 = 1;
    sbit  DDD2_bit at DDRD.B2;
    const register unsigned short int DDD2 = 2;
    sbit  DDD3_bit at DDRD.B3;
    const register unsigned short int DDD3 = 3;
    sbit  DDD4_bit at DDRD.B4;
    const register unsigned short int DDD4 = 4;
    sbit  DDD5_bit at DDRD.B5;
    const register unsigned short int DDD5 = 5;
    sbit  DDD6_bit at DDRD.B6;
    const register unsigned short int DDD6 = 6;
    sbit  DDD7_bit at DDRD.B7;
    const register unsigned short int DDD7 = 7;

sfr io unsigned short volatile PIND absolute 0x29;
    sbit  PIND0_bit at PIND.B0;
    const register unsigned short int PIND0 = 0;
    sbit  PIND1_bit at PIND.B1;
    const register unsigned short int PIND1 = 1;
    sbit  PIND2_bit at PIND.B2;
    const register unsigned short int PIND2 = 2;
    sbit  PIND3_bit at PIND.B3;
    const register unsigned short int PIND3 = 3;
    sbit  PIND4_bit at PIND.B4;
    const register unsigned short int PIND4 = 4;
    sbit  PIND5_bit at PIND.B5;
    const register unsigned short int PIND5 = 5;
    sbit  PIND6_bit at PIND.B6;
    const register unsigned short int PIND6 = 6;
    sbit  PIND7_bit at PIND.B7;
    const register unsigned short int PIND7 = 7;

sfr io unsigned short PORTC absolute 0x28;
    sbit  PORTC0_bit at PORTC.B0;
    const register unsigned short int PORTC0 = 0;
    sbit  PORTC1_bit at PORTC.B1;
    const register unsigned short int PORTC1 = 1;
    sbit  PORTC2_bit at PORTC.B2;
    const register unsigned short int PORTC2 = 2;
    sbit  PORTC3_bit at PORTC.B3;
    const register unsigned short int PORTC3 = 3;
    sbit  PORTC4_bit at PORTC.B4;
    const register unsigned short int PORTC4 = 4;
    sbit  PORTC5_bit at PORTC.B5;
    const register unsigned short int PORTC5 = 5;
    sbit  PORTC6_bit at PORTC.B6;
    const register unsigned short int PORTC6 = 6;
    sbit  PORTC7_bit at PORTC.B7;
    const register unsigned short int PORTC7 = 7;

sfr io unsigned short DDRC absolute 0x27;
    sbit  DDC0_bit at DDRC.B0;
    const register unsigned short int DDC0 = 0;
    sbit  DDC1_bit at DDRC.B1;
    const register unsigned short int DDC1 = 1;
    sbit  DDC2_bit at DDRC.B2;
    const register unsigned short int DDC2 = 2;
    sbit  DDC3_bit at DDRC.B3;
    const register unsigned short int DDC3 = 3;
    sbit  DDC4_bit at DDRC.B4;
    const register unsigned short int DDC4 = 4;
    sbit  DDC5_bit at DDRC.B5;
    const register unsigned short int DDC5 = 5;
    sbit  DDC6_bit at DDRC.B6;
    const register unsigned short int DDC6 = 6;
    sbit  DDC7_bit at DDRC.B7;
    const register unsigned short int DDC7 = 7;

sfr io unsigned short volatile PINC absolute 0x26;
    sbit  PINC0_bit at PINC.B0;
    const register unsigned short int PINC0 = 0;
    sbit  PINC1_bit at PINC.B1;
    const register unsigned short int PINC1 = 1;
    sbit  PINC2_bit at PINC.B2;
    const register unsigned short int PINC2 = 2;
    sbit  PINC3_bit at PINC.B3;
    const register unsigned short int PINC3 = 3;
    sbit  PINC4_bit at PINC.B4;
    const register unsigned short int PINC4 = 4;
    sbit  PINC5_bit at PINC.B5;
    const register unsigned short int PINC5 = 5;
    sbit  PINC6_bit at PINC.B6;
    const register unsigned short int PINC6 = 6;
    sbit  PINC7_bit at PINC.B7;
    const register unsigned short int PINC7 = 7;

sfr io unsigned short PORTB absolute 0x25;
    sbit  PORTB0_bit at PORTB.B0;
    const register unsigned short int PORTB0 = 0;
    sbit  PORTB1_bit at PORTB.B1;
    const register unsigned short int PORTB1 = 1;
    sbit  PORTB2_bit at PORTB.B2;
    const register unsigned short int PORTB2 = 2;
    sbit  PORTB3_bit at PORTB.B3;
    const register unsigned short int PORTB3 = 3;
    sbit  PORTB4_bit at PORTB.B4;
    const register unsigned short int PORTB4 = 4;
    sbit  PORTB5_bit at PORTB.B5;
    const register unsigned short int PORTB5 = 5;
    sbit  PORTB6_bit at PORTB.B6;
    const register unsigned short int PORTB6 = 6;
    sbit  PORTB7_bit at PORTB.B7;
    const register unsigned short int PORTB7 = 7;

sfr io unsigned short DDRB absolute 0x24;
    sbit  DDB0_bit at DDRB.B0;
    const register unsigned short int DDB0 = 0;
    sbit  DDB1_bit at DDRB.B1;
    const register unsigned short int DDB1 = 1;
    sbit  DDB2_bit at DDRB.B2;
    const register unsigned short int DDB2 = 2;
    sbit  DDB3_bit at DDRB.B3;
    const register unsigned short int DDB3 = 3;
    sbit  DDB4_bit at DDRB.B4;
    const register unsigned short int DDB4 = 4;
    sbit  DDB5_bit at DDRB.B5;
    const register unsigned short int DDB5 = 5;
    sbit  DDB6_bit at DDRB.B6;
    const register unsigned short int DDB6 = 6;
    sbit  DDB7_bit at DDRB.B7;
    const register unsigned short int DDB7 = 7;

sfr io unsigned short volatile PINB absolute 0x23;
    sbit  PINB0_bit at PINB.B0;
    const register unsigned short int PINB0 = 0;
    sbit  PINB1_bit at PINB.B1;
    const register unsigned short int PINB1 = 1;
    sbit  PINB2_bit at PINB.B2;
    const register unsigned short int PINB2 = 2;
    sbit  PINB3_bit at PINB.B3;
    const register unsigned short int PINB3 = 3;
    sbit  PINB4_bit at PINB.B4;
    const register unsigned short int PINB4 = 4;
    sbit  PINB5_bit at PINB.B5;
    const register unsigned short int PINB5 = 5;
    sbit  PINB6_bit at PINB.B6;
    const register unsigned short int PINB6 = 6;
    sbit  PINB7_bit at PINB.B7;
    const register unsigned short int PINB7 = 7;

sfr io unsigned short PORTA absolute 0x22;
    sbit  PORTA0_bit at PORTA.B0;
    const register unsigned short int PORTA0 = 0;
    sbit  PORTA1_bit at PORTA.B1;
    const register unsigned short int PORTA1 = 1;
    sbit  PORTA2_bit at PORTA.B2;
    const register unsigned short int PORTA2 = 2;
    sbit  PORTA3_bit at PORTA.B3;
    const register unsigned short int PORTA3 = 3;
    sbit  PORTA4_bit at PORTA.B4;
    const register unsigned short int PORTA4 = 4;
    sbit  PORTA5_bit at PORTA.B5;
    const register unsigned short int PORTA5 = 5;
    sbit  PORTA6_bit at PORTA.B6;
    const register unsigned short int PORTA6 = 6;
    sbit  PORTA7_bit at PORTA.B7;
    const register unsigned short int PORTA7 = 7;

sfr io unsigned short DDRA absolute 0x21;
    sbit  DDA0_bit at DDRA.B0;
    const register unsigned short int DDA0 = 0;
    sbit  DDA1_bit at DDRA.B1;
    const register unsigned short int DDA1 = 1;
    sbit  DDA2_bit at DDRA.B2;
    const register unsigned short int DDA2 = 2;
    sbit  DDA3_bit at DDRA.B3;
    const register unsigned short int DDA3 = 3;
    sbit  DDA4_bit at DDRA.B4;
    const register unsigned short int DDA4 = 4;
    sbit  DDA5_bit at DDRA.B5;
    const register unsigned short int DDA5 = 5;
    sbit  DDA6_bit at DDRA.B6;
    const register unsigned short int DDA6 = 6;
    sbit  DDA7_bit at DDRA.B7;
    const register unsigned short int DDA7 = 7;

sfr io unsigned short volatile PINA absolute 0x20;
    sbit  PINA0_bit at PINA.B0;
    const register unsigned short int PINA0 = 0;
    sbit  PINA1_bit at PINA.B1;
    const register unsigned short int PINA1 = 1;
    sbit  PINA2_bit at PINA.B2;
    const register unsigned short int PINA2 = 2;
    sbit  PINA3_bit at PINA.B3;
    const register unsigned short int PINA3 = 3;
    sbit  PINA4_bit at PINA.B4;
    const register unsigned short int PINA4 = 4;
    sbit  PINA5_bit at PINA.B5;
    const register unsigned short int PINA5 = 5;
    sbit  PINA6_bit at PINA.B6;
    const register unsigned short int PINA6 = 6;
    sbit  PINA7_bit at PINA.B7;
    const register unsigned short int PINA7 = 7;
